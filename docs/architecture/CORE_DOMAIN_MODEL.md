...
the audited nonthrowing helpers. The runtime is built on the completed no-exceptions/no-compiler-RTTI
policy; user-authored input must never reach assertion, termination, throwing access, or unchecked
lookup paths.
