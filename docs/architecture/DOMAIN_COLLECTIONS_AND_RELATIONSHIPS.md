...
Editor categories/tags organize source only. Trait owner kinds are Room, Character, Interactable, and Feature. Scene, Dialogue, Verb, Interaction, and Map are immutable program/vocabulary definitions and are not Property or Trait owners. A Trait owns required or optionally Defaulted typed Property contracts; it does not contribute structural fields or create a separate runtime value family. Identity Property schemas are owner-local rather than entries in a project-wide registry, so the same key may have unrelated schemas on different exact owners.

Identity Property resolution is: target runtime override, the most-specific concrete/local authored Value or definition Default, inherited Archetype Default where applicable, configured value from an attached Trait, then typed missing. Exact Interactable Instance Values are more specific than all definition-side layers. Global Property resolution is its runtime override followed by its Variable-authored Value. Unset removes one override and resumes lookup; explicit nullable null remains a value. A write to a key with no effective authored schema creates schema-less dynamic session state instead; such values remain persisted but never become Project authoring content. Identity values never propagate between owners.

Compilation rejects missing or owner-incompatible Trait attachments, incompatible same-key schemas, conflicting Trait Defaults, and unsatisfied required members on concrete identities. Reusable definitions and Archetypes may intentionally leave required members unresolved for a more-specific concrete owner. Universal same-type gameplay `extends` is retired.

## Archetypes

Declared Rooms, Characters, and Interactables may attach at most one same-kind Archetype. An Archetype may itself name at most one same-kind base Archetype, producing a single acyclic chain. Archetypes contribute immutable structural configuration, Trait attachments, and reusable Property schemas/Defaults; the declared record may author more-specific overrides on top of the resolved chain. Character `initialWorldState` and exact Interactable Instance state are identity-local and are never inherited. Clearing an override reveals the next Archetype value, while detaching materializes the current effective configuration into the declared record.
