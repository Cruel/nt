...
| `noveltea_tooling_native` / bundled `noveltea` | Native tooling links only the engine-adjacent typed modules it needs; the scriptc CLI reaches those operations through its narrow static-host/C-ABI bridge. The retired `noveltea-editor-tool` frontend is no longer built or shipped. |
