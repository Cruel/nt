...
| SCR-12 | Lua can set/clear a runtime layout slot through typed layout IDs and can query the resulting layout state; invalid layouts/slots fail without partial mutation. | retained-complete | `SessionState::layout/set_layout/clear_layout`, `RuntimeScriptApi`, and `noveltea.layouts.get/set/clear` own the capability. `tests/script/typed_runtime_session_tests.cpp` proves valid round-trip, one typed view publication, missing-layout rejection, and clear behavior; core slot validation is covered by `tests/core/session_state_tests.cpp`. |
...
| SCR-14 | Lua requests typed audio playback/stop/channel changes through the runtime host seam; unavailable or failed host execution returns diagnostics and awaited completion resumes only the matching blocker. | retained-complete | `RuntimeScriptApi` and `TypedRuntimeSession` emit `AudioOperation`; `RuntimeAudioAdapter` resolves compiled audio Asset IDs and executes them through `AudioSystem`, while `RuntimeUI` reports failures and dispatches exact completion/cancellation inputs. Public Lua exposes `audio.play/play_and_wait/stop/stop_and_wait/state`; the backend-captured binding was deleted. `tests/script/runtime_audio_adapter_tests.cpp`, `tests/script/typed_runtime_session_tests.cpp`, and `tests/script/script_runtime_tests.cpp` prove backend execution, invalid/missing/wrong-kind Asset failure, immediate and sequential awaited operations, stale correlation rejection, and absence of raw backend closures. |
