...
| SCR-11 | Lua can present/hide a Map, select/focus a location, change the defined mode/visibility, and activate a connection; operations validate against authoritative Room exits and publish the same typed Map view as player input. | retained-complete | `bind_runtime_capabilities.cpp`, `RuntimeScriptApi`, `RuntimeCommandGateway`, and `runtime::RuntimeSession` expose `noveltea.map.present/hide/select/activate/state`; activation enqueues the validated navigation command rather than mutating Flow reentrantly. Executor behavior remains covered by `tests/script/typed_map_execution_tests.cpp`; Lua success, invalid-ID/mode atomicity, view publication, and navigation are covered by `tests/script/typed_runtime_session_tests.cpp`. |
...
| MAP-01 | A Map is presentation over authoritative Room topology; present/hide, focus/selection, mode, visibility, connection availability, and activation are typed and deterministic. | retained-complete | `runtime::RuntimeExecutor` Map methods and `SessionState::MapPresentationState`; `tests/script/typed_map_execution_tests.cpp` and typed RmlUi Map snapshot tests in `tests/ui/rmlui_custom_components_tests.cpp`. Public Lua exposure is separately tracked by SCR-11. |
