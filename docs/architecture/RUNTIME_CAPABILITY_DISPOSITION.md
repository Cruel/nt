...
| SCR-03 | Engine waits and logical timers advance from session time deterministically, complete/cancel only for the matching owner, and persist as logical timer records rather than backend clocks. | retained-complete | `SessionState`, `FlowExecutor`, `runtime::RuntimeExecutor`; `tests/core/shared_evaluator_tests.cpp` engine-wait case and `tests/core/save_state_tests.cpp` deterministic time/timer and save/restore cases. |
| SCR-04 | Script-visible random operations use session-owned deterministic state whose sequence survives save/restore exactly and cannot depend on platform RNG state. | retained-complete | `SessionState` owns the specified SplitMix64 state; `RuntimeScriptApi` exposes `noveltea.random` and routes `math.random`/`math.randomseed` to the same generator; the current save state inside Save File V1 persists `randomState`. Fixed-stream, invalid-range atomicity, Lua-wrapper, and exact save/load continuation coverage is in `tests/core/session_state_tests.cpp`, `tests/core/save_state_tests.cpp`, and `tests/script/typed_runtime_session_tests.cpp`. |
| SCR-05 | Lua reads definitions for Room, Scene, Dialogue, Interaction, Map, Character, Interactable, Verb, and resources through typed summaries; missing/wrong-kind IDs fail explicitly. | retained-complete | `RuntimeScriptApi::definition`, `RuntimeCommandGateway`, and `bind_typed_script_host.cpp`; typed capability coverage in `tests/script/script_runtime_tests.cpp`. |
...
| SCR-07 | Lua reads, sets, and unsets declared identity-scoped Properties; lookup follows override/own-assignment/configured-Trait/default/missing precedence and invalid mutations are atomic. | retained-complete | `RuntimeScriptApi`, `PropertyResolver`, `SessionState`; property host tests in `tests/script/script_runtime_tests.cpp` and Trait/override tests in `tests/core/session_state_tests.cpp`. |
| SCR-08 | Lua can query and set canonical Character Location (`Room`/`Unplaced`) and Interactable Location (`Room`/owner-qualified `Inventory`/`Unplaced`); invalid destinations and Inventory-containment cycles fail atomically. | retained-complete | `RuntimeScriptApi`, `RuntimeCommandGateway`, and `runtime::RuntimeSession`; typed capability tests in `tests/script/typed_runtime_session_tests.cpp` and live-state tests in `tests/core/session_state_tests.cpp`. |
| SCR-09 | Lua requests Room navigation, transient Scene/Dialogue execution, child Scene/Dialogue calls, tail replacement, return, and end through typed flow operations with validated targets and stable failure behavior. | retained-complete | `RuntimeScriptApi`, `RuntimeCommandGateway`, and `FlowExecutor`; flow capability tests in `tests/script/script_runtime_tests.cpp` and atomic target/call tests in `tests/core/flow_executor_tests.cpp`. |
| SCR-10 | Lua drives current Dialogue choices, Room exits, transient selection, and Interaction invocation through the same closed runtime-input semantics used by the player/editor; stale indices and malformed named bindings fail deterministically. | retained-complete | `RuntimeScriptApi` and `runtime::RuntimeSession`; indexed choice/navigation, selection, autosave, and malformed interaction tests in `tests/script/typed_runtime_session_tests.cpp`. |
| SCR-11 | Lua may activate an authored Map Connection, while Map open/mode/focus/pan/zoom remain Layout-occurrence state rather than gameplay/session state. | retained-complete | `bind_runtime_capabilities.cpp`, `RuntimeScriptApi`, and `RuntimeCommandGateway` expose only `noveltea.map.activate(map_id, connection_id)`. The gateway resolves the exact active-Room Exit and enqueues the normal Navigation Attempt without synchronously re-entering Lua guard evaluation. `RuntimeExecutor` publishes semantic Map views for Layouts; `tests/script/typed_map_execution_tests.cpp` and `tests/script/typed_runtime_session_tests.cpp` cover exact Exit selection, reciprocal projection, and Lua activation. |
...
| SCR-13 | Lua can pause/resume gameplay semantically and query that state without conflating editor visibility, audio pause, modal layout policy, or platform suspension. Pause is session-only and is not restored from a save. | retained-complete | `SessionState::gameplay_paused`, `runtime::RuntimeExecutor`, `runtime::RuntimeSession`, and `Game.pause/resume/paused` own semantic gameplay pause. The executor stops before the next instruction, gameplay-advancing inputs are gated while control/load operations remain available, typed views/debug snapshots publish the flag, and restore resets it. `tests/script/typed_runtime_session_tests.cpp`, `tests/core/save_state_tests.cpp`, and editor protocol tests cover idempotence, in-flow timing, blocked advancement, view persistence, and load reset. |
| SCR-14 | Lua separates scoped reconstructible Music/Ambience intent from transient playback. Transient Voice/Sound Effect playback uses typed operations with exact completion and checkpoint classification; UI Sound is explicitly disposable. Desired loops use stable Owner/instance identity and never expose backend handles. | retained-complete | `RuntimeScriptApi` and `RuntimeCommandGateway` expose `audio.play/play_and_wait/stop/stop_and_wait/play_ui` plus `set_loop/set_music/clear_loop/clear_purpose/state`. `SessionState` owns `DesiredAudioInstance`; `PresentationCoordinator` owns transient operation lifecycle; `RuntimeAudioAdapter` reconciles fresh looping voices and executes exact transient targets through `AudioSystem`. Save File V1 persists desired configuration only. Audio adapter/session/save tests prove layered Ambience, reserved BGM replacement, overlapping one-shots, exact/Purpose stop behavior, causal/disposable classification, pause policy, Purpose mixing/Voice ducking, owner cancellation, save/load/reset reconstruction, and no one-shot replay or backend progress in save bytes. |
| SCR-15 | Lua emits a notification through a typed runtime command; command failure remains retryable and does not disappear behind a no-op. | retained-complete | `RuntimeScriptApi::request_notification`, `RuntimeCommandGateway`, and `runtime::RuntimeSession`; capability tests in `tests/script/script_runtime_tests.cpp` and retained-failed-command coverage in `tests/script/typed_runtime_session_tests.cpp`. |
...
| SCR-17 | Manual save, load, and autosave use typed slot IDs and one strict byte contract; failed writes/decodes/restores leave the live session and API attachment unchanged. | retained-complete | `RuntimeScriptApi`, `runtime::RuntimeSession`, `RuntimeCheckpointService`, and `TypedSaveSlotStore`; save/load/autosave and failure-atomicity tests in `tests/script/typed_execution_kernel_tests.cpp`, `tests/script/runtime_checkpoint_service_tests.cpp`, and `tests/script/typed_runtime_session_tests.cpp`. |
| SCR-18 | Startup Lua is a separate non-yielding hook executed before the Room/Scene/Dialogue entrypoint; failure prevents a partially started runtime. | retained-complete | `runtime::RunningGame`, `runtime::RuntimeExecutor`, and the running-game loader; startup and entrypoint coverage remains in `tests/script/compiled_runtime_tests.cpp` and editor export separation in `editor/src/renderer/test/runtime-artifact-preparation.test.ts`. |
...
| ROOM-01 | Entering a Room validates the target, runs before-leave/before-enter/after-leave/after-enter hooks in indexed order, updates current/previous navigation context and visit count at the defined commit point, and fails without a half-transition. | retained-complete | `FlowExecutor` Room transition path and `runtime::RuntimeExecutor`; hook sequencing and failed transition tests in `tests/core/flow_executor_tests.cpp`, Room execution coverage in `tests/script/typed_room_execution_tests.cpp`. |
| ROOM-02 | Room exits and same-room/cross-room navigation use stable exit IDs, authoritative destination references, and one typed navigation pipeline for Lua, Map, player, and editor playback inputs. | retained-complete | `runtime::RuntimeSession`, `runtime::RuntimeExecutor`, and `FlowExecutor`; indexed navigation tests in `tests/script/typed_runtime_session_tests.cpp`, direct transition tests in `tests/core/flow_executor_tests.cpp`, and Map execution tests in `tests/script/typed_map_execution_tests.cpp`. |
| ROOM-03 | Room placements, overlays, Character/Interactable visibility and location, and transient selection produce a typed Room view; invalid live state is rejected rather than repaired silently. | retained-complete | `SessionState`, `RoomPresentationResolver`, and `runtime::RuntimeExecutor`; Room/live-state tests in `tests/core/session_state_tests.cpp` and `tests/script/typed_room_execution_tests.cpp`. |
| MAP-01 | A Map is presentation over authoritative Room topology; per-occurrence focus/open/mode/pan/zoom, visibility, connection availability, and activation are typed and deterministic. | retained-complete | `runtime::RuntimeExecutor` publishes pure `MapView` projections while each `nt-map-view` occurrence owns its local view state; optional persistence is only through a declared Layout State Slot. `tests/script/typed_map_execution_tests.cpp` and typed RmlUi Map snapshot tests in `tests/ui/rmlui_custom_components_tests.cpp` cover projection, exact Exit selection, and occurrence isolation. Public Lua exposure is separately tracked by SCR-11. |
...
| SCN-01 | Scene executes every V1 instruction variant, including conditions/effects, text, waits, calls, flow replacement, presentation, Layout, Map, audio, notification, and autosave requests, in stable order. | retained-complete | `runtime::RuntimeExecutor`; complete vocabulary and representative composition tests remain in `tests/script/typed_execution_kernel_tests.cpp`, with exhaustive compiler/wire cases in editor tests. |
| SCN-02 | Scene failures preserve the stable instruction cursor, stale resumes do not mutate state, and nested call/return restores the exact next position. | retained-complete | `runtime::RuntimeExecutor` and `FlowExecutor`; Scene failure/cursor tests remain in `tests/script/typed_execution_kernel_tests.cpp` and generic call ordering tests in `tests/core/flow_executor_tests.cpp`. |
| INT-01 | Interaction resolution validates complete named Verb-slot bindings, matches only through the shared structural Subject Selector vocabulary, evaluates narrower containment tiers first, falls through on false pure Guards, and uses explicit priority within a tier. Equal winning priority is an ambiguity fault; declaration order is not semantic. Final commands bind exact live subjects; `bindingOrder` is only the locale-neutral progressive-selection order. | retained-complete | `runtime::RuntimeExecutor` Interaction execution and the shared evaluator; selector/Guard/priority tests in `tests/script/typed_interaction_execution_tests.cpp`, condition coverage in `tests/core/shared_evaluator_tests.cpp`. |
| INT-02 | One winning compact behavior stages its typed mutation batch atomically and permits at most one terminal response, navigation target, Scene/Dialogue call, or Lua handoff. An empty `Unhandled` result falls back through the Verb default, optional Project behavior, then localized engine response; once mutations commit or a handoff begins, fallback is impossible and later failure retains committed state. | retained-complete | Interaction execution, `SessionState`, `FlowExecutor`, save/checkpoint codec; atomicity/fallback tests in `tests/script/typed_interaction_execution_tests.cpp` and Project-fallback round-trip coverage in `tests/core/save_state_tests.cpp`. |
...
## Data, Traits, persistence, and messages
...
| DATA-02 | Identity-scoped Property lookup resolves target runtime override, own authored assignment, compatible configured Trait value, declaration default, then typed missing. Trait requirements/conflicts are compile-time errors; unset removes only the target override. | retained-complete | `PropertyResolver`, `SessionState`; focused Trait/precedence/mutation tests in `tests/core/session_state_tests.cpp` and restore tests in `tests/core/save_state_tests.cpp`. |
...
| DATA-05 | The compiled project is the sole gameplay definition source; authoring compilation deterministically retains Trait declarations/attachments and sparse Property assignments, and preview/playback/package/CLI publication uses byte-equivalent canonical gameplay bytes. | retained-complete | Authoring compiler and `CompiledProject`; editor compiler, golden corpus, wire, and `compiled-artifact-publication.test.ts` suites plus native decoder tests. |
...
| PRE-01 | Runtime presentation coordination owns world/Layout layers, modal/input/pause policy, title/game/menu transitions, resize/aspect behavior, finite operation lifecycle, and host presentation lifetimes without becoming gameplay state. | retained-complete | `RuntimePublication`, `RuntimePresentationBridge`, `PresentationCoordinator`, `RuntimeLayoutManager`, `RuntimeSystemLayouts`, and world/RmlUi/audio realization own the shipped path. Coordinator, runtime-session, Layout, world-transition, shell, protocol, readback, sandbox, player, Web, editor, and package/export tests provide the evidence recorded by the completed presentation plan. |
...
| EDT-02 | Record-specific previews are editor tooling with pooled host lifetimes and editor-owned selections; they do not define gameplay Trait semantics or runtime state. | non-runtime-tooling | Editor preview manager/pools; Room/Layout/Character/Dialogue/Scene preview tests. |
...
| REJ-04 | Universal gameplay-definition reparenting/`extends`, editor-tree parent semantics, and flattened inherited descendant values. | rejected-obsolete | The fixed model uses explicit Trait attachments over ordinary Properties; structural/executable definition fields remain local. |
| REJ-05 | Script entrypoint entities, JavaScript/Duktape, autorun collection scanning, compatibility Lua tables, and duplicate `Game.*` implementations. | rejected-obsolete | Lua-only modules plus one explicitly selected Bootstrap Module and one `RuntimeScriptApi` are the final scripting model. |
...
- Script-visible random state is deterministic and saved; Map, Layout, pause, audio, and text-log
  operations use the single `RuntimeScriptApi`/`RuntimeCommandGateway`/`runtime::RuntimeSession`
  path.
