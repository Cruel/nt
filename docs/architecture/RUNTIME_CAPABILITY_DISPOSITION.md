...
| SCR-08 | Lua can query and move a unique Interactable among Room placement, inventory, and nowhere; invalid destinations fail atomically. | retained-complete | `RuntimeScriptApi`, `RuntimeCommandGateway`, and `runtime::RuntimeSession`; typed capability tests in `tests/script/script_runtime_tests.cpp` and live-state tests in `tests/core/session_state_tests.cpp`. |
