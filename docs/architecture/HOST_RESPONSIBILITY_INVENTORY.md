...
| H21 | Sandbox demo drawing, fixture audio, resize/readback runs, and automatic demo project | `EngineRunConfig`, `Engine::render()`, `App`, and sandbox assets/CMake mix demo branches with production host state. | `apps/sandbox` or dev/test-only harness; production Engine receives only explicit backend-neutral host configuration. | Sandbox CLI, CI/readback verifiers, editor preview smoke paths. | Harness lifetime wraps an Engine host. Fixture state must not be required by player initialization or production rendering. | 4C, 4F-4G |
...
| H23 | User screenshots, readback capture, and checkpoint thumbnail capture | `EngineRunConfig`, `Engine::render()`, `Renderer` screenshot APIs, pending checkpoint capture state, and App resize/readback fixture. | PreviewHost/dev harness owns user/test screenshots; an explicit renderer capture adapter serves GameHost checkpoint requests. | Sandbox CLI/readback tests, editor preview, checkpoint service/save UI. | Requests are frame-bound and renderer-dependent. Pending IDs/revisions must be invalidated before reload, RunningGame destruction, or renderer shutdown. | 4F |
