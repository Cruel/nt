...
| Frame clocks and pause | `Engine::update()` forwards raw frame `dt` to audio, tweens, and runtime; `RuntimeUI::begin_frame()` receives platform delta; `SessionState::gameplay_paused` rejects gameplay advancement | `engine.cpp`, `session_state.cpp`, `typed_runtime_session.cpp` | all live subsystems | session/Lua pause and tween tests; no clock-domain, scale, suspension-clamp, or paused-RmlUi test | Engine clock service | `replace`; Phase 3 |
...
| Engine/RmlUi/audio absolute clocks | engine float elapsed, SDL elapsed, backend time | Omitted; restarted with subsystem lifetime | no cross-domain restore tests | Backend-only; engine domains Phase 3, RmlUi integration Phase 5 |
