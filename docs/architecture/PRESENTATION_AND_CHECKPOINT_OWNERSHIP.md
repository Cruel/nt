...
| RmlUi input and ordering | One context receives SDL input once; `RuntimeLayoutManager` deterministically governs gameplay fallthrough and Layout-Lua gameplay admission, and routes Escape dismissal by mounted owner | `runtime_layout_manager.*`, `engine.cpp`, `ui_runtime_rmlui.cpp`, `rmlui_input_sdl3.*` | host events and typed Layout gameplay commands | mounted-policy input/dismissal tests exist; no cross-context delivery test | Typed mounted-Layout input router and RmlUi contexts | `reduce`; Phase 3 semantic routing complete, Phase 5 replaces global delivery with exact contexts |
