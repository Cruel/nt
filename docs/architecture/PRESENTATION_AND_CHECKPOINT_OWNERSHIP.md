...
Phase 10C update (2026-07-17, host ownership corrected in Phase 6C): `GameHost` is the shipped host
consumer of the final coherent `runtime::RuntimePublication`. It retains the latest publication and
runtime/presentation diagnostics, applies the publication one-way to RuntimeUI, routes the snapshot to
presentation backends, and gives preview, shell, debugger, recorder/playback, and external protocol
adapters direct publication access.
...
At the time of this baseline audit, `RuntimeLayoutManager` and `RuntimeTransitionManager` were
compiled into `engine`, but repository-wide symbol inspection found no production or test consumer.
The live engine directly loaded title and runtime RmlUi documents, and `Engine::render()` used a
constant zero transition opacity. That historical state conflicted with older documentation
describing both managers as tested functional orchestration.

Phase 8C replaced that baseline. `PresentationCoordinator` now owns finite operation identity,
replacement, barriers, and terminal acknowledgement; `WorldTransitionBackend` owns revision-bound
full-world and targeted realization. `RuntimeTransitionManager`, the old callback/string-channel
`TweenService`, and durable `LogicalTransitionState` were deleted. A redesigned handle-based
`animation::TweenService` privately uses Twink as a backend-local realization primitive;
`RuntimeLayoutManager` remains only as typed mounted-document realization beneath coordinator-owned
Layout operations.
...
| Public Lua runtime gateway | One `RuntimeScriptApi` targeted at `TypedRuntimeSession` | `runtime_script_api.hpp/.cpp`, `bind_runtime_capabilities.cpp`, `bind_noveltea.cpp` | authored Lua and RmlUi Layout events | `script_runtime_tests`, `typed_runtime_session_tests`, and `runtime_contracts_tests` cover scoped background/actor/prop/environment/audio commands, stable custom Layout mount/query/removal, owner and policy validation, and gameplay-versus-shell Layout-event authority | `RuntimeScriptApi` with narrow typed ports | `retain`; Phase 10B completes the scoped presentation/custom Layout surface and engine-selected Layout-event capability profiles without adding another gameplay gateway |
...
| Gameplay-owned Layout slots | `SessionState::m_layouts` retains coarse compiled `LayoutSlot` compatibility inside the typed model; stable custom instances use `DesiredMountedLayout` | `feature_state.hpp`, `session_state.*`, `runtime_command_gateway.*`, `runtime_script_api.*` | Scene views and the live presentation snapshot/Layout realization path | session, save, presentation, Layout-manager, and typed-session Lua tests cover replacement, policy, removal, save/restore, and realization | Runtime/session state with reconstructible mounted intent | `reduce`; stable owner-scoped custom mounts are complete in Phase 10B while reserved slots remain the narrow convenience surface |
| Shell/menu documents | Direct `RuntimeUI` document map; built-in title is live, pause document asset/API exists but no engine menu stack invokes it | `ui/rmlui/runtime_ui.cpp`, system title/pause RML; `Engine::load_compiled_project()` | title screen and authored preview documents | `title_layout_asset_tests` only checks title Lua calls; no pause/settings/load/save stack test | Shell/menu state using mounted Layout instances | `replace`; Phase 8 |
...
| RmlUi lifecycle and documents | `RuntimeUI::State` owns contexts keyed by typed plane/composition-group/clock/input policy; compatible contiguous documents share contexts and plane render adapters | `ui/rmlui/runtime_ui.hpp`, `ui/rmlui/runtime_ui.cpp`, `ui/rmlui/*` | engine runtime UI and editor authored previews | lifecycle clock/order/input tests, mounted-policy tests, headless migration/reload integration, GPU readbacks, Linux/Web smoke | RmlUi/Layout presentation context registry | `retain`; Phase 5 complete |
...
| ActiveText reveal/fade/page realization | Coordinator owns typed reveal/fade lifecycle and causal barriers; `RuntimeUI::State` owns clock-driven playback and direct-render realization only | `presentation_coordinator.*`, `runtime_presentation_bridge.*`, `active_text_playback.*`, `ui/rmlui/runtime_ui.cpp` | `RuntimeUI::begin_frame()`, direct renderer snapshot | playback/layout and bridge barrier tests | Presentation coordinator; RmlUi/direct renderer are realization backends | `retain`; Phase 5 complete |
| General tween primitive | Historical `TweenService` owned target pointers, string owner/channel, and public callbacks | `animation::TweenService` owns opaque handle-addressed scalar samples and privately adapts Twink | `WorldTransitionBackend`; future ActiveText, actor-effect, Layout-effect, and material realization backends | focused tween-service tests plus coordinator/world-transition lifecycle and render tests | Coordinator owns semantic operations; each realization backend owns its clock-local tween services and transient samples | `redesigned`; Phase 8C follow-up restores the capability without restoring parallel lifecycle ownership |
| Transition state emitted by Flow | Historical `SessionState::LogicalTransitionState` duplicated operation intent beside typed operations | removed in Phase 8C | none | snapshot/session/checkpoint tests now cover its absence | Committed desired targets plus `PresentationCoordinator` operation lifecycle | `deleted`; Phase 8C removed the pseudo-target from state, views, snapshots, restore, and checkpoint gating |
| Callback transition scaffolding | Historical `RuntimeTransitionManager` owned fade phase/opacity and a midpoint callback | removed in Phase 8C | none | coordinator/world backend lifecycle and render tests | `PresentationCoordinator` plus revision-bound world/targeted realization | `deleted`; Phase 8C |
...
| Composition order | Typed world source/target passes, transition composite, targeted world interpolation, RmlUi plane ranges, ActiveText, menus/modals/debug, and host bars are explicitly ordered | `engine.cpp`, `world_presentation.*`, `world_transition.*`, renderer/RmlUi adapters | sandbox, Web, packaged player | world-transition/render tests plus existing readbacks and smoke coverage | Typed planes coordinated across presentation/RmlUi/renderer/debug | `retain`; Phase 8B/8C complete |
| Frame clocks and pause | `core::RuntimeClock` sanitizes the sole platform delta ingestion; `derive_effective_gameplay_pause()` combines the authoritative explicit session source, visible mounted Layouts, platform suspension, and engine/runtime suspension before advancement. The same typed status gates gameplay runtime inputs. | `runtime_clock.*`, `gameplay_pause.*`, `engine.cpp`, `typed_runtime_session.cpp` | live runtime, UI/finite presentation, preview diagnostics, audio and shader/demo presentation | runtime-clock, gameplay-pause, typed-session, transition, and save-state tests cover divergence, source independence, admission, and save exclusion | Engine clock service | `retain`; Phase 3A/3C complete, Phase 5/8 route RmlUi and finite realization clocks |
| Backend reset/project reload | `Engine::load_compiled_project()` terminates coordinated transient work, resets low-level realization, and replaces compiled runtime; `RuntimeUI::reload_documents_and_styles()` recreates all recorded documents in their lifecycle contexts with visibility/order/focus/listeners restored before authoritative rebinding | `engine.cpp`, `ui/rmlui/runtime_ui.cpp`, `runtime_audio_adapter.cpp` | preview reload, project load | headless document migration/reload integration, editor freshness tests, sandbox/readback smoke; desired-audio device reconciliation remains Phase 7 | Presentation coordinator reconciles fresh backends from snapshot | `replace`; Phase 4/5 foundation, completed for remaining audio persistence in Phase 7 |
...
| Web sandbox and editor iframe | Calls exported preview functions and hosts typed external protocol; same engine internals | `apps/sandbox/sandbox_app.cpp`, `web/shell.html`, `web/widget.html`, sandbox CMake | Web build/smoke, editor protocol and preview hook/component tests | Phase 8 |
...
| Logical transition pseudo-target | Historical `m_transition` | Deleted in Phase 8C | snapshot/session/checkpoint tests cover absence; render tests cover live typed operations | No durable transition pseudo-target; committed desired target plus transient coordinator operation |
...
| ActiveText reveal/fade/prompt phase | `RuntimeUI::State` playback fields | Omitted; recreated from new bound view/document | ActiveText playback/layout and bridge tests | Causal until stable input; never serialize progress; Phase 5 |
| Finite-operation progress/resources | `WorldTransitionBackend` transient records and retained exact revisions | Omitted and terminated on reset/load/project replacement | world-transition lifecycle/render tests | Backend/coordinator transient only; Phase 8B/8C |
...
The numbered implementation-evidence sections below record ownership at each historical phase
boundary. They are not current owner documentation; the inventory and architecture sections above
describe the final implemented ownership after the later cutovers.
...
| `noveltea_render_tests` and readback executables | shader/material adapters, snapshot-driven world rendering, full-world transitions, targeted background/actor/Layout realization, and current RmlUi/ActiveText/composition pixels | later menu/system-layout and final desired-audio composition scenarios |
...
| Low-level mounted-layout tests | `RuntimeLayoutManager` policy/lifetime/input/dismissal coverage | Keep semantic finite-operation lifecycle in coordinator/world-transition tests rather than adding another animation manager |
