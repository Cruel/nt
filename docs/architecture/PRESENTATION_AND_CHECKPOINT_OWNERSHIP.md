...
| Mounted Layout policy | Isolated `RuntimeLayoutManager` owns canonical typed mounted instances and deterministic ordering; RmlUi remains the realization host | `runtime_layout_manager.hpp/.cpp`; `core::MountedLayoutInstance`, strong ID, owner, and complete policy; document IDs remain adapter metadata | no live shell consumer yet | `runtime_layout_manager_tests.cpp` covers typed defaults, failures, ownership, replacement, visibility, ordering, identity lifetime, unmount, and reset | RmlUi/Layout presentation with typed mount policy; shell state for menus | `reduce`; Phase 3B complete, Phase 3C/3D add derived pause and input routing, Phase 5 integrates lifecycle contexts, Phase 7 reconstructs gameplay mounts |
...
| ActiveText reveal/fade/page realization | `RuntimeUI::State` owns playback state, page index, float targets, time, and starts string-keyed `TweenService` channels | `active_text_playback.*`, `active_text_layout.*`, `ui_runtime_rmlui.cpp`, `tween_service.*` | `RuntimeUI::begin_frame()`, direct renderer snapshot | ActiveText and tween unit tests; no checkpoint-barrier or reset/recreation integration test | Presentation coordinator; RmlUi/direct renderer remain realization backends | `replace`; Phase 5 |
...
| Frame clocks and pause | `core::RuntimeClock` sanitizes the sole platform delta ingestion into integer gameplay and unscaled-presentation domains; `Engine` routes gameplay delta to runtime/tweens/RuntimeUI and unscaled delta/time to audio and shader/demo adapters. Effective multi-source pause and per-context RmlUi clocks remain deferred. | `runtime_clock.hpp/.cpp`, `engine.cpp`, `session_state.cpp`, `typed_runtime_session.cpp` | live runtime, UI/tweens, audio, shader/demo presentation | `runtime_clock_tests.cpp` covers sanitization, clamp, divergence, suspension, overflow, and deterministic accumulation; existing session/checkpoint tests cover gameplay time mutation | Engine clock service | `retain` Phase 3A clock foundation; Phase 3C adds effective pause and Phase 5 adds RmlUi context routing |
| Backend reset/project reload | `Engine::load_compiled_project()` resets tweens/audio adapter and replaces compiled runtime; `RuntimeUI::reload_documents_and_styles()` recreates documents in the same context | `engine.cpp`, `ui_runtime_rmlui.cpp`, `runtime_audio_adapter.cpp` | preview reload, project load | editor freshness/reload tests and sandbox smoke; no snapshot reconciliation or one-shot replay test | Presentation coordinator reconciles fresh backends from snapshot | `replace`; Phase 4 foundation, completed across Phases 5–7 |
...
| ActiveText realization is ad hoc | `RuntimeUI::State` stores playback/page/time/float targets and starts string-keyed TweenService channels. | Reveal/fade lifecycle is not correlated to a presentation operation or checkpoint barrier. | Phase 5. |
...
| RmlUi documents, focus, CSS animation, render state | `RuntimeUI::State` / `Rml::Context` | Omitted; documents may be reloaded from resource paths | UI/readback tests; no checkpoint recreation test | Backend-only; reconstruct from snapshot/mounts, Phase 5 |
...
| Projection of immutable definitions plus mutable runtime intent into an idempotent presentation snapshot | Presentation projector/coordinator | Phase 4. The projector is read-only and never owns gameplay or save state. |
