...
| Mounted Layout policy | Engine-owned `RuntimeLayoutManager` owns canonical typed mounted instances and deterministic ordering; RmlUi remains the realization host | `runtime_layout_manager.hpp/.cpp`; `engine.hpp/.cpp`; `core::MountedLayoutInstance`, strong ID, owner, and complete policy; document IDs remain adapter metadata | live engine pause derivation; final shell/gameplay mount cutover remains deferred | `runtime_layout_manager_tests.cpp` covers typed policy/lifetime; `gameplay_pause_tests.cpp` covers pause participation | RmlUi/Layout presentation with typed mount policy; shell state for menus | `reduce`; Phase 3B/3C complete, Phase 3D adds input routing, Phase 5 integrates lifecycle contexts, Phase 7 reconstructs gameplay mounts |
