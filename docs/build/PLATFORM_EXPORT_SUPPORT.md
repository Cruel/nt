...
| Web | wasm32; threaded canonical, single-threaded compatibility artifact | Deployment directory/ZIP | WebGL 2; `essl-300` (legacy variants remain template-declared) | Any host with an installed matching template | Supported browser floor provisional; threaded Web requires `SharedArrayBuffer` plus COOP/COEP cross-origin isolation |
