...
- `nt-active-text`: registered as a C++ RmlUi element. It is now a layout/input host rather than a glyph markup renderer. It binds room/dialogue/cutscene body state, drives deterministic reveal and alpha playback through the engine-owned `TweenService` when available, and preserves `RichTextDocument` state. After RmlUi resolves the element box, RuntimeUI shapes visible text through the engine text stack, `ActiveTextLayout` maps shaped glyph ranges back to rich-text metadata and object hit rectangles, local page/wait segments, playback alpha, and prompt metadata, and the engine bgfx text renderer draws the direct path after RmlUi. The element intentionally does not emit visible fallback glyph RML in the current Phase B path.
