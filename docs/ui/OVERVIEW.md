...
- `docs/ui/RMLUI_DATA_MODEL_CONTRACT.md` is the authoritative authored-RML contract for the shared
  `noveltea` data model, exact projected fields and callbacks, and the remaining ActiveText/Map
...
- `docs/ui/RMLUI_RUNTIME_UI.md` describes RuntimeUI publication/action ownership, asset lookup,
  authored event handling, visual assets, encoded namespace paths, DPI/layout, template resolution,
...
- The private RuntimeUI facade, RmlUi host, lifecycle contexts, document registry, data-model owner,
  action gateway, input, ActiveText presenter, playback driver, and bgfx adapter live under
  `engine/src/ui/rmlui/` and are owned by `noveltea_engine`.
