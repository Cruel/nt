...
This matrix is the Phase 1 inventory for project diagnostics that participate in authoring Save,
...
- Phase 1 preserves existing call-site blocking behavior. Boundary-specific filtering is enabled by
  later phases.
...
| `AUTHORING_<NORMALIZED_PROJECT_VALIDATION_CODE>` | compiler semantic stage | error or warning | Original authoring pointer | underlying authoring error blocks | blocks only when the underlying authoring boundary includes runtime-package | blocks when the underlying authoring boundary includes platform-export | Preserves the Phase 1 project diagnostic rule. No error fallback. |
