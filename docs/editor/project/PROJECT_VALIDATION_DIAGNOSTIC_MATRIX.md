...
| `authoring.project.validation.<canonical-path-rule>` | `authoring-validation.ts` project-wide checks | error or warning | `/entrypoint`, `/variables/**`, `/traits/**`, owner-local Property paths under Property-bearing records/instances, `/<collection>/**`, `/editor/recordMetadata/**` | error blocks | runtime-classified error blocks; `/editor/**` does not | runtime-classified error blocks | Trait declaration/attachment/requirement/conflict checks, owner-local Property ownership/value compatibility, metadata ownership, IDs, labels, and cross-record references. `/editor/**` is authoring-only. No content fallback. |
...
| `authoring.interactions.<canonical-path-rule>` | `authoring-interactions.ts`, resolver analysis, and interaction-program validation | error, warning, or info | `/interactions/**`, `/verbs/**/defaultProgram/**` | error blocks | error blocks | error blocks | Rules, named slot selectors, instructions, variable values, targets, definite impossible selector spaces, definite unreachable rules, and unconditional equal-tier/equal-priority conflicts. Runtime-dependent overlap, Trait facts, and Lua Guards remain non-blocking conditional diagnostics. No error fallback. |
...
| `runtime-package.entrypoint.required` | `prepareRuntimeArtifact` readiness composition | error | `/entrypoint` | no separate Save decision | blocks | blocks | Explicit project-derived runtime-package readiness blocker used consistently by Play and `.ntpkg`; no fallback. |
...
| `compiled_project.missing_field`, `compiled_project.type`, `compiled_project.unknown_field`, `compiled_project.unknown_value`, `compiled_project.unknown_variant`, `compiled_project.invalid_id`, `compiled_project.duplicate_id`, `compiled_project.invalid_number`, `compiled_project.unsupported_provisional_schema`, `compiled_project.unsupported_schema`, `compiled_project.unsupported_version`, `compiled_project.unresolved_reference`, `compiled_project.invalid_variable_declaration`, `compiled_project.invalid_trait_attachment`, `compiled_project.invalid_trait_property`, `compiled_project.duplicate_trait_attachment`, `compiled_project.conflicting_trait_configuration`, `compiled_project.missing_trait_requirement` | native compiled-project decoder/linker through bundled `noveltea` | error | Native compiled-project JSON pointer | no | blocks | blocks | Native validation of the compiled wire artifact, Trait declarations/attachments, and Property compatibility before publication. No fallback. |
| `compiled.duplicate_id`, `compiled.invalid_model`, `domain.invalid_id`, `domain.invalid_diagnostic_code`, `domain.invalid_json_pointer`, `domain.invalid_source_location`, `domain.invalid_property_definition`, `domain.invalid_property_assignment`, `domain.invalid_property_override` | native compiled-project model construction through bundled `noveltea` | error | Native compiled-project JSON pointer when available | no | blocks | blocks | Typed model and domain-invariant failures reached while decoding the compiled artifact. No fallback. |
...
| `invalid_variant`, `missing_shaderc`, `missing_bgfx_include`, `missing_source`, `source_read_failed`, `source_write_failed`, `compiler_failed` | native shader compiler through the `noveltea` CLI bridge | error | Compiler path, output path, source path, or `/shaders` | no direct Save decision | blocks | blocks | Current native shader compilation failures. Stable native code is preserved; no fallback. `missing_shaderc` is retained as a stable native diagnostic code even though the compiler is now embedded. |
| `cache_read_failed`, `cache_write_failed` | native shader compiler through the `noveltea` CLI bridge | warning | Shader cache manifest path | no | no | no | Cache is ignored or rebuilt; warning is preserved with its stable native code. |
| `invalid_shader_id`, `invalid_material_id`, `invalid_json`, `invalid_schema`, `missing_required_field`, `invalid_field_type`, `unknown_shader_role`, `deferred_shader_role`, `invalid_shader_source_ref`, `invalid_compiled_binary_ref`, `invalid_uniform_declaration`, `invalid_uniform_value`, `invalid_sampler_declaration`, `invalid_texture_slot_name`, `invalid_texture_source`, `unsupported_sampler`, `unknown_input_binding`, `unsupported_blend_policy`, `unknown_shader_ref`, `undeclared_uniform`, `undeclared_sampler`, `incompatible_shader_role` | native shader/material project parser through the `noveltea` CLI bridge | error or warning | Native shader/material JSON pointer | error blocks through authoring/runtime readiness | error blocks | error blocks | All current native material diagnostic codes. Warnings permit parsing to continue; errors have no fallback. |
...
| Native package/asset publication code, when supplied | bundled `noveltea` through `editor-tool-service.ts` | error, warning, or info | Native package/asset path | no | error blocks | error blocks | Stable native code is preserved. No error fallback. |
...
| `runtime-artifact.evidence.rejected` | Prepared Runtime Artifact verification | error | `/artifact` or the rejected field such as `/artifact/sourceFingerprint` | Selected target. Reject malformed, unsupported, stale, profile-mismatched, or internally inconsistent evidence and prepare again from the current Project. |
| `runtime-artifact.shader-output-stale-or-invalid` | Prepared Runtime Artifact preparation | error | `/shaders/<shader-id>` | Selected export intent. Re-run the shader compiler adapter from the current Project; stale or invalid integrity evidence cannot enter a Prepared Runtime Artifact. |
| `runtime-artifact.shader-output-missing` | Prepared Runtime Artifact preparation | error | `/shaders/<shader-id>/data/stages/<index>/compiled/<variant>` | Selected export intent. The compiler must return exactly one output for every requested shader/stage/variant key. |
| `runtime-artifact.shader-output-duplicate` | Prepared Runtime Artifact preparation | error | `/shaders/<shader-id>/data/stages/<index>/compiled/<variant>` | Selected export intent. Duplicate compiler outputs are ambiguous evidence and block preparation. |
| `shader.compile.response-invalid` | renderer or native/CLI shader adapter | error | `/shaders` | Selected export intent. Reject malformed native compiler responses at the adapter boundary and prepare no artifact. |
| `runtime-artifact.preparation.cancelled` | Prepared Runtime Artifact preparation | warning | `/` | Cancellation publishes no Prepared Runtime Artifact or package. |
