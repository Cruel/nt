...
| `authoring.interactions.<canonical-path-rule>` | `authoring-interactions.ts` and interaction-program validation | error or warning | `/interactions/**`, `/verbs/**/defaultProgram/**` | error blocks | error blocks | error blocks | Rules, named slot selectors, instructions, variable values, targets. No error fallback. |
