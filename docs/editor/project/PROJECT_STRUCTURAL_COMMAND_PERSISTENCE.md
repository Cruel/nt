...
This document is the permanent classification contract for editor mutations that may bypass normal
...
| --- | --- | --- | --- | --- | --- | --- |
| New Entity Wizard record creation | `entity.createRecord`; `workflow:new-entity` | Project content | `/<collection>/<entityId>` plus every actual content patch emitted by the handler; emitted editor-state patches are saved in the accompanying metadata snapshot | None | None | `convert-to-manual-save` |
