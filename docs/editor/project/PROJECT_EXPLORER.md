...
Shader and Lua files are source resources whose canonical workbench identity is their normalized Project-relative path. Normal opening reuses the canonical source tab; **Open in New Tab** creates a separate tab instance for the same resource. Editor-initiated source moves repair owned Material shader paths, Script Module paths, Layout script dependencies, recognized Layout RML `<script src>` references, and project-local shader includes in the same workspace transaction. Asset files remain distinct from Asset records: the collective Assets destination stays in Project mode, while an individual Asset source can be located in Files. Built-in or otherwise non-Project source is not represented as a Files node merely because a source editor can display it.

Quick Open includes source files as results distinct from semantic records and matches both filename and displayed path. Files search uses the shared Project search machinery and indexes readable source contents, including unreferenced Lua helpers and shader include files. External source-content changes refresh both that source index and source-derived dependency analysis; a clean open source buffer reloads from disk, while a dirty buffer preserves the local edit and exposes an explicit disk-versus-local conflict. Project search continues to operate on semantic records and metadata.

Source tabs own editable buffers with an exact persisted-source revision. Saving uses compare-and-swap against that revision. A clean buffer adopts external edits automatically; if the same source changes or is deleted externally while its buffer is dirty, the local text is retained and the tab presents an explicit **Use Disk** / **Keep Mine** conflict instead of overwriting either version.

The `scripts/` and `shaders/` roots support type-aware file creation plus freeform folders. Rename, move, and delete use Project source operations rather than raw renderer filesystem access. Moves within a source root rewrite Script Module project-file paths, Material project shader paths, and recognized project-local shader includes in the same workspace transaction as the source-file writes/deletes. Material **Customize Shader** / **Make Material-specific copy** uses this same transaction authority: it writes the copied stage beneath `shaders/materials/<material-id>/` and updates the Material source reference atomically. Built-in `engine:/` source remains read-only and outside Files until copied. Folder moves are one descendant path-remap transaction. Referenced deletion is refused with the concrete usages rather than intentionally leaving broken references. Layout-projected source and Asset files remain editable/revealable where appropriate but are not generic movable Files resources.

External filesystem watching treats file identity as path identity. Additions, removals, and content changes refresh Files and source search. A delete followed by a create at another path is not inferred to be a rename; typed references to a missing source remain broken and are diagnosed until the author repairs them.
