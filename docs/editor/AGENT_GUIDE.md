...
The bottom-panel host exists independently of whether a Project is open. Each bottom-panel definition declares an availability predicate over explicit workbench context; do not hide panels through Terminal-specific or component-local Project checks. Registry order is the deterministic fallback order when the persisted/current panel is unavailable. The fallback is presentation-only so temporarily missing context does not overwrite a Project's persisted bottom-panel visibility, size, or active-panel preference. `Activity` (the existing `output` panel ID) and `Terminal` are global, while `Problems` requires an open Project. Result panels are demand-driven: `Package Export` is available only while export work is running or a retained export result exists, `Test Playback` only while a retained playback report exists, and `References` only while a Find Usages result exists. `Shader Compile` and `Command History` are developer-only Project tooling. `Runtime Events` and `Asset Performance` require an open Play preview tab, while `Preview Diagnostics` remains available after the Play tab closes for as long as retained preview diagnostics exist. Runtime Events is a semantic activity feed, not a preview-protocol inspector: continuous snapshots, telemetry, request acknowledgements, hover/focus traffic, and other transport plumbing do not belong there. Extend the shared context/predicates rather than adding component-local visibility rules when future panels depend on additional capabilities or workbench state.
