...
Release builds embed the checked-in hand-authored kit source plus the curator provenance as a private scriptc island package. `noveltea agent sync` combines the exact source texts with JSON Schemas generated from the shared Zod schemas, then writes and validates the deterministic manifest/hashes/provenance. The Markdown source is copied byte-for-byte; sync does not add or strip headers or maintain an alternate agent-facing representation. This work is command-local: ordinary CLI operations do not generate the agent-kit schemas. See `SCRIPTC_COMPATIBILITY.md`.
...
CLI differential certification covers sync generation, provenance inclusion, byte-exact hand-authored Markdown, bootstrap inspection/repair, version/hash verification, repeated idempotent sync, rollback on injected swap failure, project creation, and Node/scriptc byte equivalence. Distribution verification additionally certifies standalone relocation and that production packages contain no prohibited first-party source maps or external runtime dependencies.
