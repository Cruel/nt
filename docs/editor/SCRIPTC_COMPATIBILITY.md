...
The #289 scoped-Project-preparation check on 2026-09-18 packed `asset audit` behind an 8.8 kB preparation chunk plus a 1.3 kB command chunk while the full Project Workspace remained a separate roughly 607 kB chunk. Seven Node-reference Feature Lab runs measured 0.33–0.37 s for `asset audit` versus 0.47–0.50 s for `platform profiles`, which still used full Workspace preparation at that point. These measurements are directional local evidence rather than CI wall-clock gates.
