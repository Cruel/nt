...
and are not stored in gameplay checkpoints. Their separate strict Runtime User Settings Format V1
persistence document contains independent UI and text scales. Loading clamps both values against the
accessibility policy on the validated loaded compiled project, while disabled settings resolve to
`1.0`. A project reload retains the user selections only through that new
