...
- ActiveText Phase B hardening remains active: exact segment timing where it affects playback, cursor/continue prompt presentation, alpha/show/hide tweening, high-quality halo/blur glow, a font-family loader that requires at least a regular/base face, legacy-compatible `sys` system font fallback currently backed by Liberation Sans regular, project/user default font resolution through `fontDefault`, synthetic fallback for missing bold/italic/bold-italic faces, FreeType-backed synthetic style rasterization, advanced mixed-font fallback, bgfx/custom-geometry map rendering, optional map transition animation, and per-object/per-room materials. CPU-side Fade/FadeAcross/Pop/Nod/Shake/Tremble/Glow/Test visuals and simple renderer glow are implemented; text outline/border metadata is intentionally sidelined until an authored fixture requires it.
