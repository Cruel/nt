...
- RmlUi bgfx optimization Phase 7 partial: perf logging now classifies pass reasons and reports pass-builder operations vs reused bgfx views. Opacity/color-matrix-only filter chains fold into a composite-filter shader instead of allocating bounded postprocess targets and running copy/ping-pong passes. Same-target non-clear draws reuse bgfx views more broadly, and mask-image-only filters bypass one preliminary source copy. Readback-gallery steady state now reports `passes=89`, `views=58`, `color_filter_folds=9`, `pass_filter_copy=2`, `bounded_postprocess_passes=6`, `post_px=21776`, and the established bounded-layer invariants.
...
- RmlUi Phase 7 pass-count and render-target-switch reduction is underway. The next speed targets are redundant layer-clear reduction and any remaining safe copy/view reuse after current Phase 7 work reduced the readback gallery from `passes=108` to `passes=89`.
...
- `cmake --build --preset web-debug` and `node scripts/web-smoke.mjs` pass after the current Phase 7 partial work; web smoke reports zero full-frame child layers, `max_child_layer=114x96`, and `passes=89`.
- `cmake --build --preset linux-debug`, `cmake --build --preset linux-debug --target format-check`, and the full Linux `ctest --test-dir build/linux-debug --output-on-failure` pass after the current Phase 7 partial work.
- Latest Linux and Web readback-gallery perf after the current Phase 7 partial work has the strict bounded structural shape and lower pass/postprocess counts:
...
Resume the RmlUi bgfx optimization plan at Phase 7. Implement pass-count and render-target-switch reduction while preserving all RmlUi feature semantics. Do not start direct-base tuning or physical repository extraction until pass/FBO overhead is measured and tightened.
