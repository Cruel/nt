...
- Shader-backed ActiveText rendering, bgfx/custom-geometry map rendering, optional map transition animation, per-object/per-room materials, and RmlUi `shader(<string>)` decorator materials remain active. Project-schema shader/material records, shader metadata selection, and runtime compiled bgfx binary loading are implemented. Editor/import/export shaderc compilation, material uniform/texture binding into engine 2D draw calls, and the RmlUi material bridge remain deferred to later phases of [`docs/rendering/NOVELTEA_SHADER_MATERIAL_PLAN.md`](../rendering/NOVELTEA_SHADER_MATERIAL_PLAN.md).
...
Continue the current rendering action plan at Action 4 / shader-material Phase 3: add the host/editor/import/export shader compilation service around `shaderc` so project-authored shader source can produce the compiled bgfx binaries consumed by runtime shader metadata resolution and `BgfxShaderProgramCache`. Do not wire RmlUi `shader(<string>)`, engine 2D material binding, or runtime shader source compilation in that slice.
