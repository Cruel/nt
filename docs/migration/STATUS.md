...
- Shader-backed ActiveText rendering, bgfx/custom-geometry map rendering, optional map transition animation, and per-object/per-room materials remain active. Project-schema shader/material records, shader metadata selection, runtime compiled bgfx binary loading, host/editor/import shaderc compilation, engine 2D material binding, and the RmlUi `shader(<string>)` decorator material bridge are implemented. ActiveText custom shader rendering remains deferred to later phases of [`docs/rendering/NOVELTEA_SHADER_MATERIAL_PLAN.md`](../rendering/NOVELTEA_SHADER_MATERIAL_PLAN.md).
