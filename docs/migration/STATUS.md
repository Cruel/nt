...
Last updated: 2026-06-19 (Phase 8 runtime UI adapter slice started).
...
- `RuntimeUIViewAdapter` is the first Phase 8 bridge from backend-neutral controller commands to UI-facing state. It consumes room entry/description/navigation, dialogue text/options, cutscene text/page-break, notifications, text log, and action result commands into `RuntimeUIViewState`. The state is intentionally free of SDL3, bgfx, RmlUi, ImGui, and SFML so controller-to-UI behavior can be tested in `noveltea_core`.
- `RuntimeSessionHost` owns `GameSession`, `RuntimeController`, and `RuntimeUIViewAdapter` together. It loads projects/saves, ticks the controller, consumes controller commands into UI state, and exposes narrow action methods for navigation, dialogue option selection, active dialogue/cutscene continuation, and room actions. It is still backend-neutral.
- `RuntimeUI` now has `apply_controller_commands()`, `bind_runtime_controller()`, and a RmlUi updater for the `"runtime_game"` document. It escapes controller text before writing DOM content, updates stable element ids for mode, title, body, options, navigation, notification, prompt, and text log, and delegates RmlUi click targets with `nt-option`, `nt-nav`, and `nt-continue` attributes back to the bound `RuntimeController`. Sandbox assets `apps/sandbox/assets/rmlui/runtime_game.rml` and `.rcss` provide the first runtime-game RmlUi view.
...
- Phase 7 is complete. Phase 8 has a working minimal controller-command-to-RmlUi path, a backend-neutral `RuntimeSessionHost`, and an engine/sandbox runtime-project smoke path. Runtime-controller coverage now includes queue draining, entrypoint startup, room navigation, room enter/leave hook command order, active room/dialogue/cutscene save-state restore, transient script/custom-script mode, dialogue traversal/options/log commands, cutscene timeline expansion, next-entity transitions, room action resolution, notification/text-log event forwarding, UI-neutral command emission, and parsed rich-text payloads for dialogue/cutscene text commands. The next core-engine slice should add room action and inventory/object interaction controls, then connect Phase 9 package/asset loading so old project packages can drive this path.
