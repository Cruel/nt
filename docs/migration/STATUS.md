...
- Shader-backed ActiveText rendering, bgfx/custom-geometry map rendering, optional map transition animation, per-object/per-room materials, and RmlUi `shader(<string>)` decorator materials remain active. Project-schema shader/material records, shader metadata selection, runtime compiled bgfx binary loading, and host/editor/import shaderc compilation are implemented. Material uniform/texture binding into engine 2D draw calls and the RmlUi material bridge remain deferred to later phases of [`docs/rendering/NOVELTEA_SHADER_MATERIAL_PLAN.md`](../rendering/NOVELTEA_SHADER_MATERIAL_PLAN.md).
...
Continue the current rendering action plan at Action 5 / shader-material Phase 4: bind optional NovelTea material ids into the engine 2D draw path using the existing material schema, runtime resolver/cache, and host-compiled shader binaries. Keep the default colored/textured quad path unchanged when no material id is present. Do not wire RmlUi `shader(<string>)` or runtime shader source compilation in that slice.
