...
Last updated: 2026-06-19 (Phase 9 legacy package asset mount slice started).
...
- Phase 9 asset/package integration has started. `AssetManager::mount_legacy_package()` maps a read-only `legacy::ProjectPackage` into project logical assets for `game`, cover `image`, `fonts/*`, and `textures/*`. `Engine::load_runtime_project()` now reads runtime project bytes once, accepts normalized JSON as before, and falls back to `legacy::ProjectPackageReader` for old ZIP packages; successful package imports mount package assets before loading the imported `ProjectDocument` into `RuntimeSessionHost`.
...
Phase 9 package loading has its first runtime slice: `--runtime-project` can now fall back to legacy ZIP package import and mount package fonts/textures/cover assets. The next migration step should add deterministic runtime package fixtures or a generated package smoke asset so the sandbox can exercise the package path directly, then extend project-relative script/text asset resolution as the Lua script integration boundary is wired.
