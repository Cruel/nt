...
- RmlUi Phase 7 pass-count and render-target-switch reduction is underway. The next speed targets are redundant clear/copy reduction and safe view/pass reuse after the color-only filter fast path reduced the readback gallery from `passes=108` to `passes=90`.
...
- `cmake --build --preset web-debug` and `node scripts/web-smoke.mjs` pass after the Phase 7 color-only filter fast path; web smoke reports zero full-frame child layers, `max_child_layer=114x96`, and `passes=90`.
- `cmake --build --preset linux-debug`, `cmake --build --preset linux-debug --target format-check`, and `./build/linux-debug/tests/noveltea_ui_tests "*RmlUi*"` pass after the Phase 7 color-only filter fast path.
- Latest Linux and Web readback-gallery perf after the Phase 7 color-only filter fast path has the strict bounded structural shape and lower pass/postprocess counts:
