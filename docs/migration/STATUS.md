...
- Shader-backed ActiveText rendering, bgfx/custom-geometry map rendering, optional map transition animation, per-object/per-room materials, and RmlUi `shader(<string>)` decorator materials remain active. The `.ntmat` material asset model/parser is implemented; shader manifest selection, runtime compiled bgfx binary loading, editor/import/export shaderc compilation, and the RmlUi material bridge remain deferred to later phases of [`docs/rendering/NOVELTEA_SHADER_MATERIAL_PLAN.md`](../rendering/NOVELTEA_SHADER_MATERIAL_PLAN.md).
