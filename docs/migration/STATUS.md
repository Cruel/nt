...
Last updated: 2026-06-19 (Phase 11 compatibility completion complete).
...
- `legacy::ProjectPackageReader` uses private `miniz` ZIP reading to extract `game`, `image`, `fonts/*`, `textures/*`, and safe project-relative auxiliary assets such as `scripts/*`, `text(s)/*`, `shaders/*`, `audio/*`, `sounds/*`, `music/*`, `data/*`, and `resources/*`, then feeds `game` into `ProjectImporter`. It ignores unrelated or unsafe entries and exposes no ZIP library types publicly.
...
- `RuntimeController` is a backend-neutral gameplay sequencer that owns queue draining, room-entry/exit transitions, visit-count tracking, navigation-path resolution, action resolution, transient script/custom-script mode, active mode save/restore, and UI-neutral `ControllerCommand` emission. It accepts a `GameSession&`, calls `session.tick()` each frame, processes the startup entrypoint (Room entrypoint enters room mode directly; other types are queued), drains one entity per tick when not in Room mode (Room mode blocks), provides `navigate_path(direction)` to resolve a room path's target entity and exit the current room, and provides `process_action(verb_id, object_ids)` for room actions. Action processing validates object availability against the current room plus starting inventory, matches actions with old position-dependent or order-insensitive object semantics, emits project before/after action scripts, emits parent action scripts before child scripts, and falls back to the verb default script when no action matches. Room enter/leave hook commands follow the old project-before, room-before, project-after, room-after order. Runtime notification and text-log events are forwarded into controller commands. Dialogue and cutscene text commands now include parsed `rich_text` JSON payloads. Entities and hooks requiring script execution still emit `ScriptDeferred` commands. It deliberately avoids concrete script execution and renderer ownership.
- `RuntimeUIViewAdapter` is the first Phase 8 bridge from backend-neutral controller commands to UI-facing state. It consumes room entry/description/navigation, room objects, inventory objects, verb action controls, dialogue text/options, cutscene text/page-break, notifications, text log, and action result commands into `RuntimeUIViewState`. The state is intentionally free of SDL3, bgfx, RmlUi, ImGui, and SFML so controller-to-UI behavior can be tested in `noveltea_core`.
- `RuntimeSessionHost` owns `GameSession`, `RuntimeController`, and `RuntimeUIViewAdapter` together. It loads projects/saves, ticks the controller, consumes controller commands into UI state, derives current room/inventory object controls plus verb action controls from the loaded `ProjectModel`, and exposes narrow action methods for navigation, dialogue option selection, active dialogue/cutscene continuation, and room actions. It is still backend-neutral.
- `RuntimeUI` now has `apply_controller_commands()`, `bind_runtime_controller()`, and a RmlUi updater for the `"runtime_game"` document. It escapes controller text before writing DOM content, updates stable element ids for mode, title, body, options, navigation, room/inventory objects, action controls, notification, prompt, and text log, and delegates RmlUi click targets with `nt-option`, `nt-nav`, `nt-continue`, `nt-object`, and `nt-action` attributes back to the bound `RuntimeController`. Sandbox assets `apps/sandbox/assets/rmlui/runtime_game.rml` and `.rcss` provide the first runtime-game RmlUi view.
- `Engine`/`App` now expose `--runtime-project <asset-path>`. The engine reads normalized project JSON through `AssetManager`, loads it into `RuntimeSessionHost`, binds the `RuntimeUI` click bridge to the host controller, ticks the host during `Engine::update()`, and applies emitted controller commands to RmlUi each frame. `apps/sandbox/assets/projects/runtime_phase8.json` is the current smoke project for this path.
- Phase 9 asset/package integration is complete for the current read-only compatibility target. `AssetManager::mount_legacy_package()` maps a read-only `legacy::ProjectPackage` into project logical assets for `game`, cover `image`, `fonts/*`, `textures/*`, and safe auxiliary package entries. `Engine::load_runtime_project()` reads runtime project bytes once, accepts normalized JSON as before, and falls back to `legacy::ProjectPackageReader` for old ZIP packages; successful package imports mount package assets before loading the imported `ProjectDocument` into `RuntimeSessionHost`. Runtime asset staging now generates `projects/runtime_phase9_package.ntpkg` from text fixtures, and CTest includes a sandbox smoke for loading it through `--runtime-project`.
- Phase 10 editor preview/tooling APIs are complete for the first backend-neutral editor boundary. `noveltea::core::editor::ProjectTooling` provides normalized project JSON load, legacy game JSON import, validation diagnostics, JSON save serialization, and raw legacy entity record set/erase helpers without depending on the old Qt editor model. `RuntimePreviewSession` wraps `RuntimeSessionHost` with start, stop, reset, entrypoint override, step/tick, state inspection, navigation/dialogue/continue/action injection, and captured controller-command retrieval. These APIs live in `noveltea_core` and remain free of SDL3, bgfx, RmlUi, ImGui, Electron, and Qt.
- Phase 11 compatibility completion is complete for the current reduced-fixture target. `tests/core/compatibility_completion_tests.cpp` now provides a synthetic old-style project fixture and golden coverage for lossless legacy project import/editor save, save JSON parse/dump preservation, read-only legacy package import, cutscene/room/action/navigation/dialogue/timer/text-log/save-entrypoint/inventory runtime behavior, and command capture. `docs/migration/COMPATIBILITY.md` records supported compatibility and known limits, including read-only package support, deferred JavaScript script execution, incomplete autosave mutation policy, incomplete persisted object-location application, and the need for real old-project fixtures before broad corpus claims.
...
- Phase 8 is complete for the current runtime adapter boundary: controller commands now drive RmlUi room, navigation, object/inventory interaction, action, dialogue, cutscene, notification, and text-log UI without introducing old GUI widgets. Runtime-controller coverage includes queue draining, entrypoint startup, room navigation, room enter/leave hook command order, active room/dialogue/cutscene save-state restore, transient script/custom-script mode, dialogue traversal/options/log commands, cutscene timeline expansion, next-entity transitions, room action resolution, notification/text-log event forwarding, UI-neutral command emission, parsed rich-text payloads for dialogue/cutscene text commands, and host-level room object/action view derivation.

Verification:

- `cmake --build --preset linux-debug`: passed after Phase 7 rich-text changes.
- `cmake --build --preset linux-debug --target noveltea_core_tests`: passed after the Phase 8 runtime UI adapter slice.
- `./build/linux-debug/tests/noveltea_core_tests "RuntimeUIViewAdapter*"`: passed 23 assertions in 4 test cases.
- `./build/linux-debug/tests/noveltea_core_tests "RuntimeSessionHost*"`: passed 30 assertions in 4 test cases.
- `cmake --build --preset linux-debug`: passed after the Phase 8 runtime UI adapter slice.
- `ctest --test-dir build/linux-debug --output-on-failure`: passed 187/187 after the Phase 8 runtime-session host and RmlUi click-delegation slice.
- `./build/linux-debug/apps/sandbox/noveltea-sandbox --demo none --rmlui-document project:/rmlui/runtime_game.rml --frames 3 --no-imgui`: passed after the Phase 8 runtime UI adapter slice.
- `./build/linux-debug/apps/sandbox/noveltea-sandbox --demo none --runtime-project project:/projects/runtime_phase8.json --frames 5 --no-imgui`: passed after the Phase 8 engine/sandbox runtime-project integration slice.
- `cmake --preset web-debug`: passed after the Phase 8 runtime UI adapter slice.
- `cmake --build --preset web-debug`: passed after the Phase 8 runtime UI adapter slice with the existing Emscripten SDL3 experimental warning.
- `cd android && ./gradlew --no-daemon :app:assembleDebug`: passed after the Phase 8 engine/sandbox runtime-project integration slice with the existing compileSdk/AGP, SDK XML, NDK CMake deprecation, and HarfBuzz CMake warnings.
- `cmake --build --preset linux-debug --target noveltea_core_tests`: passed after the Phase 8 room-object/action interaction slice.
- `./build/linux-debug/tests/noveltea_core_tests`: passed 854 assertions in 111 test cases after the Phase 8 room-object/action interaction slice.
- `cmake --build --preset linux-debug`: passed after the Phase 8 room-object/action interaction slice.
- `ctest --test-dir build/linux-debug --output-on-failure`: passed 189/189 after the Phase 8 room-object/action interaction slice.
- `./build/linux-debug/apps/sandbox/noveltea-sandbox --demo none --runtime-project project:/projects/runtime_phase8.json --frames 5 --no-imgui`: passed after the Phase 8 room-object/action interaction slice with existing local RenderDoc/DRI3 warnings.
- `cmake --preset web-debug`: passed after the Phase 8 room-object/action interaction slice.
- `cmake --build --preset web-debug`: passed after the Phase 8 room-object/action interaction slice with the existing Emscripten SDL3 experimental warning.
- `cd android && ./gradlew --no-daemon :app:assembleDebug`: passed after the Phase 8 room-object/action interaction slice with the existing compileSdk/AGP and SDK XML warnings.
- `./build/linux-debug/tests/noveltea_asset_tests`: passed 84 assertions in 13 test cases after the Phase 9 legacy package asset mount slice.
- `./build/linux-debug/apps/sandbox/noveltea-sandbox --demo none --runtime-project project:/projects/runtime_phase8.json --frames 5 --no-imgui`: passed after the Phase 9 loader fallback slice with existing local RenderDoc/DRI3 warnings.
- `cmake --build --preset linux-debug`: passed after the Phase 9 legacy package asset mount slice.
- `ctest --test-dir build/linux-debug --output-on-failure`: passed 190/190 after the Phase 9 legacy package asset mount slice.
- `cmake --preset web-debug && cmake --build --preset web-debug`: passed after the Phase 9 legacy package asset mount slice with existing Emscripten SDL3 and bgfx variadic macro warnings.
- `cd android && ./gradlew --no-daemon :app:assembleDebug`: passed after adding an Android-only sandbox link option for the miniz symbol collision between the legacy package reader and bgfx/bimg image decoding; existing compileSdk/AGP, SDK XML, NDK CMake deprecation, HarfBuzz CMake, and bgfx macro warnings remain.
- `./build/linux-debug/tests/noveltea_core_tests "legacy ProjectPackageReader*"`: passed 50 assertions in 3 test cases after safe auxiliary package asset extraction was added.
- `./build/linux-debug/tests/noveltea_asset_tests`: passed 90 assertions in 13 test cases after legacy package auxiliary assets were mounted through `AssetManager`.
- `./build/linux-debug/apps/sandbox/noveltea-sandbox --demo none --runtime-project project:/projects/runtime_phase9_package.ntpkg --frames 5 --no-imgui`: passed after generated legacy package staging was added, with existing local RenderDoc/DRI3 warnings.
- `ctest --test-dir build/linux-debug --output-on-failure`: passed 191/191 after adding the generated legacy package CTest smoke.
- `cmake --preset web-debug && cmake --build --preset web-debug`: passed after Phase 9 completion with existing Emscripten SDL3 experimental and bgfx variadic macro warnings.
- `cd android && ./gradlew --no-daemon :app:assembleDebug`: passed after Phase 9 completion with existing compileSdk/AGP, SDK XML, and bgfx macro warnings.
- `cmake --build --preset linux-debug --target noveltea_core_tests`: passed after Phase 10 editor API additions.
- `./build/linux-debug/tests/noveltea_core_tests`: passed 911 assertions in 115 test cases after Phase 10 editor API additions.
- `cmake --build --preset linux-debug`: passed after Phase 10 editor API additions.
- `ctest --test-dir build/linux-debug --output-on-failure`: passed 195/195 after Phase 10 editor API additions. An earlier concurrent run while the sandbox executable was relinking produced transient `permission denied` starts for the two sandbox tests; rerunning after the relink completed passed.
- `cmake --preset web-debug && cmake --build --preset web-debug`: passed after Phase 10 editor API additions with the existing Emscripten SDL3 experimental warning.
- `cmake --build --preset linux-debug --target noveltea_core_tests`: passed after Phase 11 compatibility completion tests.
- `./build/linux-debug/tests/noveltea_core_tests "Compatibility*"`: passed 56 assertions in 4 test cases after Phase 11 compatibility completion tests.
- `./build/linux-debug/tests/noveltea_core_tests`: passed 967 assertions in 119 test cases after Phase 11 compatibility completion tests.
- `cmake --build --preset linux-debug`: passed after Phase 11 compatibility completion tests.
- `ctest --test-dir build/linux-debug --output-on-failure`: passed 199/199 after Phase 11 compatibility completion tests.
- `cmake --preset web-debug && cmake --build --preset web-debug`: passed after Phase 11 compatibility completion tests.
...
All 11 migration-plan phases are complete for the current repository target. Recommended follow-up work is post-plan hardening: add real old-project compatibility fixtures when redistributable or private fixture handling is available, wire `RuntimeController` `ScriptDeferred` commands to the Lua compatibility runtime, implement runtime save-slot/autosave mutation policy, apply persisted object-location save data to runtime object placement, and finish rich-text visual rendering parity.
