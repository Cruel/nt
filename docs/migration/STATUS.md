...
- Phase 11 Runtime Renderer and Asset Presentation v1: runtime view state exposes cover/background/room/object image slots, resolves visual metadata from room/object properties into logical project asset paths, validates missing visual assets in the RmlUi layer, and renders visuals through the existing RmlUi/bgfx texture path. Renderer layer system: 4-layer `GameLayer` enum (Background, Main, Foreground, UIOverlay) each dispatched to its own bgfx view. Scissor/clip stack: `push_scissor`/`pop_scissor` on Renderer with per-draw-call application. Frame timing: wall-clock delta drives ActiveText reveal progress in `RuntimeUI::begin_frame`. Backend-neutral project-schema shader/material records are implemented: shader records declare stages, uniforms, samplers, compiled-binary refs, and supported shader roles; material records reference shaders, select one shader role, and assign uniform/texture values. Runtime shader metadata resolution and `BgfxShaderProgramCache` now resolve material programs and direct ActiveText shader pairs to inferred compiled bgfx variants loaded through `AssetManager`; `ShaderCompilerService` and `noveltea-editor-tool compile-shaders` provide the host/editor/import path that produces those compiled binaries from project shader source; `BgfxMaterialBinder` now binds `ShaderRole::Engine2D` material programs, uniforms, and textures for material-backed engine 2D quads.
...
- Shader-backed ActiveText rendering, bgfx/custom-geometry map rendering, optional map transition animation, per-object/per-room materials, and RmlUi `shader(<string>)` decorator materials remain active. Project-schema shader/material records, shader metadata selection, runtime compiled bgfx binary loading, host/editor/import shaderc compilation, and engine 2D material binding are implemented. ActiveText custom shader rendering and the RmlUi material bridge remain deferred to later phases of [`docs/rendering/NOVELTEA_SHADER_MATERIAL_PLAN.md`](../rendering/NOVELTEA_SHADER_MATERIAL_PLAN.md).
...
Continue the current rendering action plan at Action 6 / shader-material Phase 5: bridge RmlUi `shader(<string>)` decorator usage to NovelTea material ids through a provider seam in reusable `rmlui_bgfx` and a NovelTea adapter that can resolve/bind `ShaderRole::RmlUiDecorator` materials. Keep built-in gradients unchanged. Do not add runtime shader source compilation in that slice.
