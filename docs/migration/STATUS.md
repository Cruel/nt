...
- Shader-backed ActiveText rendering, bgfx/custom-geometry map rendering, optional map transition animation, per-object/per-room materials, and RmlUi `shader(<string>)` decorator materials remain active. Next shader/material work should realign the earlier standalone material-file parser/model to project-schema shader and material records: shader records declare stages, uniforms, samplers, compiled-binary refs, and supported render contracts; material records reference shaders, select one contract, and assign uniform/texture values. Shader metadata selection, runtime compiled bgfx binary loading, editor/import/export shaderc compilation, and the RmlUi material bridge remain deferred to later phases of [`docs/rendering/NOVELTEA_SHADER_MATERIAL_PLAN.md`](../rendering/NOVELTEA_SHADER_MATERIAL_PLAN.md).
