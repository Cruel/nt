...
- Shader-backed ActiveText rendering, bgfx/custom-geometry map rendering, optional map transition animation, per-object/per-room materials, and RmlUi `shader(<string>)` decorator materials remain active. Shader/material resolution is deferred to [`docs/rendering/NOVELTEA_SHADER_MATERIAL_PLAN.md`](../rendering/NOVELTEA_SHADER_MATERIAL_PLAN.md), which defines `.ntmat` assets, editor/import/export shaderc compilation, runtime compiled bgfx binary loading, and the RmlUi material bridge.
