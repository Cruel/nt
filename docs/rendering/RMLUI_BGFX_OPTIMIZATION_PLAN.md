...
Each implementation slice must report before/after perf lines for the readback gallery. Early completed work targeted lower `full_layers`, `full_frame_passes`, `max_layer`, `max_rt`, `clear_px`, `composite_px`, and `post_px`. The next speed target is reducing the remaining `passes=89` / `views=43` through redundant clear removal and any remaining safe copy/view reuse without losing RmlUi feature accuracy.
...
```text
We need to resume docs/rendering/RMLUI_BGFX_OPTIMIZATION_PLAN.md in the middle of Phase 7. Phase 0 through Phase 6a are implemented, Phase 4.5 completed the reusable rmlui_bgfx boundary/subsystem split plus resize-readback regression coverage, and Phase 7 currently includes pass-reason diagnostics, a color-only filter composite fast path, same-target non-clear and post-clear geometry-like view reuse, and a mask-image-only direct source path.

Continue Phase 7: pass-count and render-target-switch reduction. Treat the current representative bottleneck as passes=89 / views=43 plus bgfx FBO/view changes, not filter pixel area. Preserve all RmlUi features and correctness: filters, masks, saved textures, transforms, clips, gradients, blend modes, root preservation, and WebGL feedback-loop protection must continue to work through optimized or conservative fallback paths.

Current color-only filter work is already folded: opacity/color-matrix-only chains report color_filter_folds=9, pass_filter_opacity=0, pass_filter_color=0, bounded_postprocess_passes=6, and post_px=21776 in the readback gallery. Mask-image-only filters now avoid one preliminary filter copy, and same-target/same-rect non-clear draws and geometry-like draws after clears can reuse bgfx views. Next implement the safest remaining opportunities: redundant clear avoidance for targets that are provably fully overwritten, avoidable same-layer scratch-copy narrowing/removal where feedback-loop and overlap checks allow it, and any remaining copy/view reuse where framebuffer, view rect, scissor, stencil, and shader state allow it.
