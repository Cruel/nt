...
The plan below keeps the historical failed-baseline diagnosis because it explains why Phases 0 through 4 existed. For future work, the next high-value work is release/profile smoke coverage plus feature fixtures for the RmlUi render interface gaps identified in `RMLUI_RENDER_INTERFACE_AUDIT.md`, not more speculative renderer simplification. Any optimization that changes pixels must be treated as either a regression or an explicitly approved visual-correctness change against an upstream/approved reference.
