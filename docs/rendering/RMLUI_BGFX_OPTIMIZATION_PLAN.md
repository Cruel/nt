...
9. Phase 7: reduce pass count, render-target switches, redundant clears, avoidable copies, and color-only filter postprocess passes. Partially done: color-only filter postprocess passes are folded into composite and pass-reason diagnostics are in place.
...
Each implementation slice must report before/after perf lines for the readback gallery. Early completed work targeted lower `full_layers`, `full_frame_passes`, `max_layer`, `max_rt`, `clear_px`, `composite_px`, and `post_px`. The next speed target is reducing the remaining `passes=90` / `views=60` through redundant clear/copy removal and safe pass reuse without losing RmlUi feature accuracy.
...
```text
We need to resume docs/rendering/RMLUI_BGFX_OPTIMIZATION_PLAN.md in the middle of Phase 7. Phase 0 through Phase 6a are implemented, Phase 4.5 completed the reusable rmlui_bgfx boundary/subsystem split plus resize-readback regression coverage, and the first Phase 7 slice added pass-reason diagnostics plus a color-only filter composite fast path.

Continue Phase 7: pass-count and render-target-switch reduction. Treat the current representative bottleneck as passes=90 / views=60 plus bgfx FBO/view changes, not filter pixel area. Preserve all RmlUi features and correctness: filters, masks, saved textures, transforms, clips, gradients, blend modes, root preservation, and WebGL feedback-loop protection must continue to work through optimized or conservative fallback paths.

Current color-only filter work is already folded: opacity/color-matrix-only chains report color_filter_folds=9, pass_filter_opacity=0, pass_filter_color=0, bounded_postprocess_passes=6, and post_px=21776 in the readback gallery. Next implement the safest remaining opportunities: redundant clear avoidance for targets that are provably fully overwritten, avoidable same-layer scratch-copy narrowing/removal where feedback-loop and overlap checks allow it, and same-target/same-rect pass reuse where framebuffer, view rect, scissor, stencil, and shader state allow it.
