...
The plan below keeps the historical failed-baseline diagnosis because it explains why Phases 0 through 4 existed. Actions 1 through 8 in `RMLUI_BGFX_RENDERER_REFACTOR_PLAN.md` are implemented, so future work should start from a new focused plan rather than speculative renderer simplification. Any optimization that changes pixels must be treated as either a regression or an explicitly approved visual-correctness change against an upstream/approved reference.
...
Each rendering-output implementation slice must report before/after perf lines for the readback gallery. Early completed work targeted lower `full_layers`, `full_frame_passes`, `max_layer`, `max_rt`, `clear_px`, `composite_px`, and `post_px`. The current action plan in [`RMLUI_BGFX_RENDERER_REFACTOR_PLAN.md`](RMLUI_BGFX_RENDERER_REFACTOR_PLAN.md) is complete through Action 8. Do not reopen Phase 7 or Phase 8 unless a concrete fixture demonstrates safe redundant-clear, copy-removal, or saved-mask metadata opportunities.
