...
This document replaces the previous 11-phase optimization plan. The earlier plan added many useful pieces: counters, rectangle helpers, postprocess target sizing, rectangle-aware composite metadata, no-op filter simplification, and a web smoke harness. The later bounded-compositor work succeeded for the representative readback gallery: current release/profile Web runs are capped at 120 FPS with zero full-frame child layers, zero unbounded layer fallbacks, zero full-frame postprocess passes, and no steady-state render-target churn.

The historical blocker was real: the renderer once executed most layer, clear, composite, and filter work over the full framebuffer. That is no longer the current measured state. Treat the old 5-7 FPS Web numbers as a debug/WebGL validation artifact unless reproduced in an optimized release/profile build with render perf counters compiled in and ImGui disabled.

The current goal is to keep work proportional to affected UI content while hardening feature coverage and regression gates. Future optimization must preserve filters, masks, clips, transforms, gradients, layers, blend modes, saved textures, WebGL feedback-loop safety, and the RmlUi render interface contract. The renderer should stay fast because it does less unnecessary GPU work, not because it silently drops advanced RmlUi behavior.

Feature coverage audit: see [`RMLUI_RENDER_INTERFACE_AUDIT.md`](RMLUI_RENDER_INTERFACE_AUDIT.md). User-provided shader/material support and the RmlUi `shader(<string>)` bridge are planned separately in [`NOVELTEA_SHADER_MATERIAL_PLAN.md`](NOVELTEA_SHADER_MATERIAL_PLAN.md).

## Optimization Priority Update

The original restarted plan correctly attacked the catastrophic full-frame child-layer problem first. That work is now successful in the readback gallery: current steady-state release/profile Web runs report `fps=120`, `full_frame_child_layers=0`, `full_frame_postprocess_passes=0`, `unbounded_layer_fallbacks=0`, `rt_alloc=0`, `rt_destroy=0`, `layer_alloc=0`, `layer_destroy=0`, `max_child_layer=143x121`, `max_child_rt=143x121`, `max_rt=121x121`, and `post_px=31813` at a 1508x1869 framebuffer.

That changes the priority again. The renderer is no longer an emergency Web-performance rescue project. Phase 7 already reduced the pass/view envelope materially, and the browser now reaches its frame cap on the representative gallery. Further pass-count work, filter pixel tightening, and direct-base policy work should be treated as opportunistic cleanup after correctness and coverage gates, not as blockers.

The plan below keeps the historical failed-baseline diagnosis because it explains why Phases 0 through 4 existed. For future work, continue the current action plan in `RMLUI_BGFX_RENDERER_REFACTOR_PLAN.md`, not more speculative renderer simplification. Release/profile smoke coverage and the first focused feature fixtures are implemented; remaining renderer-adjacent work is package/platform verification and documented feature gaps. Any optimization that changes pixels must be treated as either a regression or an explicitly approved visual-correctness change against an upstream/approved reference.
...
Use this order for historical context, but do not keep pushing performance work merely because phases remain. The historical full-frame bounds problem is solved for the readback gallery, Phase 7 tightened pass/view switching, Phase 8 closed saved texture/mask-image bounds for the current gates, and Web release/profile now reaches the browser frame cap. The next work should harden release/profile regression gates and close RmlUi render-interface coverage gaps before further filter rectangle tightening or direct base presentation.
...
11. Phase 9: keep web smoke thresholds enforcing the optimized shape, and add a release/profile smoke path so debug WebGL validation cannot masquerade as renderer cost again.
12. Feature coverage: keep the focused fixtures and tests from [`RMLUI_RENDER_INTERFACE_AUDIT.md`](RMLUI_RENDER_INTERFACE_AUDIT.md) healthy. Initial coverage now includes `backdrop-filter`, perspective/3D transforms, nested `PopLayer()` state restoration, and lifecycle tests for release methods. CSS `box-shadow` shadow pixels and `BlendMode::Replace` reachability remain tracked limitations. For generic `shader(<string>)` and custom shader/material policy, implement [`NOVELTEA_SHADER_MATERIAL_PLAN.md`](NOVELTEA_SHADER_MATERIAL_PLAN.md).
...
Each rendering-output implementation slice must report before/after perf lines for the readback gallery. Early completed work targeted lower `full_layers`, `full_frame_passes`, `max_layer`, `max_rt`, `clear_px`, `composite_px`, and `post_px`. The next target is not more renderer speed work; it is the current action plan in [`RMLUI_BGFX_RENDERER_REFACTOR_PLAN.md`](RMLUI_BGFX_RENDERER_REFACTOR_PLAN.md), currently Android/package verification for material-backed content. Do not reopen Phase 7 or Phase 8 unless a concrete fixture demonstrates safe redundant-clear, copy-removal, or saved-mask metadata opportunities.
