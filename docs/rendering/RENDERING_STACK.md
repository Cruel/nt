...
Every Material role's fragment output contract is premultiplied RGBA, even when the role uses replacement rather than alpha blending. Project shaders that finish with a straight-alpha RGBA value should include the engine-provided `noveltea_shader.sc` and call `noveltea_premultiply_alpha(...)` at the output boundary. This conversion is explicit rather than automatic because some renderer inputs, most notably `engine.postprocess_source`, are already premultiplied and must not be multiplied by alpha a second time. The author-facing contract and examples live in [`docs/engine/SHADER.md`](../engine/SHADER.md).

Material-backed engine 2D quads use `ShaderRole::Engine2D`. The generated Engine2D role contract owns `s_texColor` at stage 0 as `engine.draw_texture`; each draw binds that draw's current texture, or a neutral opaque-white texture for untextured geometry. Material sampling policy is independent from that source/stage ownership: address may be clamp or repeat, while filter may explicitly select nearest/linear or inherit the current draw image's nearest/linear policy. No authored texture source or `$draw.texture` source alias participates in this path, so a Material draw cannot inherit a stale sampler binding from an earlier draw. Pipeline blend/output-alpha state is also role-contract state rather than a Material override.

RmlUi decorator materials use `ShaderRole::RmlUiDecorator` through the NovelTea adapter for `rmlui-bgfx`. The generated RmlUi role contract owns `u_projection`, `u_transform`, `u_translate`, and renderer-provided `s_texColor` at stage 0; the decorator texture is fixed to clamp/linear sampling and cannot be authored as a Material texture source. Runtime and Material preview both provide those renderer inputs from the same contract and compose premultiplied output with premultiplied-alpha blending. RmlUi uses the generic standard semantics such as `engine.paint_dimensions`, `engine.context_logical_to_raster_scale`, and `engine.viewport_pixel_dimensions`; `rmlui.media_query_resolution` is the only RmlUi-specific standard semantic.
