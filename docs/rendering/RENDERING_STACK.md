...
Material-backed engine 2D quads use `ShaderRole::Engine2D`. The generated Engine2D role contract owns `s_texColor` at stage 0 as `engine.draw_texture`; each draw binds that draw's current texture and sampling filter, or a neutral opaque-white texture for untextured geometry. No authored texture source or `$draw.texture` source alias participates in this path, so a Material draw cannot inherit a stale sampler binding from an earlier draw. Pipeline blend/output-alpha state is also role-contract state rather than a Material override.
