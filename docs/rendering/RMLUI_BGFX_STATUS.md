...
- Filters now expose conservative composited output bounds separately from tighter semantic valid-output bounds. The current shader work still runs over already-bounded targets; per-valid-rect postprocess draws remain deferred until visual regression coverage is stronger.
- Pass count is still high (`passes=108`). Since bounded work is now stable for the readback gallery, the next high-value speed target is Phase 7 pass-count and render-target-switch reduction.
...
The remaining optimization work is no longer discovery, child-layer materialization, normal transform bounding, or the filter allocation/content-bounds data-model split. It is pass-count/render-target-switch reduction first, then saved texture/mask-image follow-up, visual-regression-safe filter tightening when profiling justifies it, and later direct-base policy cleanup.
...
Phase 6a is complete as a behavior-preserving data-model split. Resume the optimization plan at Phase 7: pass-count and render-target-switch reduction. Do not spend more time shrinking already-small filter pixel rectangles unless profiling shows `post_px` is again dominant.
