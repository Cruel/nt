...
Next work should start from a new focused plan. Remaining gaps are CSS `box-shadow` shadow pixels, custom RmlUi filter policy, `BlendMode::Replace` reachability if a markup/property path is found, and physical extraction of `rmlui_bgfx` after the public extension policy is clearer.
