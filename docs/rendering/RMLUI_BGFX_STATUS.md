...
Representative steady-state Linux perf after the current Phase 7 partial work at 1280x720:
...
- `passes=89`, `views=58`, and `view_reuses=31`: the perf log distinguishes pass-builder operations from reused bgfx views, and the scheduler now reuses adjacent same-target/same-rect non-clear views across logical pass kinds.
...
- Pass count is still high (`passes=89`, `views=58`). The next Phase 7 targets are redundant layer clear reduction and any remaining copy/view reuse cases that can be proven safe.
...
| Phase 7: Pass Count and Render-Target Switch Reduction | IMPLEMENTED, PARTIAL | Phase 7 now adds pass-reason diagnostics, prints pass-builder operations vs reused bgfx views, folds opacity/color-matrix-only chains into a composite-filter shader, reuses adjacent same-target/same-rect non-clear views, and bypasses the preliminary filter copy for mask-image-only filters. Linux build, full test suite, readback, resize-readback, web build/smoke, and format-check pass. Readback-gallery steady state drops from `passes=108` to `89`, `views=60` to `58`, `bounded_postprocess_passes=15` to `6`, and reports `color_filter_folds=9`. |
...
| Web structural smoke | VERIFIED | `node scripts/web-smoke.mjs` passes with zero full-frame child layers, zero unbounded fallbacks, zero full-frame postprocess passes, `max_child_layer=114x96`, and `passes=89`. |
...
The remaining optimization work is no longer discovery, child-layer materialization, normal transform bounding, or the filter allocation/content-bounds data-model split. Phase 7 pass-count/render-target-switch reduction is underway; color-only filter chains now fold into composition, mask-image-only filters avoid the preliminary filter copy, and same-target non-clear draws can reuse bgfx views. Remaining Phase 7 work is mainly redundant layer-clear reduction plus any remaining safe copy/view reuse, then saved texture/mask-image follow-up, visual-regression-safe filter tightening when profiling justifies it, and later direct-base policy cleanup.
...
Continue Phase 7 after color-only composite folding, same-target view reuse, and the mask-image-only direct source path. The next targets are redundant clear skipping where a bounded target is provably fully overwritten, same-layer scratch-copy narrowing where WebGL feedback-loop and overlap checks allow it, and any remaining safe copy reductions.
