...
- Pass count is still high (`passes=89`, `views=43`). The next Phase 7 targets are redundant layer clear reduction and any remaining copy/view reuse cases that can be proven safe.
...
Continue Phase 7 after color-only composite folding, same-target/post-clear view reuse, and the mask-image-only direct source path. The next targets are redundant clear skipping where a bounded target is provably fully overwritten, same-layer scratch-copy narrowing where WebGL feedback-loop and overlap checks allow it, and any remaining safe copy reductions.
