...
| Phase 8: Saved Texture and Mask-Image Bounds | IMPLEMENTED, PARTIAL | `SaveLayerAsMaskImage()` now copies `valid_content_bounds` when those bounds are tighter than the materialized layer, stores matching saved texture dimensions, and emits exact mask bounds for later mask-image sampling. A unit test covers the copied region, saved texture metadata, and filter mask metadata. Linux readback remains correct; representative gallery perf is unchanged because the existing saved mask is already tightly bounded. |
...
Continue Phase 8 by auditing saved texture ownership/release lifetime and adding targeted tests for owned saved mask release. The saved-mask copy-bounds path is implemented, but Phase 8 is not complete until release lifetime coverage is in place.
