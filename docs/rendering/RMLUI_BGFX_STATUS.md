...
Current status: Phases 0 through 8 of [`RMLUI_BGFX_OPTIMIZATION_PLAN.md`](RMLUI_BGFX_OPTIMIZATION_PLAN.md) are implemented and verified on the Linux debug test suite, Phase 4.5's reusable-boundary/subsystem split is complete, and the reusable renderer has been physically extracted into the sibling `rmlui-bgfx` repository. NovelTea now consumes the renderer through the external `rmlui_bgfx::rmlui_bgfx` package/FetchContent target rather than building an internal `rmlui_bgfx_renderer` static library. Local development uses CMake's standard `FETCHCONTENT_SOURCE_DIR_RMLUI_BGFX` override; the NovelTea repository must not add repository-relative `add_subdirectory` calls or any other repository-relative dependency path. The renderer package owns the reusable core implementation, public provider interfaces, migrated renderer unit tests, and reusable RmlUi bgfx shader sources. NovelTea keeps only adapter policy: AssetManager texture loading, packaged shader/material mapping, view-id allocation, SDL input/system interfaces, Lua, custom components, and runtime/editor integration.
...
| Phase 4.5: Reusable Renderer Boundary and Subsystem Split | VERIFIED | Completed reusable `rmlui_bgfx` core boundary, NovelTea adapter services, shared renderer-private types, `TextureRegion` coordinate discipline, target cache, pass builder, draw context, filter pipeline, layer system, thin adapter cleanup, and resize/readback regression coverage. The reusable core has now been physically extracted into `https://github.com/Cruel/rmlui-bgfx` with standalone CMake, vcpkg manifest, public headers, private implementation files, migrated unit tests, shader sources, shader helper CMake, and sample-backend overlay documentation. NovelTea links `rmlui_bgfx::rmlui_bgfx` and keeps only the NovelTea adapter layer. |
...
| Generic `shader(<string>)` decorator | IMPLEMENTED, NOT FULLY VERIFIED | Built-in gradients are supported. Generic shader decorators now resolve the string as a NovelTea `ShaderRole::RmlUiDecorator` material id through the provider bridge and precompiled bgfx binaries. A focused `material_shader.rml` fixture smoke passes; package/export tests verify runtime material metadata and required shader binaries. Broader visual/readback coverage can still be expanded. |
| Material runtime package export | VERIFIED | `shader-materials.json` package export, source stripping, recursive shader binary inclusion, required material variant diagnostics, package reader preservation, runtime package metadata loading, and sandbox material-package smoke pass in the Linux test slice. |
...
| Android package assemble | VERIFIED | `sh ./gradlew :app:assembleDebug` passes and `stageNovelTeaRuntimeAssets` verifies required `essl-300` material shader binaries. |
...
Actions 1 through 8 in [`RMLUI_BGFX_RENDERER_REFACTOR_PLAN.md`](RMLUI_BGFX_RENDERER_REFACTOR_PLAN.md) are implemented. The old emergency performance path is no longer the next priority. Release/profile web smoke coverage, the shader/material data model, runtime program resolution, host shader compilation, engine 2D material binding, RmlUi `shader(<string>)` material bridge, feature fixtures, package metadata export, missing material-variant export failures, runtime package material metadata loading, and Android material-shader asset staging checks are in place.

Next work should start from a new focused plan. Remaining renderer gaps are CSS `box-shadow` shadow pixels, custom RmlUi filter policy, and `BlendMode::Replace` reachability if a markup/property path is found. Remaining package/extraction follow-up is to implement the full upstream RmlUi sample backend overlay. During active tandem development, NovelTea intentionally tracks the `master` branch of `rmlui-bgfx`; once the renderer API stabilizes, switch to a pushed commit or release tag.
