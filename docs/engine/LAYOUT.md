...
Activation-triggered Mounts may carry an immutable engine-owned **Trigger Context** containing canonical normalized pointer and source-bounds geometry. The snapshot contains no DOM, renderer, Hotspot, or world-object handles. `context:trigger()` transforms that snapshot into the receiving Layout's current logical coordinate space. `context:anchor(width, height, source?, side?, alignment?, gap?, padding?, offsetX?, offsetY?)` resolves pointer, source-bounds, or nearest-source-point anchoring with preferred side/alignment, applies the optional signed logical X/Y offset, then performs viewport-safe adjustment. `gap` remains the distance away from the selected source along the chosen side axis. Because the geometry is immutable, directly called Interaction/Scene/Dialogue presentation can reuse it without retaining a live activation element.
