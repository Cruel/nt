...
`Primary Activate` asks runtime to execute a unique immediately-complete primary Offer or otherwise open the ordinary Verb Menu. `Open Verb Menu` only opens that menu and never auto-selects a primary Offer. The ordinary pointer policy maps left-click/tap to Primary Activate and right-click to Open Verb Menu. Both paths capture immutable Trigger Context geometry at activation; that snapshot follows the resulting Interaction and directly called Scene/Dialogue so contextual Layout presentation can anchor without retaining a live pointer target. Ambiguous primary Offers produce a typed diagnostic observation and leave the player at the menu.

The Verb Menu is a separate reusable Layout, not part of the multi-slot Command Builder. Project Settings may select a default Verb Menu Layout; otherwise runtime uses the built-in anchored fallback. Opening one ordinary Verb Menu replaces the previous ordinary menu presentation while preserving the selected semantic subject and Trigger Context for command execution.
