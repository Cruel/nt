...
`noveltea.test-suite-report` aggregate. These catalog/report documents are generated disposable
products that inherit their owning cache/CLI/runtime boundaries; they use schema discriminators but do
not define independent numeric compatibility versions. Executed entries retain their complete
individual playback report. Their suite status is `passed` or `failed` from the playback report;
native/runtime execution failures are `error`, with actionable error diagnostics promoted onto the
aggregate entry while the complete playback report remains intact. A failed/error entry does not stop
later independent entries, while common Compiled Project admission failure aborts the suite before
per-test results are published.
