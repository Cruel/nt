...
assignments. Mutable Location/enabled/visible state does not belong to the Definition. Editor edits
...
state, Trait additions/removals, and sparse Property overrides. Multiple Instances may reference the
same Definition and remain independent identities.
...
- **Authoring Definition:** collection-specific Interactable record, Trait attachments, typed Property
  assignments, owner-local Features, immutable presentation, and semantic Hotspot targets.
- **Authoring Instance:** infrastructure-level `interactableInstances` registry entry with exact ID,
  Definition reference, optional editor label, Location/state, and Trait/Property deltas.
...
existing Instance atomically. The creation wizard accepts Definition identity, an optional
...
Rooms/Inventories and containment cycles atomically.
