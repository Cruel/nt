...
Runtime Material Parameters support typed mutation and query without mutating compiled Material definitions. Interactable applications can layer runtime state at Material-wide, Interactable Definition, or concrete Interactable occurrence scope; direct values and Property or standard engine-facet bindings use the same specificity model. Interactable Definitions and concrete Instances also support independent runtime Material-selection overrides. Existing presentation occurrences continue to support their occurrence-local runtime parameter state, including Scene assignment/tweening and Lua presentation APIs.
...
Texture overrides separate source ownership from sampling policy. Author-owned slots may contain a `source`, `address` (`clamp` or `repeat`), `filter` (`nearest` or `linear` after runtime lowering), binding metadata, and editor metadata. Renderer-owned slots never accept an authored source, but may store sparse `address`/`filter` policy when their role contract explicitly permits it. A missing `source` therefore means either that the current Material inherits an author-owned source or that the source is renderer-owned; an empty URI is not a valid way to clear a texture. `engine.draw_texture` additionally permits `filter: "inherit"`, which preserves the draw image's nearest/linear sampling while applying the Material's address mode.
...
Interactable Definitions and concrete Interactable Instances are complete Engine2D hosts for this contract. A Definition owns the inherited application. Each Instance stores only a specialization delta: an optional Material-selection replacement plus sparse named parameter and author-owned texture overrides. Instance entries win over matching Definition entries; absent entries fall through unchanged to the Definition and then to Material/preset defaults. Clearing an Instance entry therefore reveals the Definition value without rewriting either lower layer. Ordinary parameter overrides may be typed literals, bindings to compatible Interactable Properties, or supported standard facets such as occurrence time and paint/viewport dimensions. Standard facets are scalar float sources. The compiled/runtime presentation path resolves the effective parameter independently for each concrete Interactable occurrence and passes only that coherent value through the ordinary occurrence-uniform binder.

Runtime state adds two independent sparse layers over that authored application: Material selection and Material Parameters. Effective Interactable Material selection precedence is concrete Instance runtime selection, authored Instance Material override, Interactable Definition runtime selection, then the authored/effective Definition selection. Clearing a runtime selection removes only that desired-state record and immediately exposes the next lower selection; it never rewrites compiled Definition or Instance data.

For the selected Interactable Material, effective parameter precedence is: concrete occurrence runtime state, authored Instance specialization, Interactable Definition runtime state, authored Definition specialization, Material-wide runtime state, then the authored Material/preset/default value. Runtime parameter records remain keyed by scope, Material identity, and parameter name even while another Material is selected. Switching Materials therefore makes incompatible parameter values or bindings dormant rather than deleting them, and switching back reactivates the stored state. A runtime parameter layer may hold either a direct typed value or a binding. Clearing one layer deletes only that state and immediately reveals the next applicable layer. Renderer-owned inputs are excluded at every runtime scope by the same certified Material-interface validation used by authored applications. Material-wide and Definition runtime state is projected to concrete occurrence overrides before rendering, so the renderer receives at most one effective override for a given Interactable parameter. Selection and parameter desired-state records participate in the normal checkpoint/save/restore path.
...
Named application entries are non-destructive across Material changes at both Definition and Instance layers. If the newly selected effective Material does not expose a compatible authorable parameter or texture slot, the saved entry remains dormant rather than being deleted; selecting a compatible Material again can reactivate it. Instance Material selection is independent from its sparse named deltas, so Reset-to-inherited does not destroy Material-keyed experimentation. Runtime application considers only entries compatible with the selected Material's certified interface.
...
For the `engine-2d` role, `s_texColor` is the renderer-owned `engine.draw_texture` input at stage 0. A Material does not author a texture source for that sampler: each draw binds the current sprite/draw texture, or a neutral opaque-white texture when the draw has no visual texture. The Material may independently specialize address as `clamp` or `repeat` and filter as `inherit`, `nearest`, or `linear`; `inherit` uses the current draw image's nearest/linear policy without changing the Material-selected address mode. There is no `$draw.texture` alias or source-string fallback in the canonical contract. The same contract-owned fixture behavior and sampling policy are used by Material preview, so preview data does not synthesize an authored `s_texColor` source. Engine2D output is premultiplied RGBA and uses the role's premultiplied-alpha pipeline state. Custom fragment shaders that finish with a straight-alpha RGBA value should include `noveltea_shader.sc` and write `noveltea_premultiply_alpha(color)`; see `SHADER.md` for the output contract and the important exception for already-premultiplied inputs such as postprocess source textures.
...
Role is read-only and comes from the terminal preset. Reflected shader inputs drive the focused parameter/texture controls for custom Materials; preset metadata and authored editor metadata decorate those inputs with labels, ranges, and control intent. The focused editor has a distinct **Renderer Inputs** section for reserved attributes, backend/predefined uniforms, and renderer-owned samplers. These entries show their physical type, semantic, and sampler stage as read-only contract data; only sampler `address`/`filter` controls explicitly allowed by the role contract are editable. Renderer-owned sampler sources remain runtime supplied.

The reusable Material Application editor uses that same certified reflected interface. It combines Material selection with per-occurrence Override/Reset controls, shows whether a value is inherited from the Material or explicitly specialized by the host, exposes compatible Property/standard-facet sources, and keeps incompatible saved entries visible as dormant configuration. Material-selector candidate previews receive the complete previewable application context: literal values, resolvable Property values, dynamic standard facets, and author-owned texture overrides. Candidate compatibility counts include saved parameter and texture deltas, so an unresolved or incompatible entry is reported as unapplied rather than silently disappearing from the preview accounting.
