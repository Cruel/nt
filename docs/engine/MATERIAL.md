...
Runtime Material Parameters support typed mutation and query without mutating compiled Material definitions. Interactable applications can layer runtime state at Material-wide, Interactable Definition, or concrete Interactable occurrence scope; direct values and Property or standard engine-facet bindings use the same specificity model. Existing presentation occurrences continue to support their occurrence-local runtime parameter state, including Scene assignment/tweening and Lua presentation APIs.
...
Runtime parameter state is an additional sparse layer over that authored application. For a selected Interactable Material, effective parameter precedence is: concrete occurrence runtime state, authored Instance specialization, Interactable Definition runtime state, authored Definition specialization, Material-wide runtime state, then the authored Material/preset/default value. A runtime layer may hold either a direct typed value or a binding. Clearing one layer deletes only that state and immediately reveals the next applicable layer; it never rewrites the authored Definition, Instance, or Material. Renderer-owned inputs are excluded at every runtime scope by the same certified Material-interface validation used by authored applications. Material-wide and Definition runtime state is projected to concrete occurrence overrides before rendering, so the renderer receives at most one effective override for a given Interactable parameter. These scoped records participate in the normal checkpoint/save/restore path.
