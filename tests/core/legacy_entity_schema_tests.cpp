...
nlohmann::json props()
{
    return nlohmann::json::object({{"seen", true}});
}
...
TEST_CASE("legacy entity schema parses scalar entity array records")
{
    ...
    auto object = parse_entity_record(
        EntityType::Object,
    ...
    auto script = parse_entity_record(
        EntityType::Script,
        nlohmann::json::array({"boot", "", props(), true, "toast('hi');"}),
        errors,
        "script/boot");
...
TEST_CASE("legacy entity schema parses room and map nested records")
{
    ...
    auto room = parse_entity_record(
        EntityType::Room,
        nlohmann::json::array({"foyer", "", props(), "text='Foyer';", "return true;", "", "return true;", "",
    ...
    auto map = parse_entity_record(
        EntityType::Map,
        nlohmann::json::array({"main", "", props(), "return true;", "return true;",
...
TEST_CASE("legacy entity schema parses dialogue and cutscene records")
{
    ...
    auto dialogue_segment = nlohmann::json::array(
        {1, -1, true, false, true, false, true, true, "return true;", "toast('x');", "[Guide] Hello",
         nlohmann::json::array({1, 2})});
    auto dialogue = parse_entity_record(
        EntityType::Dialogue,
        nlohmann::json::array({"intro_dialogue", "", props(), "Guide", ref(EntityType::Room, "foyer"), 0, false,
                               true, 1, nlohmann::json::array({dialogue_segment})}),
        errors,
    ...
    auto cutscene = parse_entity_record(
        EntityType::Cutscene,
        nlohmann::json::array({"intro", "", props(), true, true, 1.25, ref(EntityType::Dialogue, "intro_dialogue"),
                               nlohmann::json::array({
...
TEST_CASE("legacy entity schema reports malformed nested records with field paths")
{
    ...
    const auto parsed = parse_entity_record(
        EntityType::Map,
        nlohmann::json::array({"main", "", props(), "", "", nlohmann::json::array({
                                   nlohmann::json::array({"bad room", 0, 0, 10, "wide", nlohmann::json::array(), "", 1}),
                               }),
                               nlohmann::json::array()}),
        errors,
        "map/main");
