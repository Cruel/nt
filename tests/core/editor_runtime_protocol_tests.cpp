        ...
        {"world",
         {{"presentationSpace",
           {{"size", {{"width", 1920.0}, {"height", 1080.0}}},
            ...
            {"edgePolicy", "contain"},
            {"view",
             {{"center", {{"x", 960.0}, {"y", 540.0}}}, {"zoom", 1.0}, {"rotationDegrees", 0.0}}}}},
...
TEST_CASE("focused preview manifest image sampling is explicit and discriminated")
{
    ...
    for (const std::string_view kind :
        ...
        request["resources"][0].erase("sampling");
        auto non_image = decode_focused_editor_document_request_text(request.dump());
        REQUIRE(non_image);
        CHECK(non_image.value().resources.front().kind == kind);
...
TEST_CASE("typed playback report encoder has stable external shape")
{
    ...
    step.index = 4;
    step.handled = true;
    step.events.push_back(
        noveltea::runtime::ObservationEvent{RuntimeObservation{PlaybackObservation{4, true}}});
