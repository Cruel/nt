...
ProjectDocument make_valid_project()
{
    ...
    root[project_ids::map] = nlohmann::json::object({
        {"main", nlohmann::json::array({"main", "", props(), "", "",
    ...
    root[project_ids::cutscene] = nlohmann::json::object({
        {"intro", nlohmann::json::array({"intro", "", props(), true, true, 1.0, ref(EntityType::Dialogue, "intro_dialogue"),
                                         nlohmann::json::array({nlohmann::json::array(
...
bool has_issue(const std::vector<ValidationIssue>& issues, std::string_view path, std::string_view text)
{
...
TEST_CASE("ProjectValidator reports missing entrypoint inventory action and room refs")
{
    ...
    CHECK(has_issue(issues, "/action/take_lamp[5][0]", "missing object entity 'missing_action_object'"));
...
TEST_CASE("ProjectValidator reports map and dialogue graph index errors")
{
    ...
    CHECK(has_issue(issues, "/dialogue/intro_dialogue[9][0][11][0]", "dialogue child id is out of range"));
    CHECK(has_issue(issues, "/dialogue/intro_dialogue[9][1][1]", "dialogue link id is out of range"));
