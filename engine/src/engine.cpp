...
std::string shell_mode_name(RuntimeShellMode mode)
{
    switch (mode) {
...
nlohmann::json debug_entity_ref(const core::EntityRef& ref, const core::ProjectModel* project)
{
                                           ...
                                           const core::ProjectModel* project)
{
    ...
    if (diagnostic.source)
        out["source"] = debug_entity_ref(*diagnostic.source, project);
...
bool diagnostics_have_error(const nlohmann::json& diagnostics)
{
...
std::string Engine::runtime_preview_debug_snapshot() const
{
    ...
    for (const auto& object : view.objects) {
        ...
        if (host.loaded()) {
            if (const auto location = session.effective_object_location(object.id); location) {
                item["location"] = debug_entity_ref(*location, project);
            }
        }
        ...
        for (const auto& [object_id, object_model] : project->objects()) {
            ...
            inventory.push_back(
                {{"id", object_id},
                 ...
                 {"selected", false},
                 {"enabled", true},
                 {"location", debug_entity_ref(*location, project)}});
        }
    }

    for (const auto& diagnostic : m_runtime_shell.last_diagnostics()) {
        diagnostics.push_back(runtime_diagnostic_snapshot(diagnostic, project));
    }
    ...
    } else if (m_runtime_shell.paused()) {
        waiting_kind = "paused";
        waiting_reason = "runtime shell is paused";
    } else if (diagnostics_have_error(diagnostics)) {
        waiting_kind = "error";
    ...
    } else if (m_runtime_shell.mode() == RuntimeShellMode::Title) {
        waiting_kind = "title";
    ...
    nlohmann::json snapshot = {{"loaded", host.loaded()},
                               {"running", host.loaded() && m_preview_running},
                               {"shellMode", shell_mode_name(m_runtime_shell.mode())},
    ...
    if (const auto entrypoint = session.startup_entrypoint(); entrypoint && entrypoint->has_id()) {
        snapshot["entrypoint"] = debug_entity_ref(*entrypoint, project);
    }
    if (const auto current_entity = session.current_entity();
        current_entity && current_entity->has_id()) {
        snapshot["currentEntity"] = debug_entity_ref(*current_entity, project);
