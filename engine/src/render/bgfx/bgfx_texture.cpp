...
uint16_t Renderer::load_ppm_texture(std::string_view logical_path)
{
    ...
    if (!bytes) {
        SDL_Log("[renderer] failed to read texture %.*s: %s",
            static_cast<int>(logical_path.size()), logical_path.data(), bytes.error.c_str());
        return UINT16_MAX;
    }
    ...
    for (int i = 0; i < width * height; ++i) {
        ...
        int b = 0;
        if (!(in >> r >> g >> b)) {
            SDL_Log("[renderer] incomplete PPM texture %.*s",
                static_cast<int>(logical_path.size()), logical_path.data());
            return UINT16_MAX;
        }
    ...
    SDL_Log("[renderer] loaded texture: %.*s (%dx%d)", static_cast<int>(logical_path.size()), logical_path.data(), width, height);
    return texture.idx;
}
