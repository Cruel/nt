                               ...
                               Diagnostics& diagnostics)
{
    ...
    for (const auto& cursor : cursor_settings.named) {
        if (!valid_strong_id(cursor.id, StrongIdSyntax::KebabCase) || reserved_cursor_id(cursor.id) ||
            !cursor_ids.insert(cursor.id).second) {
            diagnostics = invalid_model("Named cursor ID is invalid or reserved");
            return false;
        }
        const auto asset = std::ranges::find_if(
            input.assets, [&](const compiled::AssetResource& candidate) { return candidate.id == cursor.image; });
        if (asset == input.assets.end() || asset->kind != compiled::AssetKind::Image || !asset->width ||
            !asset->height || *asset->width > 128 || *asset->height > 128 ||
    ...
    const auto target_named_exists = [&](const compiled::CursorTarget& target) {
        return target.kind != compiled::CursorTargetKind::Named || cursor_ids.contains(target.named_id);
    };
