...
{
    const auto it = std::find_if(m_listeners.begin(), m_listeners.end(), [id](const ListenerRecord& listener) {
...
void RuntimeEventBus::push(RuntimeEvent event)
{
...
std::size_t RuntimeEventBus::queued_count() const noexcept
{
    return m_queue.size();
}

bool RuntimeEventBus::empty() const noexcept
{
...
RuntimeTimerHandle RuntimeTimerScheduler::start(double duration_seconds, RuntimeTimerCallback callback)
{
...
RuntimeTimerHandle RuntimeTimerScheduler::start_repeat(double duration_seconds, RuntimeTimerCallback callback)
{
    if (duration_seconds <= 0.0) {
...
{
    return std::any_of(m_timers.begin(), m_timers.end(), [id](const Timer& timer) {
        return timer.id == id && !timer.completed;
    });
}

std::size_t RuntimeTimerScheduler::active_count() const noexcept
{
    return static_cast<std::size_t>(std::count_if(m_timers.begin(), m_timers.end(), [](const Timer& timer) {
        return !timer.completed;
    }));
}

void RuntimeTimerScheduler::reset()
{
    m_timers.clear();
}
...
bool RuntimeTimerScheduler::update(double delta_seconds, RuntimeEventBus* events)
{
    ...
    for (const auto id : initial_timer_ids) {
        auto find_timer = [&]() {
            return std::find_if(m_timers.begin(), m_timers.end(), [id](const Timer& timer) {
                return timer.id == id;
            });
        };
        ...
        while (it != m_timers.end() && !it->completed && it->elapsed_seconds >= it->duration_seconds) {
    ...
    m_timers.erase(std::remove_if(m_timers.begin(), m_timers.end(), [](const Timer& timer) {
        return timer.completed;
    }), m_timers.end());
