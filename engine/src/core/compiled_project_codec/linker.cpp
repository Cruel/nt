...
std::optional<std::string> realize_static_text(const TextContent& text,
                                               ...
                                               const std::string& path, std::string_view field_name)
{
    ...
    if (const auto* message = std::get_if<MessageRef>(&text.source)) {
...
std::optional<std::string> realize_display_name(const TextContent& text,
                                                const compiled::Localization& localization,
                                                Diagnostics& diagnostics,
                                                std::string_view source_path,
                                                const std::string& path)
{
    return realize_static_text(text, localization, diagnostics, source_path, path, "Display name");
}
...
#define LINK_DEFINITIONS(wire_member, output_member, output_type, id_type, body)                   \
    ...
    for (std::size_t index = 0; index < wire.characters.size(); ++index) {
        ...
        if (!identity)
            continue;
        auto display_name = realize_display_name(value.display_name, wire.localization, diagnostics,
                                                 source_path, path + "/displayName");
        if (!display_name)
            continue;
        characters.push_back(compiled::CharacterDefinition{
            std::move(*identity), std::move(*display_name), std::move(*property_contracts),
    ...
    for (std::size_t index = 0; index < wire.rooms.size(); ++index) {
        ...
        rooms.push_back(compiled::RoomDefinition{
            ...
            compiled::RoomLifecycle{
                std::move(value.lifecycle.can_enter), std::move(value.lifecycle.can_leave),
                std::move(value.lifecycle.before_enter), std::move(value.lifecycle.after_enter),
                std::move(value.lifecycle.before_leave), std::move(value.lifecycle.after_leave),
                std::move(value.lifecycle.on_enter_rejected),
                std::move(value.lifecycle.on_leave_rejected)},
            std::move(value.overlays), std::move(value.cast), std::move(value.interactables),
            std::move(value.fallback_interactable_placement), std::move(value.props),
            std::move(value.environments), std::move(value.script_hooks),
    ...
    for (std::size_t index = 0; index < wire.interactables.size(); ++index) {
        ...
        interactables.push_back(compiled::InteractableDefinition{
            std::move(*identity), std::move(*display_name), value.stackable,
            ...
            if (identity && property_contracts && display_name)
                configuration = compiled::RoomDefinition{
                    ...
                    compiled::RoomLifecycle{std::move(room->lifecycle.can_enter),
                                            ...
                                            std::move(room->lifecycle.before_enter),
                                            std::move(room->lifecycle.after_enter),
                                            std::move(room->lifecycle.before_leave),
                                            std::move(room->lifecycle.after_leave),
                                            std::move(room->lifecycle.on_enter_rejected),
                                            std::move(room->lifecycle.on_leave_rejected)},
                    std::move(room->overlays),
                    std::move(room->cast),
                    std::move(room->interactables),
                    std::move(room->fallback_interactable_placement),
                    std::move(room->props),
    ...
    LINK_DEFINITIONS(
        scenes, scenes, SceneDefinition, SceneId,
        (compiled::SceneDefinition{
            std::move(identity),
            realize_display_name(value.display_name, wire.localization, diagnostics, source_path,
...
#undef LINK_DEFINITIONS
    ...
    compiled::RuntimeSettings settings{
        std::move(wire.settings.display),
        std::move(wire.settings.accessibility),
    ...
    auto result = CompiledProject::create(compiled::CompiledProjectInput{
        ...
        .flow_prediction = std::move(wire.flow_prediction),
