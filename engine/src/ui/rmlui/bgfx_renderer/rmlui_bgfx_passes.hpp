...
public:
    ...
    [[nodiscard]] std::optional<RmlUiPass> geometry(bgfx::FrameBufferHandle target, int width,
                                                    int height,
    ...
    [[nodiscard]] std::optional<RmlUiPass> composite(bgfx::FrameBufferHandle target,
                                                     LocalFbRect destination_rect,
                                                     RmlUiPassKind kind, const char* name);
    [[nodiscard]] std::optional<RmlUiPass> copy(bgfx::FrameBufferHandle target, int width,
                                                int height, const char* name);
    [[nodiscard]] std::optional<RmlUiPass> postprocess(bgfx::FrameBufferHandle target, int width,
                                                       int height, const char* name);
