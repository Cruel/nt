...
EM_JS(void, nt_preview_emit_object_clicked, (const char* object_id, float ox, float oy, float px, float py), {
    const bridge = globalThis.NovelTeaPreviewBridge;
    if (bridge && typeof bridge.send === 'function') {
        bridge.send({
            version: 1,
...
void emit_object_clicked(
    const char* object_id,
    NormalizedPosition object_position,
    NormalizedPosition pointer_position)
{
#if defined(__EMSCRIPTEN__)
    nt_preview_emit_object_clicked(
        object_id,
        object_position.x,
        object_position.y,
        pointer_position.x,
