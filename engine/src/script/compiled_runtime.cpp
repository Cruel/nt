...
CompiledRuntime::load(CompiledRuntimeLoadInput input, ScriptRuntime& scripts,
                      core::TypedSaveSlotStore& saves)
{
    ...
    auto lua_diagnostics = certify_compiled_project_lua(package.value_if()->project(), scripts);
    if (!lua_diagnostics.empty())
        return core::Result<std::unique_ptr<CompiledRuntime>, core::Diagnostics>::failure(
            std::move(lua_diagnostics));

    auto runtime =
        std::unique_ptr<CompiledRuntime>(new CompiledRuntime(std::move(*package.value_if())));
