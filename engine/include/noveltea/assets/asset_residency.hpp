...
// Resolved policies carry explicit per-domain Warm ceilings. Both the planner and
// AssetResidencyManager use these helpers so admission advice and final residency enforcement
// cannot drift in their Warm-budget semantics.
