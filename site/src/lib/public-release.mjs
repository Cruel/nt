...
function parseAsset(value, collection) {
  if (!value || typeof value !== 'object') throw new Error(`${collection} release asset must be an object.`);
  for (const key of ['platform', 'arch', 'format', 'label', 'file', 'url', 'sha256']) {
...
export function loadPublicReleaseManifest(manifestPath = process.env.NOVELTEA_RELEASE_MANIFEST_PATH) {
...
export function groupAssetsByPlatform(assets) {
  return ['windows', 'linux']
    .map((platform) => ({ platform, assets: assets.filter((asset) => asset.platform === platform) }))
