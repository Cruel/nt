...
The release manifest is validated before rendering. It also records the exact `noveltea-examples` revision, matching release-player build ID, and public `noveltea-examples-<tag>.zip` archive. Release CI creates that archive by qualifying the examples pin stored in the tagged `nt` checkout against the exact released Linux CLI and canonical threaded Web player. The site downloads and stages that immutable release generation under `/examples`, while `/examples/dev` continues to use the separately qualified development generation. Product download links and release-example archives must point directly to `Cruel/noveltea-releases/releases/download/...`; source-repository release URLs are not accepted by the site contract.
...
The static tests verify the built routes, catalog, handoff actions, isolation contract, channel selection, and that each generated showcase contains exactly one shared Wasm player rather than one copy per example. Browser tests serve the built site with the production isolation headers, start both development examples, and—when release inputs are present—exercise the release showcase independently with the exact release-tagged player while keeping `/examples/dev` on its development generation.
