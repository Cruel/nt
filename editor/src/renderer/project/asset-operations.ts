...
import { assetDataFromImportMetadata, defaultAssetIdFromFilename, parseAssetData, validateAssetAlias } from '../../shared/project-schema/authoring-assets';
import { buildAssetAliasIndex, findAssetAliasUsages, findAssetRecordByAlias } from '../../shared/project-schema/authoring-asset-references';
...
function aliasConflict(project: AuthoringProject, alias: string, allowedAssetId?: string): string | null {
  const owner = findAssetRecordByAlias(project, alias);
  if (!owner) return null;
  return owner.entityId === allowedAssetId ? null : owner.entityId;
}

export function importAssetRecordsPatches(document: JsonValue | unknown, payload: AssetImportPayload): AssetOperationResult {
  const project = validateProject(document);
  if (!isAuthoringProject(project)) return { patches: [], diagnostics: [project] };
  if (!payload.assets.length) return { patches: [], diagnostics: [error('No imported assets were provided.')] };
...
export function assignAssetAliasPatches(document: JsonValue | unknown, payload: AssetAliasPayload): AssetOperationResult {
  const project = validateProject(document);
  if (!isAuthoringProject(project)) return { patches: [], diagnostics: [project] };
  const record = project.assets[payload.assetId];
  if (!record) return { patches: [], diagnostics: [error('Asset does not exist.', assetPath(payload.assetId))] };
  const aliasError = validateAssetAlias(payload.alias);
  if (aliasError) return { patches: [], diagnostics: [error(aliasError, assetAliasesPath(payload.assetId))] };
  const conflict = aliasConflict(project, payload.alias, payload.assetId);
  if (conflict) return { patches: [], diagnostics: [error(`Alias is already assigned to assets/${conflict}.`, assetAliasesPath(payload.assetId))] };
  const data = parseAssetData(record.data);
  if (!data) return { patches: [], diagnostics: [error('Asset record has invalid asset data.', assetDataPath(payload.assetId))] };
...
export function removeAssetAliasPatches(document: JsonValue | unknown, payload: AssetAliasPayload): AssetOperationResult {
  const project = validateProject(document);
  if (!isAuthoringProject(project)) return { patches: [], diagnostics: [project] };
  const record = project.assets[payload.assetId];
  if (!record) return { patches: [], diagnostics: [error('Asset does not exist.', assetPath(payload.assetId))] };
  ...
  if (usages.length > 0) {
    return { patches: [], diagnostics: [error(`Alias is referenced by ${usages.length} usage${usages.length === 1 ? '' : 's'}.`, assetAliasesPath(payload.assetId))] };
  }
  const path = assetAliasesPath(payload.assetId);
  return { patches: [{ op: 'replace', path, value: toJsonValue(data.aliases.filter((alias) => alias !== payload.alias)) }], affectedPaths: [path] };
}

export function renameAssetAliasPatches(document: JsonValue | unknown, payload: AssetRenameAliasPayload): AssetOperationResult {
  const project = validateProject(document);
  if (!isAuthoringProject(project)) return { patches: [], diagnostics: [project] };
  const aliasError = validateAssetAlias(payload.toAlias);
  if (aliasError) return { patches: [], diagnostics: [error(aliasError)] };
  if (payload.fromAlias === payload.toAlias) return { patches: [], diagnostics: [error('New alias must be different.')] };
  ...
  if (conflict) return { patches: [], diagnostics: [error(`Alias is already assigned to assets/${conflict}.`)] };
  const record = project.assets[owner.entityId]!;
  const data = parseAssetData(record.data)!;
  const patches: JsonPatchOperation[] = [{
    op: 'replace',
    path: assetAliasesPath(owner.entityId),
    value: toJsonValue(data.aliases.map((alias) => alias === payload.fromAlias ? payload.toAlias : alias)),
  }];
...
export function reimportAssetPatches(document: JsonValue | unknown, payload: AssetReimportPayload): AssetOperationResult {
  const project = validateProject(document);
  if (!isAuthoringProject(project)) return { patches: [], diagnostics: [project] };
  const record = project.assets[payload.assetId];
  if (!record) return { patches: [], diagnostics: [error('Asset does not exist.', assetPath(payload.assetId))] };
  const current = parseAssetData(record.data);
  if (!current) return { patches: [], diagnostics: [error('Asset record has invalid asset data.', assetDataPath(payload.assetId))] };
  const next = { ...assetDataFromImportMetadata(payload.asset), aliases: current.aliases };
  return { patches: [{ op: 'replace', path: assetDataPath(payload.assetId), value: toJsonValue(next) }], affectedPaths: [assetDataPath(payload.assetId)] };
}

export function deleteAssetPatches(document: JsonValue | unknown, payload: AssetDeletePayload): AssetOperationResult {
  const project = validateProject(document);
  if (!isAuthoringProject(project)) return { patches: [], diagnostics: [project] };
  const record = project.assets[payload.assetId];
  if (!record) return { patches: [], diagnostics: [error('Asset does not exist.', assetPath(payload.assetId))] };
  const stableUsages = findUsages(buildReferenceIndex(project), { collection: 'assets', id: payload.assetId }).filter(
    (usage) => !(usage.sourceCollection === 'assets' && usage.sourceId === payload.assetId),
  );
  const data = parseAssetData(record.data);
  const aliasUsages = data ? data.aliases.flatMap((alias) => findAssetAliasUsages(buildAssetAliasIndex(project), alias)) : [];
  const usageCount = stableUsages.length + aliasUsages.length;
  if (!payload.force && usageCount > 0) {
    return { patches: [], diagnostics: [error(`Asset is referenced by ${usageCount} usage${usageCount === 1 ? '' : 's'}.`, assetPath(payload.assetId))] };
  }
  return { patches: [{ op: 'remove', path: assetPath(payload.assetId) }], affectedPaths: [assetPath(payload.assetId)] };
}
