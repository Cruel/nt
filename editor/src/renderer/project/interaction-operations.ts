...
import { parseInteractionData, validateInteractionData } from '../../shared/project-schema/authoring-interactions';
...
export function replaceInteractionDataPatches(document: JsonValue | unknown, payload: { interactionId: string; data: unknown }) {
  ...
  const failure = validateInteractionData(document, payload.interactionId, { ...record, data }).find((item) => item.severity === 'error');
