...
export function replaceVerbDataPatches(document: JsonValue | unknown, payload: { verbId: string; data: unknown }) {
  ...
  const record = document.verbs[payload.verbId]; const data = parseVerbData(payload.data);
  if (!record) return { patches: [], diagnostics: [{ severity: 'error' as const, message: 'Verb record does not exist.', path: buildJsonPointer(['verbs', payload.verbId]) }] };
  if (!data) return { patches: [], diagnostics: [{ severity: 'error' as const, message: 'Verb data is invalid.', path }] };
  const failure = validateInteractionProgram(document, data.defaultProgram, `${path}/defaultProgram`).find((item) => item.severity === 'error');
