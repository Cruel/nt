...
export function replaceShaderDataPatches(
  ...
  if (!project.shaders[payload.shaderId])
    return {
      patches: [],
      diagnostics: [
        error('Shader record does not exist.', buildJsonPointer(['shaders', payload.shaderId])),
      ],
    };
  ...
  if (!parsed)
    return {
      patches: [],
      diagnostics: [error('Shader data is invalid.', shaderDataPath(payload.shaderId))],
    };
  return {
    patches: [
      { op: 'replace', path: shaderDataPath(payload.shaderId), value: toJsonValue(parsed) },
    ],
    affectedPaths: [shaderDataPath(payload.shaderId)],
  };
}
...
export function setMaterialBasePatches(
  ...
  if (payload.baseMaterialId === payload.materialId)
    return {
      patches: [],
      diagnostics: [
        error(
          'Material cannot inherit from itself.',
          buildJsonPointer(['materials', payload.materialId, 'data', 'baseMaterialId']),
        ),
      ],
    };
  if (payload.baseMaterialId && !project.materials[payload.baseMaterialId])
    return {
      patches: [],
      diagnostics: [
        error(
          'Base material does not exist.',
          buildJsonPointer(['materials', payload.baseMaterialId]),
        ),
      ],
    };
  ...
  return {
    patches: [{ op: 'replace', path, value: payload.baseMaterialId }],
    affectedPaths: [path],
  };
}
...
export function applyShaderCompiledOutputsPatches(
  ...
  for (const output of payload.outputs) {
    ...
    if (!shader) {
      diagnostics.push(
        error(
          `Shader '${output.shader}' is missing or invalid.`,
          buildJsonPointer(['shaders', output.shader, 'data']),
        ),
      );
      continue;
    }
    ...
    if (stageIndex < 0) {
      diagnostics.push(
        error(
          `Shader '${output.shader}' has no '${output.stage}' stage.`,
          buildJsonPointer(['shaders', output.shader, 'data', 'stages']),
        ),
      );
      continue;
    }
    ...
    patches.push(
      Object.prototype.hasOwnProperty.call(stage, 'compiled')
        ? { op: 'replace', path, value: toJsonValue(next) }
        : { op: 'add', path, value: toJsonValue(next) },
    );
