...
export interface SetChapterColorPayload {
  chapterId: string;
  color: string | null;
}
export interface AssignChaptersPayload {
  collection: AuthoringCollectionKey;
  entityId: string;
  chapterIds: string[];
}
export interface SetHiddenCollectionsPayload {
  hiddenCollectionKeys: AuthoringCollectionKey[];
}
export interface SetExplorerOptionsPayload {
...
function ensureEditorPatches(documentValue: JsonValue) {
  ...
  if (!hasJsonAtPointer(documentValue, '/editor'))
    patches.push({ op: 'add', path: '/editor', value: {} });
  if (!hasJsonAtPointer(documentValue, '/editor/chapters'))
    patches.push({ op: 'add', path: '/editor/chapters', value: { records: {}, assignments: {} } });
  if (!hasJsonAtPointer(documentValue, '/editor/chapters/records'))
    patches.push({ op: 'add', path: '/editor/chapters/records', value: {} });
  if (!hasJsonAtPointer(documentValue, '/editor/chapters/assignments'))
    patches.push({ op: 'add', path: '/editor/chapters/assignments', value: {} });
  if (!hasJsonAtPointer(documentValue, '/editor/explorer'))
    patches.push({ op: 'add', path: '/editor/explorer', value: {} });
  return patches;
}
...
function chapterRecords(document: unknown): Record<string, { id: string; label: string }> {
  ...
  const editor = (document as { editor?: unknown }).editor as
    | { chapters?: { records?: Record<string, { id: string; label: string }> } }
    | undefined;
...
function chapterAssignments(document: unknown): Record<string, string[]> {
  if (typeof document !== 'object' || document === null) return {};
  const editor = (document as { editor?: unknown }).editor as
    | { chapters?: { assignments?: Record<string, string[]> } }
    | undefined;
  return editor?.chapters?.assignments ?? {};
}

function recordKey(collection: string, entityId: string) {
...
export function createChapterPatches(
  document: unknown,
  ...
  if (!chapterId)
    return {
      patches: [],
      diagnostics: [error('Chapter ID is required.', '/editor/chapters/records')],
    };
  if (!payload.label.trim())
    return {
      patches: [],
      diagnostics: [error('Chapter label is required.', `/editor/chapters/records/${chapterId}`)],
    };
  ...
  if (hasJsonAtPointer(documentValue, path))
    return { patches: [], diagnostics: [error('A chapter with this ID already exists.', path)] };
  patches.push({
    op: 'add',
    path,
    value: toJsonValue({
...
export function renameChapterPatches(
  document: unknown,
  payload: RenameChapterPayload,
): EntityOperationResult {
  if (!payload.label.trim())
    return { patches: [], diagnostics: [error('Chapter label is required.')] };
  const documentValue = toJsonValue(document);
  const path = buildJsonPointer(['editor', 'chapters', 'records', payload.chapterId, 'label']);
  if (!hasJsonAtPointer(documentValue, path))
    return { patches: [], diagnostics: [error('Chapter does not exist.', path)] };
  return { patches: [{ op: 'replace', path, value: payload.label.trim() }], affectedPaths: [path] };
}

export function setChapterColorPatches(
  document: unknown,
  payload: SetChapterColorPayload,
): EntityOperationResult {
  const documentValue = toJsonValue(document);
  ...
  if (!hasJsonAtPointer(documentValue, basePath))
    return { patches: [], diagnostics: [error('Chapter does not exist.', basePath)] };
  return { patches: [addOrReplace(documentValue, path, payload.color)], affectedPaths: [path] };
}

export function deleteChapterPatches(
  document: unknown,
  payload: DeleteChapterPayload,
): EntityOperationResult {
  const documentValue = toJsonValue(document);
  const recordPath = buildJsonPointer(['editor', 'chapters', 'records', payload.chapterId]);
  if (!hasJsonAtPointer(documentValue, recordPath))
    return { patches: [], diagnostics: [error('Chapter does not exist.', recordPath)] };
...
export function assignChaptersPatches(
  document: unknown,
  payload: AssignChaptersPayload,
): EntityOperationResult {
  if (!isAuthoringProject(document))
    return { patches: [], diagnostics: [error('Current document is not a NovelTea project.')] };
  if (collectiveCollectionSet.has(payload.collection))
    return {
      patches: [],
      diagnostics: [error('Collective categories are not assignable to chapters in V1.')],
    };
  if (!document[payload.collection][payload.entityId])
    return {
      patches: [],
      diagnostics: [
        error('Record does not exist.', buildJsonPointer([payload.collection, payload.entityId])),
      ],
    };
  const documentValue = toJsonValue(document);
  const patches = ensureEditorPatches(documentValue);
...
export function setHiddenCollectionsPatches(
  document: unknown,
  payload: SetHiddenCollectionsPayload,
): EntityOperationResult {
  const documentValue = toJsonValue(document);
  const patches = ensureEditorPatches(documentValue);
  const hidden = [...new Set(payload.hiddenCollectionKeys.filter(isAuthoringCollectionKey))].sort(
...
export function setExplorerOptionsPatches(
  document: unknown,
  payload: SetExplorerOptionsPayload,
): EntityOperationResult {
  const documentValue = toJsonValue(document);
  const patches = ensureEditorPatches(documentValue);
  if (payload.followActiveTab !== undefined)
    patches.push(
      addOrReplace(documentValue, '/editor/explorer/followActiveTab', payload.followActiveTab),
    );
  if (payload.organizeByChapter !== undefined)
    patches.push(
      addOrReplace(documentValue, '/editor/explorer/organizeByChapter', payload.organizeByChapter),
    );
  if (payload.groupUnassignedItems !== undefined)
    patches.push(
      addOrReplace(
        documentValue,
        '/editor/explorer/groupUnassignedItems',
        payload.groupUnassignedItems,
      ),
    );
  if (payload.hideEmptyCategories !== undefined)
    patches.push(
      addOrReplace(
        documentValue,
        '/editor/explorer/hideEmptyCategories',
        payload.hideEmptyCategories,
      ),
    );
  if (payload.showInfoOnHover !== undefined)
    patches.push(
      addOrReplace(documentValue, '/editor/explorer/showInfoOnHover', payload.showInfoOnHover),
    );
  return { patches, affectedPaths: patches.map((patch) => patch.path) };
}
