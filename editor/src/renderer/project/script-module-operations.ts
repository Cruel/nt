...
import { parseScriptModuleData, validateScriptModuleData } from '../../shared/project-schema/authoring-script-modules';
...
export function replaceScriptModuleDataPatches(document: JsonValue | unknown, payload: { scriptId: string; data: unknown }) {
  ...
  const failure = validateScriptModuleData(document, payload.scriptId, { ...record, data }).find((item) => item.severity === 'error');
