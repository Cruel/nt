    ...
    <section className="min-h-0 overflow-auto border-r p-4">
      ...
      {!selected ? null : <div className="space-y-4">
        ...
        {selected.type === 'set-background' && <>
          ...
          <Label>Transition<Select value={selected.transition} onValueChange={(transition) => replaceStep({ ...selected, transition: transition as typeof selected.transition })}>{['none','fade','cut'].map((value) => <SelectItem key={value} value={value}>{title(value)}</SelectItem>)}</Select></Label>
        </>}
        {selected.type === 'actor-cue' && <>
          ...
          <Label>Transition<Select value={selected.transition} onValueChange={(transition) => replaceStep({ ...selected, transition: transition as typeof selected.transition })}>{['none','fade','slide'].map((value) => <SelectItem key={value} value={value}>{title(value)}</SelectItem>)}</Select></Label>
        </>}
        ...
        {selected.type === 'set-layout' && <><Label>Layout<Select value={refId(selected.layout)} onValueChange={(id) => { if (id) replaceStep({ ...selected, layout: id === '__none__' ? null : sceneLayoutRef(id) }); }}><SelectItem value="__none__">None</SelectItem>{Object.entries(project.layouts).map(([id, item]) => <SelectItem key={id} value={id}>{item.label}</SelectItem>)}</Select></Label><Label>Action<Select value={selected.action} onValueChange={(action) => replaceStep({ ...selected, action: action as typeof selected.action })}>{['show','hide','swap'].map((value) => <SelectItem key={value} value={value}>{title(value)}</SelectItem>)}</Select></Label><Label>Slot<Select value={selected.slot} onValueChange={(slot) => replaceStep({ ...selected, slot: slot as typeof selected.slot })}>{['hud','dialogue-box','overlay','custom'].map((value) => <SelectItem key={value} value={value}>{title(value)}</SelectItem>)}</Select></Label></>}
        {selected.type === 'transition' && <><Label>Transition<Select value={selected.transitionKind} onValueChange={(transitionKind) => replaceStep({ ...selected, transitionKind: transitionKind as typeof selected.transitionKind })}>{['fade','cut','dissolve'].map((value) => <SelectItem key={value} value={value}>{title(value)}</SelectItem>)}</Select></Label><Label>Duration (ms)<Input type="number" min="0" value={selected.durationMs} onChange={(event) => replaceStep({ ...selected, durationMs: Number(event.target.value) })} /></Label><Label>Color<Input value={selected.color ?? ''} onChange={(event) => replaceStep({ ...selected, color: event.target.value || null })} /></Label><Label className="flex items-center gap-2">Wait for completion<Switch checked={selected.waitForCompletion} onCheckedChange={(waitForCompletion) => replaceStep({ ...selected, waitForCompletion })} /></Label></>}
