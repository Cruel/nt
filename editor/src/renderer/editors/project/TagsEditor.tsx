...
import { authoringCollectionMetadata, authoringCollectionKeys } from '../../../shared/project-schema/authoring-collections';
import { isAuthoringProject } from '../../../shared/project-schema/authoring-project';
import { collectProjectTags, TAG_COLOR_POOL, type ProjectTagSummary, type TagColor } from '../../../shared/project-schema/authoring-tags';
...
export function TagsEditor(_props: WorkbenchEditorProps) {
  ...
  const tags = project ? collectProjectTags(project).sort((left, right) => left.name.localeCompare(right.name)) : [];
  const registryKeys = new Set(Object.keys(project?.editor.tags?.records ?? {}));

  if (!project) return <div className="p-4 text-sm text-muted-foreground">Open an authoring project to manage tags.</div>;

  function setColor(tag: ProjectTagSummary, color: TagColor) {
    executeCommand({ type: 'project.setTagColor', label: `Set tag color ${tag.name}`, payload: { tag: tag.name, color } });
  }
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-auto bg-background p-4">
      <div className="mb-4 min-w-0">
        ...
        <p className="text-xs text-muted-foreground">Review tag usage, change tag colors, and remove unused registry entries.</p>
      </div>
      <div className="rounded border">
        ...
        <div className="max-h-[70vh] overflow-auto">
          {tags.length === 0 ? <div className="p-4 text-sm text-muted-foreground">No tags exist yet.</div> : tags.map((tag) => {
            ...
            return (
              <div key={tag.key} className="grid grid-cols-[minmax(12rem,1fr)_6rem_minmax(12rem,1fr)_9rem_6rem] items-center gap-2 border-b px-3 py-2 text-sm last:border-b-0">
                ...
                <Select value={tag.color} onValueChange={(value) => setColor(tag, value as TagColor)}>
                  <SelectTrigger className="w-32"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {TAG_COLOR_POOL.map((color) => <SelectItem key={color} value={color}>{color.replace('tag-', '')}</SelectItem>)}
                  </SelectContent>
                </Select>
                <Button size="sm" variant="destructive" disabled={!unusedRegistryEntry} onClick={() => deleteUnusedTag(tag)} title={unusedRegistryEntry ? 'Delete unused registry tag' : 'Only unused registry tags can be deleted here'}>
