...
export function ChaptersEditor({ tab }: WorkbenchEditorProps) {
  ...
  const editorState = useMemo(() => project ? editorProjectStateFromProject(project) : null, [project]);
  ...
  const targetRecord = project && collection && entityId ? project[collection][entityId] ?? null : null;
  ...
  if (!project) return <div className="p-4 text-sm text-muted-foreground">Open an authoring project to manage chapters.</div>;
  ...
  function renameChapter(id: string, nextLabel: string) {
    if (nextLabel.trim()) run({ type: 'project.renameChapter', label: `Rename chapter ${id}`, payload: { chapterId: id, label: nextLabel } });
  }
  ...
  function applyAssignment() {
    ...
    run({ type: 'project.assignChapters', label: `Assign chapters to ${collection}/${entityId}`, payload: { collection, entityId, chapterIds: [...selected] } });
  }
    ...
    <div className="flex h-full min-h-0 flex-col overflow-auto bg-background p-4">
      <div className="mb-4 flex min-w-0 items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted-foreground">Create, rename, color, delete, and assign editor-only chapters.</p>
        </div>
      </div>
      {message ? <div className="mb-4 rounded border border-destructive/40 bg-destructive/10 p-2 text-xs text-destructive">{message}</div> : null}
      {targetRecord && collection && entityId ? (
        <section className="mb-4 rounded border p-3">
          <div className="mb-2 text-sm font-medium">Assign chapters to {targetRecord.label || entityId}</div>
          <div className="mb-3 text-xs text-muted-foreground">{authoringCollectionMetadata[collection].singularLabel} · <span className="font-mono">{collection}/{entityId}</span></div>
          ...
          <div className="mt-3 flex justify-end"><Button size="sm" onClick={applyAssignment}>Apply Assignment</Button></div>
        </section>
      ) : null}
      <section className="rounded border p-3">
        ...
        <div className="mt-4 max-h-[52vh] space-y-2 overflow-auto">
          {chapterEntries.length === 0 ? <p className="text-sm text-muted-foreground">No chapters exist yet.</p> : chapterEntries.map(([id, chapter]) => (
            <div key={id} className="grid items-center gap-2 rounded border p-2 md:grid-cols-[1fr_8rem_auto]">
              <div className="min-w-0">
