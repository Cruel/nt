...
import type { PackageExportOptions, PackageExportResponse, ShaderCompileDiagnostic, ToolDiagnostic } from '../../shared/editor-tooling';
...
function defaultShaderCompileOptions(projectRoot: string | null, outputRoot: string | null, shaderVariants: string[]) {
...
function packageOptionsWithShaderRoot(options: PackageExportOptions, projectRoot: string | null): PackageExportOptions {
  if (options.shaderVariants && options.shaderVariants.length > 0 && !options.shaderAssetRoot && projectRoot) {
...
function normalizePackageResponse(value: unknown): PackageExportResponse {
  ...
  return {
    ...
    checksums: record.checksums && typeof record.checksums === 'object' ? record.checksums as Record<string, string> : undefined,
    error: typeof record.error === 'string' ? record.error : undefined,
  };
}

export async function runPackageExportWorkflow(options: RunPackageExportWorkflowOptions): Promise<PackageExportWorkflowResult> {
  ...
  if (!authoringValidationSucceeded(validationDiagnostics)) {
    const result = failureResult('failed', options, validationDiagnostics, { validationDiagnostics });
    exportStore.finish(result);
    workspace.setLastExportResult(result);
    workspace.addTimelineEntry({ source: 'export', message: 'Export blocked by validation errors', detail: result });
    workspace.setStatusMessage(`Export blocked by ${validationDiagnostics.filter((diagnostic) => diagnostic.severity === 'error').length} validation error(s)`);
  ...
  if (!built.ok || !built.runtimeProject) {
    ...
    exportStore.finish(result);
    workspace.setLastExportResult(result);
    workspace.addTimelineEntry({ source: 'export', message: 'Export blocked by runtime conversion diagnostics', detail: result });
  ...
  if (options.profile.compileShadersBeforeExport && hasAuthoringShadersOrMaterials(options.project)) {
    ...
    const response = await useShaderCompileStore.getState().runCompile(shaderProject.project, defaultShaderCompileOptions(options.projectRoot, options.projectRoot ? `${options.projectRoot}/.noveltea/build` : null, options.profile.shaderVariants));
    ...
    if (!response.success || hasErrors(response.diagnostics)) {
      ...
      exportStore.finish(result);
      workspace.setLastExportResult(result);
      workspace.addTimelineEntry({ source: 'export', message: 'Export blocked by shader compile errors', detail: result });
    ...
    if (response.outputs.length > 0) {
      ...
      if (!command.ok) {
        const diagnostics: ToolDiagnostic[] = command.diagnostics.length > 0
          ? command.diagnostics.map((diagnostic) => ({ severity: diagnostic.severity, path: diagnostic.path ?? '/', message: diagnostic.message, category: diagnostic.commandType ?? 'shader' }))
          : [{ severity: 'error' as const, path: '/shaders', message: 'Failed to apply shader compile outputs.', category: 'shader' }];
        const result = failureResult('failed', options, diagnostics, { validationDiagnostics, shaderDiagnostics, shaderOutputs });
        exportStore.finish(result);
        workspace.setLastExportResult(result);
  ...
  exportStore.finish(result);
  workspace.setLastExportResult(result);
  workspace.addTimelineEntry({ source: 'export', message: success ? `Exported package ${options.outputPath}` : response.error ?? 'Package export failed', detail: result });
  workspace.setStatusMessage(success ? `Exported ${response.byteCount ?? 0} bytes to ${options.outputPath}` : response.error ?? 'Package export failed');
  useBottomPanelStore.getState().setActivePanelId('package-export');
  return result;
}
