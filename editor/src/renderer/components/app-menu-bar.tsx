...
import { Maximize2, Minus, Play, Redo2, Save, Square, Undo2, X } from 'lucide-react';
...
function WorkspaceTopToolbar() {
  ...
  const canRedo =
    !!project &&
    commandHistory.cursor < commandHistory.entries.length - 1 &&
    !commandHistory.activeTransaction;

  return (
    <div className="flex min-w-0 items-center gap-1" style={noDragStyle}>
      <span
        className="mr-2 max-w-64 truncate font-mono text-[11px] text-muted-foreground"
        title="NovelTea"
      >
        NovelTea
      </span>
      {project ? (
        <>
          <Button
            size="icon-xs"
            variant="ghost"
            onClick={() => dispatchWorkspaceToolbarCommand('play-game')}
            title={t('actions.playDebugGame')}
          >
          ...
          <Button
            size="icon-xs"
            variant="ghost"
            onClick={() => dispatchWorkspaceToolbarCommand('undo')}
            disabled={!canUndo}
          ...
          <Button
            size="icon-xs"
            variant="ghost"
            onClick={() => dispatchWorkspaceToolbarCommand('redo')}
            disabled={!canRedo}
          ...
          <Button
            size="icon-xs"
            variant={saveDirty ? 'secondary' : 'ghost'}
            onClick={() => dispatchWorkspaceToolbarCommand('save')}
            disabled={!saveDirty || isSaving}
        ...
        >
          <MenubarMenu>
            ...
            <MenubarContent className="min-w-56">
              <MenubarItem
                onClick={() =>
                  dispatchWorkspaceToolbarCommand(project ? 'new-entity' : 'new-project')
                }
              >
                {project ? t('menu:items.newEntity') : t('menu:items.newProject')}
                <MenubarShortcut>Ctrl+N</MenubarShortcut>
              </MenubarItem>
              <MenubarItem onClick={() => dispatchWorkspaceToolbarCommand('open-project')}>
                {t('menu:items.openProject')}
                    ...
                    recentProjects.map((entry) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <MenubarShortcut
                            className="max-w-36 truncate normal-case tracking-normal"
                            title={projectKey}
                          >
                            {projectKey}
                          </MenubarShortcut>
              ...
              <MenubarItem
                disabled={!project}
                onClick={() => dispatchWorkspaceToolbarCommand('close-project')}
              >
                {t('menu:items.closeProject')}
              </MenubarItem>
              <MenubarSeparator />
              <MenubarItem
              ...
              <MenubarSeparator />
              <MenubarItem onClick={() => dispatchWorkspaceToolbarCommand('export-package')}>
                {t('menu:items.packageExport')}
              </MenubarItem>
            </MenubarContent>
          </MenubarMenu>
          ...
          <MenubarMenu>
            ...
            <MenubarContent className="min-w-52">
              ...
              >
                {t('menu:items.projectSettings')}
              </MenubarItem>
              <MenubarSeparator />
              <MenubarItem
                disabled={!project}
                onClick={() => dispatchWorkspaceToolbarCommand('validate')}
              >
                {t('menu:items.validateProject')}
              </MenubarItem>
              <MenubarItem
                disabled={!project}
                onClick={() => dispatchWorkspaceToolbarCommand('export-package')}
              >
                {t('menu:items.packageExport')}
              </MenubarItem>
            </MenubarContent>
          ...
          <MenubarMenu>
            ...
            <MenubarContent className="min-w-52">
              <MenubarItem onClick={() => dispatchWorkspaceToolbarCommand('undo')}>
                {t('common:actions.undo')}
                <MenubarShortcut>Ctrl+Z</MenubarShortcut>
              </MenubarItem>
              <MenubarItem onClick={() => dispatchWorkspaceToolbarCommand('redo')}>
          ...
          <MenubarMenu>
            ...
            <MenubarContent className="min-w-52">
              ...
              <MenubarItem onClick={() => dispatchWorkspaceToolbarCommand('toggle-bottom-panel')}>
                {t('menu:items.toggleBottomPanel')}
