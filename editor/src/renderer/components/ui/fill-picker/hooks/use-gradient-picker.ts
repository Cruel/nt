...
export function useGradientPicker(props: UseGradientPickerProps = {}): GradientPickerState {
  ...
  // controlled syncs.
  const idTrackedRef = React.useRef(hasStopIds(value ?? defaultValue ?? DEFAULT_LINEAR));
  ...
  if (isControlled && value !== prevControlledValue) {
    ...
    if (value !== lastEmittedRef.current) {
      ...
      const sameShape = uniqueIds
        ? byId
        : sameLength && prev.stops.every((s, i) => s.position === incoming[i].position);
  ...
  const apply = React.useCallback((compute: (prev: InternalState) => InternalState | null) => {
    const prev = stateRef.current;
