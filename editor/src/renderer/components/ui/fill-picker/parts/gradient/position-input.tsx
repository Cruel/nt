...
import { FieldDivider, FieldInput, FieldInputGroup, FieldShell, FieldSuffix } from '../field';

export const PositionInput = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  function PositionInput({ className, ...rest }, ref) {
    ...
    const commit = (axis: 'x' | 'y', raw: string) => {
      ...
      ctx.setCenter(axis === 'x' ? { x: clamp(n), y: center.y } : { x: center.x, y: clamp(n) });
    };
