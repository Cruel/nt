...
import { projectStopPosition, reverseProjectStopPosition } from '../../lib/gradient';
import {
  focusNeighborOption,
...
import { FieldInput, FieldInputGroup, FieldShell, FieldSuffix } from '../field';
...
export const StopList = React.forwardRef<HTMLDivElement, StopListProps>(function StopList(
  ...
  const handleAddStop = () => {
    const { position, color } = insertStopAfterSelected(ctx.stops, ctx.selectedStopId);
          ...
          <FieldInput
            ...
            onChange={(e) => {
              ...
              if (Number.isFinite(v))
                ctx.moveStop(s.id, fromDisplay(Math.max(0, Math.min(1, v / 100))));
            }}
