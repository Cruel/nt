 ...
 */
export const Overlay = React.forwardRef<HTMLDivElement, OverlayProps>(function Overlay(
  { className, conicDialRadius, style, ...rest },
  ref,
) {
  ...
  const handles = React.useMemo(() => {
    ...
    if (gradient.type === 'linear') {
      ...
      const inset = HANDLE_PX / 2;
      const t = edgeExtent(dir, Math.max(0, w / 2 - inset), Math.max(0, h / 2 - inset));
      return {
    ...
    if (gradient.type === 'radial') {
      ...
      if (gradient.shape === 'circle' && gradient.radiusPx !== undefined) {
        ...
        const seeded = keywordToRadii(gradient.shape, gradient.size, gradient.center, w, h);
        bx = ax + seeded.x * w;
        by = ay + seeded.y * h;
      }
      return {
  ...
  type HandleKind = 'linear-a' | 'linear-b' | 'center' | 'conic-dial' | 'radial-edge';
  ...
  const onKeyDownRadii = (e: React.KeyboardEvent<HTMLButtonElement>) => {
    ...
    if (renderAsCircle) {
      ...
      const currentPx =
        ...
        (gradient.radii
          ...
          : keywordToRadii(gradient.shape, gradient.size, gradient.center, w, h).x * w);
    ...
    const current =
      gradient.radii ?? keywordToRadii(gradient.shape, gradient.size, gradient.center, w, h);
    let { x, y } = current;
  ...
  const onKeyDownCenter = (e: React.KeyboardEvent<HTMLButtonElement>) => {
    ...
    ctx.setCenter(next);
    announce(`Center x ${Math.round(next.x * 100)}%, y ${Math.round(next.y * 100)}%`);
  };

  const beginDrag = (kind: HandleKind) => (e: React.PointerEvent<HTMLButtonElement>) => {
    e.preventDefault();
    e.stopPropagation();
          ...
          {gradient.type === 'linear' ? (
            <>
              ...
              <Handle
                ...
                style={ctx.stops[0] ? stopSwatchStyle(ctx.stops[0].color) : undefined}
              />
              {handles.b && (
                <Handle
                  label={
            ...
            <>
              ...
              {handles.b && gradient.type === 'radial' && (
                <Handle
                  ...
                  aria-valuemax={200}
                  aria-valuenow={
                    gradient.shape === 'circle' && gradient.radiusPx !== undefined
...
interface HandleProps extends Omit<React.ButtonHTMLAttributes<HTMLButtonElement>, 'aria-label'> {
  label: string;
  position: XY;
}
