...
export const ContrastReadout = React.forwardRef<HTMLDivElement, ContrastReadoutProps>(
  function ContrastReadout(
    ...
    const initial = defaultMetric && metrics.includes(defaultMetric) ? defaultMetric : metrics[0];
    ...
    const popover =
      active === 'wcag'
        ? wcagPopover(contrast.wcag, contrast.wcagLevel.aaNormal, contrast.wcagLevel.aaaNormal)
      ...
      <TooltipProvider delay={150}>
        <Tooltip>
          ...
          <TooltipContent
            ...
            className="max-w-[260px] bg-popover p-2.5 text-popover-foreground shadow-md"
          >
            <PopoverPanel title={popover.title} rows={popover.rows} fg={fgCss} bg={bgCss} />
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    );
    ...
    <>
      {(showLabel || showValue) && (
        <div className="flex items-center gap-1.5">
          ...
          {showValue && <span className="font-mono font-medium">{wcag.toFixed(2)}:1</span>}
        </div>
      )}
      {showBadges && (
    ...
    <>
      {(showLabel || showValue) && (
        <div className="flex items-center gap-1.5">
          ...
          {showValue && <span className="font-mono font-medium">Lc {lc.toFixed(1)}</span>}
        </div>
      )}
      {showBadges && (
        <div className="flex items-center gap-1">
...
function Badge({ ok, children }: { ok: boolean; children: React.ReactNode }) {
  return (
    <span
      aria-label={
        typeof children === 'string' ? `${children} ${ok ? 'passes' : 'fails'}` : undefined
      }
      className={cn(
        ...
        {rows.map((r) => (
          <li key={r.label} className="flex items-start gap-2">
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <span className="text-[11px] leading-snug text-muted-foreground">{r.detail}</span>
            </div>
          </li>
        ))}
      </ul>
...
function wcagPopover(ratio: number, aa: boolean, aaa: boolean): { title: string; rows: PassRow[] } {
  return {
