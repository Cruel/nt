...
export function useEnginePreviewStatusBridge({
  ...
  const recordTransportError = useCallback((message: string, setConnectionState: (next: PreviewConnectionState) => void) => {
  ...
  const handlePreviewMessage = useCallback((message: PreviewToEditorMessage, options: {
  ...
  }, [embedded, onPreviewMessage, recordPreviewDiagnostic, sessionId, setLastPreviewEvent, setSelectedRuntimeObjectId, setSessionCapabilities, setSessionStatus, setStatusMessage]);
