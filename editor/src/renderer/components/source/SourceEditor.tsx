...
const rmlExtraTags = Object.fromEntries(
  rmlElementCompletions.map((completion) => [completion.label, {} satisfies TagSpec]),
);
const rmlExtraAttributes = Object.fromEntries(
  rmlAttributeCompletions.map((completion) => [completion.label, null]),
);
  ...
  if (language === 'rml')
    return [
      html({
        ...
        extraTags: rmlExtraTags,
        extraGlobalAttributes: rmlExtraAttributes,
      }),
      completionData,
    ];
...
export const SourceEditor = forwardRef<SourceEditorHandle, SourceEditorProps>(function SourceEditor(
    ...
    () => ({
      ...
      restoreViewState: (state) => {
        ...
        window.requestAnimationFrame(() => {
          if (state.scroll) {
            view.scrollDOM.scrollTop = Number.isFinite(state.scroll.scrollTop)
              ? state.scroll.scrollTop
              : 0;
            view.scrollDOM.scrollLeft = Number.isFinite(state.scroll.scrollLeft)
      ...
      revealLine: (lineNumber, columnNumber = 1) => {
        ...
        const line = view.state.doc.line(
          Math.min(Math.max(1, Math.trunc(lineNumber)), view.state.doc.lines),
        );
  ...
  useEffect(() => {
    ...
    const state = EditorState.create({
      ...
      extensions: [
        basicSetup,
        linter((view) =>
          diagnosticsRef.current.map((item) => toCodemirrorDiagnostic(item, view.state.doc.length)),
        ),
  ...
  return (
    <div
      ref={hostRef}
      className={cn('min-h-48 overflow-hidden rounded border bg-background', className)}
    />
  );
});
