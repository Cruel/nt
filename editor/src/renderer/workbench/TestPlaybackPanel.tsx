...
function playbackDiagnosticItems(
  diagnostics: unknown[],
  ...
  return (
    <div className="space-y-3 p-3 text-xs">
      <div className="flex flex-wrap items-center gap-2">
        <Badge variant={passed === false ? 'destructive' : 'default'}>
      ...
      {failures.length > 0 ? (
        <section className="space-y-2 rounded border p-3">
          ...
          {failures.map((failure, index) => (
            <div key={index} className="rounded bg-destructive/10 p-2 text-destructive">
              {String(failure)}
            </div>
          ))}
        </section>
              ...
              {assertionFailures.length > 0 ? (
                <div className="mt-2 space-y-1">
                  {assertionFailures.map((failure, failureIndex) => (
                    <div
                      key={failureIndex}
                      className="rounded bg-destructive/10 p-1 text-destructive"
                    >
                      {String(failure)}
                    </div>
                  ))}
