...
describe('SourceEditor language completion metadata', () => {
  it('includes RmlUi RML elements and data binding attributes', () => {
    expect(rmlElementCompletions.map((completion) => completion.label)).toEqual(expect.arrayContaining(['rml', 'body', 'button']));
    expect(rmlAttributeCompletions.map((completion) => completion.label)).toEqual(expect.arrayContaining(['data-model', 'data-for', 'data-if', 'data-event-*']));
  });

  it('includes RmlUi RCSS additions', () => {
    expect(rcssCompletions.map((completion) => completion.label)).toEqual(expect.arrayContaining(['decorator', 'decorator: shader()', 'filter: blur()']));
  });
  ...
  it('includes bgfx shaderc tokens, helpers, uniforms, and varying attributes', () => {
    expect(shaderCompletions.map((completion) => completion.label)).toEqual(expect.arrayContaining([
      '$input',
      '$output',
...
describe('SourceEditor completion sources', () => {
  ...
  it('returns RCSS completions for RCSS documents', () => {
    expect(completeLabels('rcss', 'decor')).toEqual(expect.arrayContaining(['decorator', 'decorator: shader()']));
  });

  it('returns Lua runtime completions for Lua documents', () => {
    expect(completeLabels('lua', 'nov')).toEqual(expect.arrayContaining(['noveltea', 'noveltea.lua_version']));
  });

  it('returns shader and project shader interface completions for shader documents', () => {
    expect(completeLabels('shader', '$')).toEqual(expect.arrayContaining(['$input', '$output', 'SAMPLER2D', 'u_time', 's_diffuse']));
  });
