...
function archiveFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'nt-template-')); roots.push(root); const content = path.join(root, 'content'); fs.mkdirSync(path.join(content, 'bin'), { recursive: true }); fs.writeFileSync(path.join(content, 'bin/player'), 'player', { mode: 0o755 }); const player = fs.readFileSync(path.join(content, 'bin/player')); const descriptor = { format: 'noveltea.player-template', formatVersion: 1, templateId: 'linux-x64-release', buildId: 'build-1', engineVersion: '1', platform: 'linux', architecture: 'x64', minimumPlatformVersion: 'glibc 2.35', graphicsBackends: ['opengl'], shaderVariants: ['glsl-120'], runtimePackageApi: { minimum: 1, maximum: 1 }, playerConfigApi: { minimum: 1, maximum: 1 }, compiledFeatures: ['lua'], capabilities: [], buildFlavor: 'release', packageAccessModes: ['sidecar'], files: [{ path: 'bin/player', size: player.length, mode: fs.statSync(path.join(content, 'bin/player')).mode & 0o777, sha256: hash(player) }], runtimeDependencies: [{ path: 'bin/player', kind: 'library' }], artifacts: { archive: 'template.tar.gz', symbols: 'symbols.tar.gz', sbom: 'SBOM.cdx.json', notices: 'NOTICE.txt' }, provenance: { provider: 'local', source: 'test' }, host: { assembly: 'any', requiresToolchain: false, tools: [] } }; fs.writeFileSync(path.join(content, 'template.json'), JSON.stringify(descriptor)); const archive = path.join(root, 'template.tar.gz'); execFileSync('cmake', ['-E', 'tar', 'czf', archive, '.'], { cwd: content }); configureTemplateRegistryRoot(path.join(root, 'registry')); return { root, archive };
}
