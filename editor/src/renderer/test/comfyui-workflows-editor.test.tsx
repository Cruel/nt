...
describe('ComfyUiWorkflowsEditor', () => {
  it('shows manager columns and hides project copy action without an open project', async () => {
  ...
  it('requests overridden rows only when Show overridden is enabled', async () => {
    ...
    expect(window.noveltea.listComfyUiWorkflowLibrary).toHaveBeenLastCalledWith({ projectFilePath: null, includeOverridden: false });
  ...
  it('shows project actions when a project is open and refreshes with the project path', async () => {
    ...
    expect(await screen.findByText('Base Workflow')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /Copy Base Workflow to Project/ })).toBeEnabled();
  ...
  it('runs workflow copy, reveal, delete, and verify actions through the library API', async () => {
    useProjectStore.getState().loadProjectDocument({
      document: createAuthoringProject(),
      projectPath: '/mock/project',
      projectFilePath: '/mock/project/game.json',
    ...
    fireEvent.click(screen.getByRole('button', { name: /Copy Custom Workflow to Project/ }));
    ...
    fireEvent.click(screen.getByRole('button', { name: /Reveal Custom Workflow in folder/ }));
    ...
    fireEvent.click(screen.getByRole('button', { name: /Delete Custom Workflow/ }));
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Verify with ComfyUI' }));
