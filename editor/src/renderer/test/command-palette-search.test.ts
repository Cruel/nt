...
function project() {
  ...
  next.layouts.coolLayout = {
    id: 'coolLayout',
    label: 'Layout lol',
    data: { target: 'cool-layout' } as never,
  };
  next.layouts.sadf = {
    id: 'sadf',
    label: 'Layout sadf',
    data: { target: 'sadf-layout' } as never,
  };
...
describe('command palette search', () => {
  ...
  it('returns top tag match provenance for matched tags', () => {
    ...
    expect(results[0]?.matches).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ fieldKind: 'tag', fieldLabel: 'Tag', value: 'cool' }),
      ]),
    );
  });
  ...
  it('filters selector items by image asset kind', () => {
    const items = filterSelectorItems(buildCommandPaletteItems(project()), {
      collections: ['assets'],
      assetKinds: ['image'],
  ...
  it('filters selector items by entrypoint collections', () => {
    const items = filterSelectorItems(buildCommandPaletteItems(project()), {
      collections: ['rooms', 'scenes', 'dialogues', 'scripts'],
      includeActions: false,
    });
    expect(items.map((item) => item.id)).toEqual(
      expect.arrayContaining([
