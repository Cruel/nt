...
describe('EnginePreview', () => {
  it('renders the lower-level iframe host without preview-manager wrapper state', () => {
    ...
    expect(screen.getByTitle('NovelTea engine preview')).toHaveAttribute(
      'src',
      'http://localhost:4173/widget.html',
    );
  ...
  it('activates the containing workbench group when the preview iframe receives click focus or child interaction', async () => {
    useWorkbenchStore.setState({
      layout: {
        ...
        sizesByChild: { 'group:left': 50, 'group:right': 50 },
      },
      groupsById: {
      ...
      tabsById: {
        'tab:left': {
          ...
          title: 'Left',
          editorType: 'raw-json',
          resource: { kind: 'raw', stableId: 'left' },
        },
        'tab:right': {
          id: 'tab:right',
          title: 'Right',
          editorType: 'engine-preview',
          resource: { kind: 'preview', stableId: 'preview:right' },
        },
      },
      activeGroupId: 'left',
      recentlyClosedTabs: [],
    ...
    render(
      <div data-workbench-group-id="right">
        <EnginePreview />
      </div>,
    );
    const iframe = (await screen.findByTitle('NovelTea engine preview')) as HTMLIFrameElement;
  ...
  it('prefers explicit editor location when preview interaction DOM ancestry is stale', async () => {
    useWorkbenchStore.setState({
      layout: {
        kind: 'split',
        id: 'split',
    ...
    render(
      <div data-workbench-group-id="left">
  ...
  it('minimal embedded previews load preview documents with embedded iframe params', async () => {
    ...
    const modeRequest = editorPort.sent.find(
      (message) => (message as { type?: string }).type === 'set-preview-mode',
    ) as { requestId: string };
    await act(async () => {
      previewPort.postMessage({
        version: 1,
        type: 'command-result',
        requestId: modeRequest.requestId,
        ok: true,
      });
    });
    expect(editorPort.sent).toContainEqual({
  ...
  it('sends preview activity and requests a safe refresh when a dedicated preview becomes visible', async () => {
    ...
    expect(
      editorPort.sent.filter((message) =>
        ['runtime-stop', 'runtime-reset', 'stop'].includes(
