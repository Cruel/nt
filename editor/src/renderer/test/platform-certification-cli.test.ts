...
function setup() {
  ...
  execFileSync('cmake', ['-E', 'tar', 'cf', archive, '--format=zip', 'template.json'], {
    cwd: templateRoot,
  });
  ...
  const evidence = checks.map((check, index) => {
    ...
    return {
      ...
      target: 'web',
      artifact,
      producer: 'github-actions/release/web',
...
describe('platform certification CLI fail-closed behavior', () => {
  it('creates and verifies a report only from complete explicit per-check results', () => {
    ...
    expect(run(['verify', '--archive', value.archive, '--report', value.reportPath]).status).toBe(
      0,
    );
  });
