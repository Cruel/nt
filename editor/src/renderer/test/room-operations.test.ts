...
describe('room commands', () => {
  ...
  it('repairs Character and Interactable initial locations when placements are renamed or removed', () => {
    ...
    guard.initialWorldState.location = { kind: 'room-placement', placement: { room: 'foyer', placement: 'lamp-placement' } };
    ...
    expect(renameResult.document).toMatchObject({ characters: { guard: { data: { initialWorldState: { location: { kind: 'room-placement', placement: { room: 'foyer', placement: 'lamp-anchor' } } } } } } });
    ...
    expect(removeResult.document).toMatchObject({ characters: { guard: { data: { initialWorldState: { location: { kind: 'nowhere' } } } } } });
  });
  ...
  it('repairs Room-local and Interaction placement references on rename and rejects unsafe removal', () => {
    ...
    program.instructions = [{ id: 'move', kind: 'move-interactable', interactable: { $ref: { collection: 'interactables', id: 'lamp' } }, target: { kind: 'room-placement', placement: { room: 'foyer', placement: 'anchor' } } }];
    interaction.rules = [{ id: 'use-lamp', verb: { $ref: { collection: 'verbs', id: 'use' } }, operands: [{ kind: 'any-interactable' }], context: { kind: 'room-placement', placement: { room: 'foyer', placement: 'anchor' } }, program }];
    project.interactions['use-lamp'] = { id: 'use-lamp', label: 'Use Lamp', data: interaction };
    project.interactables.lamp = { id: 'lamp', label: 'Lamp', data: defaultInteractableData('Lamp') };
    let state = createInitialCommandBusState(toJsonValue(project));
    ...
    expect(renameResult.document).toMatchObject({
      ...
      interactions: { 'use-lamp': { data: { rules: [{ context: { placement: { placement: 'renamed-anchor' } }, program: { instructions: [{ target: { placement: { placement: 'renamed-anchor' } } }] } }] } } },
    });
    ...
    expect(removeResult.diagnostics).toEqual(expect.arrayContaining([expect.objectContaining({ message: expect.stringContaining('cannot be removed') })]));
  });
});
