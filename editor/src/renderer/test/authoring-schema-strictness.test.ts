...
describe('authoring schema strictness', () => {
  it('rejects unknown fields at representative nested data in every collection family', () => {
    ...
    const cases: Array<{ name: string; accepted: boolean }> = [
      {
        name: 'assets',
        accepted: assetDataSchema.safeParse({
          kind: 'binary',
          source: { type: 'project-file', path: 'assets/file.bin', unexpected: true },
          aliases: [],
        }).success,
      },
      {
      ...
      {
        name: 'materials',
        accepted: materialDataSchema.safeParse({
          ...defaultMaterialData(),
          preview: { geometry: 'quad', background: 'checker', unexpected: true },
        }).success,
      },
      {
        name: 'layouts',
        accepted: layoutDataSchema.safeParse({
          ...layout,
          rml: { ...layout.rml, unexpected: true },
        }).success,
      },
      {
        name: 'characters',
      ...
      {
        name: 'rooms',
        accepted: roomDataSchema.safeParse({
          ...room,
          background: { ...room.background, unexpected: true },
        }).success,
      },
      {
        name: 'interactables',
        accepted: interactableDataSchema.safeParse({
          ...interactable,
          presentation: { ...interactable.presentation, unexpected: true },
        }).success,
      },
      {
        name: 'verbs',
        accepted: verbDataSchema.safeParse({
          ...defaultVerbData(),
          defaultProgram: {
            instructions: [],
      ...
      {
        name: 'interactions',
        accepted: interactionDataSchema.safeParse({ ...defaultInteractionData(), unexpected: true })
          .success,
      },
      {
        name: 'dialogues',
        accepted: dialogueDataSchema.safeParse({
          ...dialogue,
          blocks: [{ ...dialogue.blocks[0], unexpected: true }],
        }).success,
      },
      {
        name: 'scenes',
        accepted: sceneDataSchema.safeParse({
          ...scene,
          steps: [{ ...defaultSceneStep('wait'), unexpected: true }],
        }).success,
      },
      {
        name: 'maps',
        accepted: mapDataSchema.safeParse({
          ...defaultMapData(),
          presentation: { ...defaultMapData().presentation, unexpected: true },
        }).success,
      },
      {
        name: 'scripts',
        accepted: scriptModuleDataSchema.safeParse({
          ...defaultScriptModuleData(),
          source: { kind: 'inline-lua', source: '', unexpected: true },
        }).success,
      },
      {
        name: 'tests',
        accepted: testDataSchema.safeParse({
          ...test,
          steps: [{ ...test.steps[0], tick: { ...test.steps[0]!.tick, unexpected: true } }],
        }).success,
      },
    ];
