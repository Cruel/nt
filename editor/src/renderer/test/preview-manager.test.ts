...
import type { EntityPreviewRequest, PreviewDocument, ThumbnailRequestInput } from '@/preview/preview-types';
...
describe('preview manager policy', () => {
  it('admits entity previews up to the configured pool size', () => {
    const sessions = {};
    expect(chooseEntityPreviewAdmission(sessions, request('tab-a'), DEFAULT_ENTITY_PREVIEW_POOL_SIZE)).toEqual({
      type: 'create',
  ...
  it('evicts the least-recently-used idle entity session', () => {
    const left = { ...createEntityPreviewSession(request('tab-a'), 'preview-session:entity:tab-a', 1), active: false, lastUsedAt: 10 };
    const right = { ...createEntityPreviewSession(request('tab-b'), 'preview-session:entity:tab-b', 2), active: false, lastUsedAt: 20 };
    expect(chooseEntityPreviewAdmission({ [left.id]: left, [right.id]: right }, request('tab-c', 30), 2)).toEqual({
      type: 'evict',
  ...
  it('rejects when all entity sessions are active', () => {
    ...
    expect(chooseEntityPreviewAdmission({ [left.id]: left, [right.id]: right }, request('tab-c', 30), 2)).toMatchObject({
      type: 'reject',
    });
  });
});
...
describe('preview manager store', () => {
  ...
  it('records bounded entity preview requests and replay documents', () => {
    ...
    expect(usePreviewManagerStore.getState().replay.documentsBySessionId[result.session.id]).toEqual(symbolicDocument);
    expect(usePreviewManagerStore.getState().replay.modeBySessionId[result.session.id]).toBe('symbolic');
  });

  it('releases owners and reuses idle capacity', () => {
    expect(usePreviewManagerStore.getState().requestEntityPreview(request('tab-a', 1)).ok).toBe(true);
    expect(usePreviewManagerStore.getState().requestEntityPreview(request('tab-b', 2)).ok).toBe(true);
    ...
    expect(result.ok).toBe(true);
    expect(usePreviewManagerStore.getState().sessionsById['preview-session:entity:tab-a']?.status).toBe('disposed');
  });
  ...
  it('dedupes and invalidates thumbnail cache entries', () => {
    ...
    usePreviewManagerStore.getState().setThumbnailStatus(first.cacheKey, 'ready', { dataUrl: 'data:image/png;base64,test', now: 3 });
    expect(usePreviewManagerStore.getState().thumbnailRequestsByKey[first.cacheKey]?.status).toBe('ready');
    expect(createThumbnailCacheKey(input)).toBe(first.cacheKey);
    usePreviewManagerStore.getState().invalidateThumbnailsForTarget({ collection: 'materials', entityId: 'mat-a' });
    expect(usePreviewManagerStore.getState().thumbnailRequestsByKey[first.cacheKey]).toBeUndefined();
  });
});
