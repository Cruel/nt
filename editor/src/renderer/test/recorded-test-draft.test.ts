...
describe('recorded test draft conversion', () => {
    ...
    const result = recordedTestDraftToTestData(
      {
        actions: [
          ...
          {
            id: 'select-1',
            kind: 'select-subjects',
            label: 'Select guard and lamp',
            input: {
          ...
          },
          {
            ...
            label: 'Clear',
            input: { type: 'clear-subject-selection' },
          },
          {
            id: 'action-1',
            kind: 'run-interaction',
            label: 'Show lamp to guard',
            input: {
  ...
  it('normalizes runtime action ids into valid authoring step ids', () => {
    const result = recordedTestDraftToTestData({
      actions: [
        { id: '9D7327AF-CLICK', kind: 'continue', label: 'Continue', input: { type: 'continue' } },
      ],
    });
  ...
  it('can be saved as new or applied to existing tests through the command bus', () => {
    ...
    expect(created.document).toMatchObject({
      tests: { recorded: { data: { steps: [{ input: 'tick' }, { input: 'continue' }] } } },
    });
    ...
    expect(applied.document).toMatchObject({
      tests: {
        existing: {
          data: { displayName: 'Recorded', steps: [{ input: 'tick' }, { input: 'continue' }] },
        },
      },
    });
    expect(undoCommand(applied.state).document).toMatchObject({
      tests: { existing: { data: { displayName: 'Existing', steps: [{ input: 'tick' }] } } },
    });
  });
});
