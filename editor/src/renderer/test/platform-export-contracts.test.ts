...
describe('platform export contracts', () => {
  ...
  it('parses a compatible template descriptor and rejects inverted API ranges', () => {
    const descriptor = { format: TEMPLATE_DESCRIPTOR_FORMAT, formatVersion: TEMPLATE_DESCRIPTOR_FORMAT_VERSION, templateId: 'linux-x64', buildId: 'build-1', platform: 'linux', architecture: 'x64', minimumPlatformVersion: 'provisional', graphicsBackends: ['opengl'], shaderVariants: ['glsl-120'], runtimePackageApi: { minimum: 1, maximum: 1 }, compiledFeatures: ['lua'], capabilities: [], buildFlavor: 'release', runtimeDependencies: [{ path: 'licenses/NOTICE.txt', kind: 'notice' }], host: { assembly: 'any', requiresToolchain: false, tools: [] } } as const;
