...
suite('Android player template integration', () => {
  beforeAll(async () => {
    root =
      process.env.NOVELTEA_ANDROID_INTEGRATION_ROOT ??
      (await mkdtemp(path.join(os.tmpdir(), 'noveltea-android-export-')));
    await mkdir(root, { recursive: true });
    configureTemplateRegistryRoot(path.join(root, 'registry'));
  }, 30_000);
  afterAll(async () => {
    if (root && !process.env.NOVELTEA_ANDROID_INTEGRATION_ROOT)
      await rm(root, { recursive: true, force: true });
  });

  it(
    'installs an immutable template and exports a verified APK through the headless export command',
    async () => {
      ...
      const output =
        process.env.NOVELTEA_ANDROID_EXPORT_OUTPUT ?? path.join(root, 'Output ü space');
      ...
      const androidNdk =
        ...
        (androidSdk && process.env.ANDROID_NDK_VERSION
          ? path.join(androidSdk, 'ndk', process.env.ANDROID_NDK_VERSION)
          : '');
      const androidCmake =
        process.env.ANDROID_CMAKE_ROOT ||
        (androidSdk && process.env.ANDROID_CMAKE_VERSION
      ...
      expect(result.deployment?.android).toMatchObject({
        applicationId: 'org.noveltea.platformexportacceptance',
        versionCode: revision,
        abi,
        artifacts: expectedKinds,
      });
      expect(
        result.manifest?.files.find((entry) => entry.origin === 'runtime-package')?.sha256,
      ).toMatch(/^[0-9a-f]{64}$/);
    },
    15 * 60_000,
  );
