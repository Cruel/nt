...
describe('authoring V2 Phase 3A infrastructure', () => {
  it('accepts only Room, Scene, or Dialogue entrypoints and a separate strict startup hook', () => {
    ...
    expect(isAuthoringProject({ ...project, entrypoint: { kind: 'script', id: 'boot' } })).toBe(false);
    expect(isAuthoringProject({ ...project, startupHook: { source: '', yields: false } })).toBe(false);
  });

  it('rejects unknown record and content fields instead of stripping them', () => {
    const project = createAuthoringProject();
    project.rooms.start = { id: 'start', label: 'Start', data: defaultRoomData('Start') };
    expect(isAuthoringProject({ ...project, rooms: { start: { ...project.rooms.start, parent: null } } })).toBe(false);
    expect(isAuthoringProject({
      ...project,
      rooms: { start: { ...project.rooms.start, data: { ...project.rooms.start.data, mystery: true } } },
    })).toBe(false);
  });
  ...
  it('keeps category and tag metadata separate from runtime extends and assignments', () => {
    ...
    project.rooms.child = { id: 'child', label: 'Child', extends: 'area', properties: {}, data: defaultRoomData('Child') };
  ...
  it('validates property declarations, owner allowlists, nullability, and assignment types', () => {
    ...
    project.properties.map = {
      id: 'map', label: 'Map', type: 'string', nullable: false,
      defaultValue: 'main', ownerKinds: ['room'], persistence: 'Save',
    };
    project.rooms.area = { id: 'area', label: 'Area', properties: { map: null }, data: defaultRoomData('Area') };
    expect(validateAuthoringProject(project)).toContainEqual(expect.objectContaining({ path: '/rooms/area/properties/map' }));
  });
  ...
  it('uses globally scoped variable IDs and validates localization catalogs', () => {
    ...
    expect(isAuthoringProject({
      ...
      variables: { route: { id: 'route', label: 'Route', data: { ...defaultVariableData('string'), runtimeName: 'legacy' } } },
    })).toBe(false);
  });
});
