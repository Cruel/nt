...
import {
  checkComfyUiConnection as requestComfyUiConnection,
  getComfyUiQueue as requestComfyUiQueue,
...
function progressChanged(previous: ComfyUiQueueProgress, next: ComfyUiQueueProgress) {
  return (
    previous.promptId !== next.promptId ||
...
export const useComfyUiStore = create<ComfyUiStore>()((set, get) => ({
  ...
  checkConnection: async (overrideConfig, options = {}) => {
    ...
    if (!config.enabled) {
      ...
      const current = get();
      if (visibleStatusChanged(current.status, status) || current.config !== config)
        set({ config, status });
      return status;
    }
    ...
    if (status.state === 'ready')
      void triggerComfyUiWorkflowVerification(
        config,
        useProjectStore.getState().projectFilePath,
        status.comfyUiVersion,
      );
    return status;
  },
  refreshQueue: async (overrideConfig) => {
    ...
    if (progressChanged(get().progress, progress) || visibleStatusChanged(get().status, nextStatus))
      set({ progress, status: nextStatus });
    return progress;
  },
