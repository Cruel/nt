...
const excludedWizardCollections = new Set<AuthoringCollectionKey>(
...
export const newEntityWizardDefinitions = [...typedWizardDefinitions, ...metadataWizardDefinitions]
  .filter((definition) => !excludedWizardCollections.has(definition.collection))
  .sort(
    (left, right) =>
      authoringCollectionKeys.indexOf(left.collection) -
      authoringCollectionKeys.indexOf(right.collection),
  );

export const newEntityWizardDefinitionByCollection: Partial<
  Record<AuthoringCollectionKey, NewEntityWizardTypeDefinition>
> = Object.fromEntries(
...
export function newEntityWizardDefinition(
  collection: AuthoringCollectionKey,
): NewEntityWizardTypeDefinition {
...
export function assertNewEntityWizardCoverage(): AuthoringCollectionKey[] {
  return newEntityWizardCollectionKeys.filter(
    (collection) => !newEntityWizardDefinitionByCollection[collection],
  );
}
