...
async function safeAreaDiagnostic(
  source: string,
  platform: IconPlatform,
  inset: number,
  ...
  return {
    severity: 'warning',
    code: 'unsafe-icon-content',
    platform,
    message: `${platform === 'web' ? 'Maskable' : 'Adaptive'} icon foreground reaches outside the recommended safe area; it may be clipped by platform masks.`,
  };
}
...
export async function generateAppIcons(
      ...
      diagnostics: [
        {
          ...
          message: `Cannot read app icon: ${error instanceof Error ? error.message : String(error)}`,
        },
      ],
    };
  }
  if (!metadata.width || !metadata.height)
    diagnostics.push({
      ...
      message: 'App icon dimensions could not be determined.',
    });
  else {
    if (metadata.width !== metadata.height)
      diagnostics.push({
        ...
        message: `App icon is ${metadata.width}x${metadata.height}; transparent padding will be added to make square outputs.`,
      });
    if (Math.min(metadata.width, metadata.height) < 1024)
      diagnostics.push({
        severity: 'warning',
        code: 'undersized-source',
        message: `App icon is below the recommended 1024x1024 source size and some outputs will be upscaled.`,
      });
  }
  if (metadata.space && metadata.space !== 'srgb')
    diagnostics.push({
      severity: 'warning',
      code: 'color-space-conversion',
      message: `App icon uses ${metadata.space}; generated icons are converted to sRGB.`,
    });
  if (!metadata.hasAlpha && platforms.has('android') && !request.overrides?.androidForeground)
    diagnostics.push({
      severity: 'warning',
      code: 'missing-alpha',
      platform: 'android',
      message:
        'The Android adaptive foreground has no alpha channel; a fully opaque foreground may hide the adaptive background and crop poorly.',
    });
  ...
  const add = (
    ...
    source: 'generated' | 'override',
    size?: number,
  ) => files.push({ platform, kind, path: output, width: size, height: size, source });
  ...
  if (platforms.has('web')) {
    ...
    for (const size of [16, 32, 180, 192, 512])
      add(
        'web',
        size <= 32 ? 'favicon' : 'icon',
        await write(request.stagingRoot, `web/icons/icon-${size}.png`, await png(source, size)),
        request.overrides?.web ? 'override' : 'generated',
        size,
      );
    add(
      'web',
      ...
      await write(
        ...
        ico(
          await Promise.all(
            [16, 32].map(async (size) => ({ size, data: await png(source, size) })),
          ),
        ),
      ),
      request.overrides?.web ? 'override' : 'generated',
    ...
    diagnostics.push(
      ...[await safeAreaDiagnostic(source, 'web', 0.1)].filter(
        (value): value is IconGenerationDiagnostic => value !== null,
      ),
    );
  }
  ...
  if (platforms.has('android')) {
    ...
    for (const [density, size] of [
      ...
      add(
        'android',
        'legacy',
        await write(request.stagingRoot, `android/res/mipmap-${density}/ic_launcher.png`, icon),
        request.overrides?.androidLegacy ? 'override' : 'generated',
        size,
      );
      add(
        'android',
    ...
    for (const [density, size] of [
      ...
      add(
        ...
        await write(
          ...
          await png(foreground, size),
        ),
        request.overrides?.androidForeground ? 'override' : 'generated',
        size,
      );
    const bg = request.overrides?.androidBackground;
    const backgroundXml = bg
      ? `<bitmap xmlns:android="http://schemas.android.com/apk/res/android" android:src="@drawable/ic_launcher_background_image" android:gravity="fill" />\n`
    ...
    if (bg)
      add(
        ...
        await write(
          request.stagingRoot,
          'android/res/drawable/ic_launcher_background_image.png',
          await png(bg, 432),
        ),
        'override',
        432,
    ...
    for (const qualifier of ['mipmap-anydpi-v26', 'mipmap-anydpi-v33']) {
      const monochrome = qualifier.endsWith('v33')
      ...
      add(
        'android',
        'adaptive-xml',
        await write(request.stagingRoot, `android/res/${qualifier}/ic_launcher.xml`, xml),
        'generated',
      );
      add(
        'android',
        'round-adaptive-xml',
        await write(request.stagingRoot, `android/res/${qualifier}/ic_launcher_round.xml`, xml),
        'generated',
      );
    }
    diagnostics.push(
      ...[await safeAreaDiagnostic(foreground, 'android', 0.185)].filter(
        (value): value is IconGenerationDiagnostic => value !== null,
      ),
    );
  }
          ...
          ico(
            await Promise.all(
              [16, 24, 32, 48, 64, 128, 256].map(async (size) => ({
                size,
                data: await png(request.sourcePath, size),
              })),
            ),
          ),
        );
          ...
          icns(
            await Promise.all(
              [
                ...
                type: String(type),
                data: await png(request.sourcePath, Number(size)),
              })),
            ),
          ),
        );
