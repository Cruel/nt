...
function cancelled(operationId: string): PlatformStageResult {
  return { ok: false, success: false, cancelled: true, operationId, diagnostics: [{ severity: 'warning', code: 'export-cancelled', path: '/', message: 'Platform export was cancelled.' }] };
}
...
export async function exportProjectToPlatform(
  ...
  const progress = (stage: PlatformExportProgressEvent['stage'], message: string) => onProgress({ operationId, stage, message });
  try {
  ...
  const loaded = request.project
    ? { success: true, project: request.project, projectPath: request.projectRoot ?? '' } as OpenProjectResponse
    : request.projectPath
  ...
  if (!loaded?.success || !loaded.project || !loaded.projectPath) {
    return failure(operationId, (loaded?.diagnostics ?? []).map((item) => diagnostic('project-load-failed', item.path ?? '/', item.message)).concat(
      loaded ? [] : [diagnostic('project-load-failed', '/projectPath', 'A project path or in-memory project with projectRoot is required.')],
    ));
  }
  ...
  try {
    ...
    return failure(operationId, [diagnostic('invalid-project', '/', error instanceof Error ? error.message : String(error))]);
  }

  const projectRoot = loaded.projectPath;
  const profiles = parseProjectPlatformExportSettings((project.settings as Record<string, unknown>).platformExport);
  const profile = profiles.profiles.find((item) => item.id === request.profileId);
  if (!profile) return failure(operationId, [diagnostic('profile-missing', '/profileId', `Platform export profile '${request.profileId}' does not exist.`)]);

  const validation = validateAuthoringProject(project);
  const validationErrors = validation.filter((item) => item.severity === 'error');
  if (validationErrors.length > 0) {
    return failure(operationId, validationErrors.map((item) => diagnostic('project-validation-failed', item.path, item.message)));
  }
  ...
  checkPlatformExportCancelled(operationId);
  const resolved = request.templateToken ? { success: true, token: request.templateToken, diagnostics: [] } : await resolvePlayerTemplate({ requirements: {
    profile,
    runtimePackageApi: 1,
    playerConfigApi: 1,
    shaderVariants: targetRuntimeProfile.shaderVariants,
  ...
  try {
    ...
    if (error instanceof Error && error.message === 'NOVELTEA_EXPORT_CANCELLED') return cancelled(operationId);
    throw error;
  } finally {
