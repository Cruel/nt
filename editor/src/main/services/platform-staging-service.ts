...
const canvas=document.getElementById('canvas');let runtimeReady=false;let pendingResize=null;let resizeScheduled=false;let appliedResize='';
const measureCanvas=()=>{const rect=canvas.getBoundingClientRect();const dpr=window.devicePixelRatio||1;const logicalWidth=Math.max(1,Math.round(rect.width));const logicalHeight=Math.max(1,Math.round(rect.height));const framebufferWidth=Math.max(1,Math.round(logicalWidth*dpr));const framebufferHeight=Math.max(1,Math.round(logicalHeight*dpr));if(canvas.width!==framebufferWidth)canvas.width=framebufferWidth;if(canvas.height!==framebufferHeight)canvas.height=framebufferHeight;pendingResize={logicalWidth,logicalHeight,framebufferWidth,framebufferHeight,scaleX:framebufferWidth/logicalWidth,scaleY:framebufferHeight/logicalHeight};scheduleResize()};
...
new ResizeObserver(measureCanvas).observe(canvas);window.addEventListener('resize',measureCanvas);measureCanvas();
