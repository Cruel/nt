...
export function checkPlatformExportCancelled(operationId: string) {
  if (cancellations.has(operationId)) throw new Error('NOVELTEA_EXPORT_CANCELLED');
}
export function clearPlatformExportCancellation(operationId: string) {
  cancellations.delete(operationId);
}
function safeRoot(root: string, relative: string) {
  const resolved = path.resolve(root, relative);
  if (resolved !== path.resolve(root) && !resolved.startsWith(`${path.resolve(root)}${path.sep}`))
    throw new Error(`Path '${relative}' escapes its root.`);
  return resolved;
}
async function listFiles(root: string, prefix = ''): Promise<string[]> {
  ...
  for (const entry of await readdir(safeRoot(root, prefix), { withFileTypes: true })) {
    ...
    if (entry.isSymbolicLink())
      throw new Error(`Symbolic link '${relative}' is not allowed in export inputs.`);
...
function classifyTemplate(
  relative: string,
  dependencyKinds: Map<string, StagedFileOrigin>,
): StagedFileOrigin {
  return dependencyKinds.get(relative) ?? 'template';
}
...
function macosEntitlements(capabilities: readonly string[]) {
  ...
  if (capabilities.includes('microphone'))
    entries.push('  <key>com.apple.security.device.audio-input</key>\n  <true/>');
...
async function finalizeMacosStage(
  ...
  if (!descriptor.macosDependencies || !descriptor.macosRpaths || !descriptor.macosMachO) {
    throw new Error(
      'macOS template must declare per-binary Mach-O dependencies, runtime paths, and UUIDs.',
    );
  }
  ...
  if (
    ...
    throw new Error(
      'macOS executable name may contain only letters, numbers, dot, underscore, plus, and hyphen.',
    );
  }
  const bundleId = request.identity.applicationId;
  if (!/^[A-Za-z0-9-]+(?:\.[A-Za-z0-9-]+)+$/.test(bundleId))
    throw new Error(`macOS bundle ID '${bundleId}' is invalid.`);
  ...
  const iconEntry = files.find(
    (item) => item.origin === 'icon' && item.path.endsWith('/macos/AppIcon.icns'),
  );
  ...
  const nativeEntries = files.filter(
    (item) => item.origin === 'native-dependency' && /\.dylib$/i.test(item.path),
  );
  for (const entry of nativeEntries)
    await moveEntry(entry, `${frameworks}/${path.posix.basename(entry.path)}`);
  for (const entry of files.filter(
    (item) => !item.path.startsWith('Contents/') && !item.path.startsWith('symbols/'),
  )) {
    await moveEntry(entry, `${resources}/${entry.path}`);
  }
  const bundledNames = new Set(nativeEntries.map((item) => path.posix.basename(item.path)));
  const systemDependency = (value: string) =>
    value.startsWith('/System/Library/') || value.startsWith('/usr/lib/');
  const stagedMachO = new Map<string, string>();
  stagedMachO.set(playerDescriptor.path, playerEntry.path);
  for (const entry of nativeEntries) {
    const original = descriptor.files.find(
      (item) =>
        item.role === 'native-dependency' &&
        path.posix.basename(item.path) === path.posix.basename(entry.path),
    );
    ...
    for (const binary of descriptor.macosMachO) {
      ...
      for (const dependency of binary.dependencies) {
        ...
        if (dependency !== replacement)
          await run('install_name_tool', ['-change', dependency, replacement, absolute]);
      }
    }
    if (
      nativeEntries.length &&
      !descriptor.macosRpaths.includes('@executable_path/../Frameworks')
    ) {
      await run('install_name_tool', [
        '-add_rpath',
        '@executable_path/../Frameworks',
        safeRoot(stage, playerEntry.path),
      ]);
    }
  }
  ...
  files.push(
    await writeGenerated(
      ...
      `CFBundleDisplayName = "${request.identity.displayName.replaceAll('"', '\\"')}";\n`,
  ...
  for (const symbol of symbolEntries) {
    symbolFiles.push({
      path: symbol.path.slice('symbols/'.length),
      data: await readFile(safeRoot(stage, symbol.path)),
    });
  ...
  return {
    executablePath: playerEntry.path,
    entitlements,
    frameworkPaths: nativeEntries.map((item) => item.path),
    symbolFiles,
  };
}
...
async function finalizeLinuxStage(
  ...
  if (
    ...
    throw new Error(
      'Linux executable name may contain only letters, numbers, dot, underscore, plus, and hyphen.',
    );
  }
  ...
  const forbiddenRpath = descriptor.linuxRpaths.find(
    (item) => item.startsWith('/') || /(^|:)(?:\/home|\/tmp|[^:]*build\/)/.test(item),
  );
  if (forbiddenRpath)
    throw new Error(`Linux template contains nonportable runtime path '${forbiddenRpath}'.`);
  const nativeLibraries = new Set(
    files
      .filter(
        (item) =>
          item.origin === 'native-dependency' &&
          item.path !== playerEntry.path &&
          /\.so(?:\.|$)/.test(item.path),
      )
  ...
  const unresolved = descriptor.linuxNeeded.filter(
    (name) => !nativeLibraries.has(name) && !systemLibraries.some((pattern) => pattern.test(name)),
  );
  if (unresolved.length)
    throw new Error(
      `Linux player has unresolved non-system ELF dependencies: ${unresolved.join(', ')}`,
    );
  if (nativeLibraries.size && !descriptor.linuxRpaths.some((item) => item.includes('$ORIGIN'))) {
    throw new Error(
      'Linux templates with bundled native libraries must use an $ORIGIN-relative runtime path.',
    );
  }

  const id = linuxDesktopId(request);
  const iconEntries = files.filter(
    (item) => item.origin === 'icon' && item.path.includes('/linux/hicolor/'),
  );
  ...
  const symbolEntries = descriptor.files.filter(
    (item) => item.role === 'symbol' || /\.debug$/i.test(item.path),
  );
  const symbolFiles: Array<{ path: string; data: Buffer }> = [];
  for (const symbol of symbolEntries) {
    ...
    symbolFiles.push({
      ...
      data: await readFile(safeRoot(stage, entry.path)),
    });
...
async function buildLinuxAppImage(
  ...
  output: string,
  request: PlatformStageRequest,
  linux: NonNullable<Awaited<ReturnType<typeof finalizeLinuxStage>>>,
) {
  const appDir = `${stage}.AppDir`;
  await rm(appDir, { recursive: true, force: true });
  await mkdir(appDir, { recursive: true });
  for (const file of await listFiles(stage))
    await copyFileTracked(safeRoot(stage, file), appDir, file, 'template', 'appimage-stage');
  await writeFile(
  ...
  await writeFile(
    path.join(appDir, `${linuxDesktopId(request)}.desktop`),
    await readFile(desktopSource),
    { mode: 0o644 },
  );
  ...
  await writeFile(
    path.join(appDir, `${linuxDesktopId(request)}.png`),
    await readFile(safeRoot(stage, largestIcon.path)),
    { mode: 0o644 },
  );
  const tool = request.linuxAppImageTool ?? 'appimagetool';
  await rm(output, { force: true });
  try {
    await run(tool, [appDir, output], {
      cwd: path.dirname(stage),
      env: {
        ...process.env,
        ARCH: request.profile.architecture === 'arm64' ? 'aarch64' : 'x86_64',
      },
    });
  } catch (error) {
    throw new Error(
      `AppImage creation requires a working appimagetool (${error instanceof Error ? error.message : String(error)}).`,
    );
  }
  await rm(appDir, { recursive: true, force: true });
}
...
function parseWindowsVersion(versionName: string): [number, number, number, number] {
  const parts = versionName
    ...
    .filter(Boolean)
    .slice(0, 4)
    .map((part) => Math.min(65535, Number.parseInt(part, 10) || 0));
...
async function finalizeWindowsStage(
  stage: string,
  request: PlatformStageRequest,
  files: StagedFileEntry[],
  descriptor: Awaited<ReturnType<typeof verifyTemplateToken>>['descriptor'],
  ...
  if (executable.newHeader.optionalHeader.subsystem !== 2)
    throw new Error('Windows release player must use the GUI subsystem.');
  ...
  resources.replaceResourceEntryFromString(
    ...
    1,
    1033,
    windowsManifest(request.identity.applicationId),
  );
  ...
  const undeclaredDlls = files.filter(
    (item) => /\.dll$/i.test(item.path) && item.origin !== 'native-dependency',
  );
  if (undeclaredDlls.length)
    throw new Error(
      `Windows template contains undeclared DLL dependencies: ${undeclaredDlls.map((item) => item.path).join(', ')}`,
    );
  if (!descriptor.windowsImports)
    throw new Error('Windows template must declare its imported DLL closure.');
  const stagedDlls = new Set(
    files
      .filter((item) => /\.dll$/i.test(item.path))
      .map((item) => path.posix.basename(item.path).toLowerCase()),
  );
  ...
  const missingImports = descriptor.windowsImports.filter(
    (name) =>
      !stagedDlls.has(name) &&
      !systemDlls.has(name) &&
  ...
  if (missingImports.length)
    throw new Error(
      `Windows player has unresolved non-system DLL imports: ${missingImports.join(', ')}`,
    );

  const symbolEntries = descriptor.files.filter(
    (item) => item.role === 'symbol' || /\.pdb$/i.test(item.path),
  );
  const symbolFiles: Array<{ path: string; data: Buffer }> = [];
  for (const symbol of symbolEntries) {
    const entry = files.find((item) => item.path === symbol.path);
...
async function runWindowsSigningHook(
  stage: string,
  request: PlatformStageRequest,
  files: StagedFileEntry[],
  ...
  if (!request.windowsSigning.verifyCommand)
    throw new Error('Windows signing requires an independent verification command.');
  const signable = files
    .filter((item) => /\.(?:exe|dll)$/i.test(item.path))
    .sort(
      (left, right) =>
        Number(/\.exe$/i.test(left.path)) - Number(/\.exe$/i.test(right.path)) ||
        left.path.localeCompare(right.path),
    );
  if (!signable.some((item) => item.path === executablePath))
    throw new Error('Windows executable is missing from the ordered signing subject set.');
  const absoluteExecutable = safeRoot(stage, executablePath);
  const expand = (value: string, subject: string) =>
    value
  ...
  for (const entry of signable) {
    ...
    await run(
      request.windowsSigning.command,
      request.windowsSigning.args.map((value) => expand(value, absoluteSubject)),
      { cwd: stage },
    );
    await run(
      request.windowsSigning.verifyCommand,
      (request.windowsSigning.verifyArgs ?? []).map((value) => expand(value, absoluteSubject)),
      { cwd: stage },
    );
...
async function finalizeWebStage(
  stage: string,
  request: PlatformStageRequest,
  files: StagedFileEntry[],
) {
  ...
  const find = (suffix: string) =>
    files.find((item) => item.path === suffix || item.path.endsWith(`/${suffix}`));
  const script = find('player.js');
  const wasm = find('player.wasm');
  const data = find('player.data');
  const packageEntry = files.find((item) => item.origin === 'runtime-package');
  if (!script || !wasm || !data || !packageEntry)
    throw new Error(
      'Web template must contain player.js, player.wasm, and player.data plus the runtime package.',
    );
  ...
  const manifestId =
    request.identity.webManifestId ?? `/${encodeURIComponent(request.identity.applicationId)}`;
  ...
  const iconEntries = allWebIcons.filter((item) =>
    /icon-(192|512)\.[0-9a-f]{16}\.png$/.test(item.path),
  );
  const icon = (size: number) =>
    iconEntries.find((item) => new RegExp(`icon-${size}\\.[0-9a-f]{16}\\.png$`).test(item.path))
      ?.path;
  const webManifest = {
    id: manifestId,
  ...
  const exportHash = sha256(
    Buffer.from(
      immutable.map((item) => files.find((file) => file.path === item)?.sha256 ?? '').join(''),
    ),
  ).slice(0, 20);
  const cachePrefix = `noveltea-${request.identity.applicationId.replace(/[^a-zA-Z0-9._-]/g, '_')}-`;
  const serviceWorker =
    profile.pwa && profile.serviceWorker === 'offline'
      ? `
...
<html lang="${htmlEscape(request.identity.defaultLocale ?? 'en')}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><title>${htmlEscape(request.identity.displayName)}</title><link rel="manifest" href="${basePath}manifest.webmanifest">${favicon ? `<link rel="icon" href="${basePath}${favicon}">` : ''}<meta name="theme-color" content="${themeColor}"><style>html,body{width:100%;height:100%;margin:0;background:${backgroundColor}}body{position:relative;overflow:hidden;font-family:system-ui,sans-serif}#canvas{position:absolute;left:0;top:0;display:block;max-width:none;max-height:none}#launch,#failure{position:fixed;inset:0;display:grid;place-content:center;text-align:center;padding:2rem;background:${backgroundColor};color:white;z-index:2}#failure{display:none}#fps-counter{display:none;position:fixed;left:16px;bottom:16px;z-index:3;padding:4px 6px;border-radius:4px;background:rgba(15,23,42,.84);color:#cbd5e1;font:11px/1.2 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;pointer-events:none}button{font:inherit;padding:.8rem 1.2rem}</style></head><body><canvas id="canvas" tabindex="-1"></canvas><div id="launch"><h1>${htmlEscape(request.identity.displayName)}</h1><button id="start">Start</button></div><div id="failure"><h1>Unable to start</h1><p id="failure-message"></p></div><div id="fps-counter"></div><script>
...
const params=new URLSearchParams(window.location.search);const showFps=['1','true','yes'].includes((params.get('showFps')||'').toLowerCase());const fpsCap=Math.max(0,Math.min(1000,Number.parseInt(params.get('fpsCap')||'0',10)||0));const requestedRmluiSnap=(params.get('rmluiSnap')||'all').toLowerCase();const rmluiSnap=['none','geometry','text','all'].includes(requestedRmluiSnap)?requestedRmluiSnap:'all';
globalThis.NovelTeaPreviewBridge={send(){},markEngineReady(){},setFpsCounter(fps,frameTimeMs,cap){const counter=document.getElementById('fps-counter');if(!counter)return;if(!showFps||!Number.isFinite(fps)||fps<=0){counter.style.display='none';counter.textContent='';return}counter.textContent=fps.toFixed(1)+' fps · '+frameTimeMs.toFixed(2)+' ms · '+(cap>0?'engine cap '+cap+', display paced':'display paced');counter.style.display='block'}};
const canvas=document.getElementById('canvas');let runtimeReady=false;let pendingResize=null;let resizeScheduled=false;let appliedResize='';let dprMediaQuery=null;
const currentDevicePixelRatio=()=>{const dpr=Number(window.devicePixelRatio);return Number.isFinite(dpr)&&dpr>0?dpr:1};
// BEGIN web-player-surface-metrics
function resolveWebPlayerSurfaceMetrics(cssWidth,cssHeight,devicePixelRatio){const measuredCssWidth=Number.isFinite(cssWidth)&&cssWidth>0?cssWidth:1;const measuredCssHeight=Number.isFinite(cssHeight)&&cssHeight>0?cssHeight:1;const dpr=Number.isFinite(devicePixelRatio)&&devicePixelRatio>0?devicePixelRatio:1;const logicalWidth=Math.max(1,Math.round(measuredCssWidth));const logicalHeight=Math.max(1,Math.round(measuredCssHeight));const framebufferWidth=Math.max(1,Math.round(measuredCssWidth*dpr));const framebufferHeight=Math.max(1,Math.round(measuredCssHeight*dpr));return{logicalWidth,logicalHeight,framebufferWidth,framebufferHeight,scaleX:framebufferWidth/logicalWidth,scaleY:framebufferHeight/logicalHeight}}
// END web-player-surface-metrics
const measureCanvas=()=>{const rect=document.body.getBoundingClientRect();const resize=resolveWebPlayerSurfaceMetrics(rect.width,rect.height,currentDevicePixelRatio());if(!runtimeReady){if(canvas.width!==resize.framebufferWidth)canvas.width=resize.framebufferWidth;if(canvas.height!==resize.framebufferHeight)canvas.height=resize.framebufferHeight;canvas.style.width=rect.width+'px';canvas.style.height=rect.height+'px'}pendingResize={...resize,cssWidth:rect.width,cssHeight:rect.height};scheduleResize()};
const applyCanvasPresentation=r=>{const capacityWidth=typeof Module._noveltea_player_backbuffer_width==='function'?Module._noveltea_player_backbuffer_width():canvas.width;const capacityHeight=typeof Module._noveltea_player_backbuffer_height==='function'?Module._noveltea_player_backbuffer_height():canvas.height;if(capacityWidth<=0||capacityHeight<=0)return false;canvas.style.width=r.cssWidth*capacityWidth/r.framebufferWidth+'px';canvas.style.height=r.cssHeight*capacityHeight/r.framebufferHeight+'px';return true};
const applyResize=()=>{if(!runtimeReady||!pendingResize||typeof Module._noveltea_player_resize!=='function')return;const r=pendingResize;const key=r.logicalWidth+'x'+r.logicalHeight+':'+r.framebufferWidth+'x'+r.framebufferHeight+':'+r.scaleX+'x'+r.scaleY;if(key===appliedResize){applyCanvasPresentation(r);return}if(Module._noveltea_player_resize(r.logicalWidth,r.logicalHeight,r.framebufferWidth,r.framebufferHeight,r.scaleX,r.scaleY)!==1){scheduleResize();return}appliedResize=key;applyCanvasPresentation(r);requestAnimationFrame(()=>{if(pendingResize)applyCanvasPresentation(pendingResize)})};
const scheduleResize=()=>{if(resizeScheduled)return;resizeScheduled=true;requestAnimationFrame(()=>{resizeScheduled=false;applyResize()})};
const handleDevicePixelRatioChange=()=>{armDevicePixelRatioWatcher();measureCanvas()};
const armDevicePixelRatioWatcher=()=>{if(dprMediaQuery){dprMediaQuery.removeEventListener('change',handleDevicePixelRatioChange);dprMediaQuery=null}if(typeof window.matchMedia!=='function')return;dprMediaQuery=window.matchMedia('(resolution: '+currentDevicePixelRatio()+'dppx)');dprMediaQuery.addEventListener('change',handleDevicePixelRatioChange,{once:true})};
armDevicePixelRatioWatcher();new ResizeObserver(measureCanvas).observe(document.body);window.addEventListener('resize',measureCanvas);window.addEventListener('pageshow',measureCanvas);document.addEventListener('visibilitychange',()=>{if(!document.hidden)measureCanvas()});measureCanvas();
document.getElementById('start').addEventListener('click',()=>{document.getElementById('launch').style.display='none';measureCanvas();const args=['--rmlui-snap',rmluiSnap];if(showFps)args.push('--show-fps');if(fpsCap>0)args.push('--fps-cap',String(fpsCap));window.Module={canvas,arguments:args,locateFile:p=>p==='player.wasm'?${JSON.stringify(`${basePath}${wasm.path}`)}:p==='player.data'?${JSON.stringify(`${basePath}${data.path}`)}:${JSON.stringify(basePath)}+p,onRuntimeInitialized:()=>{runtimeReady=true;measureCanvas();scheduleResize()},onNovelTeaStartupError:fail};const s=document.createElement('script');s.src=${JSON.stringify(`${basePath}${script.path}`)};s.onerror=()=>fail('The player could not be downloaded.');document.body.appendChild(s)});
...
export async function stagePlatformExport(
  ...
  const symbolArchivePath =
    (request.profile.target === 'windows' ||
      request.profile.target === 'linux' ||
      request.profile.target === 'macos') &&
    request.profile.includeDebugSymbols
      ? `${request.outputDirectory}-symbols.${request.profile.target === 'linux' ? 'tar.gz' : 'zip'}`
      : undefined;
  const symbolArchiveTemp = symbolArchivePath
    ? `${symbolArchivePath}.tmp-${request.operationId.replace(/[^a-zA-Z0-9_-]/g, '_')}`
    : undefined;
  const symbolArchiveBackup = symbolArchivePath
    ? `${symbolArchivePath}.previous-${request.operationId.replace(/[^a-zA-Z0-9_-]/g, '_')}`
    : undefined;
  const appImagePath =
    request.profile.target === 'linux' && request.profile.desktop.artifact === 'appimage'
      ? `${request.outputDirectory}.AppImage`
      : undefined;
  const appImageTemp = appImagePath
    ? `${appImagePath}.tmp-${request.operationId.replace(/[^a-zA-Z0-9_-]/g, '_')}`
    : undefined;
  const appImageBackup = appImagePath
    ? `${appImagePath}.previous-${request.operationId.replace(/[^a-zA-Z0-9_-]/g, '_')}`
    : undefined;
  const dmgPath =
    request.profile.target === 'macos' && request.macosDmg
  ...
  const dmgBackup = dmgPath
    ...
    : undefined;
  const signingReportPath =
    request.profile.target === 'macos' && request.macosSigning
      ? `${request.outputDirectory.replace(/\.app$/i, '')}-signing-report.json`
      : undefined;
  const signingReportTemp = signingReportPath
    ? `${signingReportPath}.tmp-${request.operationId.replace(/[^a-zA-Z0-9_-]/g, '_')}`
    : undefined;
  ...
  try {
    ...
    let templateRoot;
    try {
      templateRoot = templateRootForToken(request.templateToken);
      ({ descriptor } = await verifyTemplateToken(request.templateToken));
    } catch (error) {
      return {
        ok: false,
        success: false,
    ...
    const built = buildPlatformDeployment(request, descriptor);
    ...
    if (!request.iconSourcePath)
      diagnostics.push(
        diagnostic(
          'missing-icon',
          '/iconSourcePath',
          'Application icon is required for platform staging.',
        ),
      );
    else if (!existsSync(request.iconSourcePath))
      diagnostics.push(
        diagnostic('missing-icon', '/iconSourcePath', 'Application icon does not exist.'),
      );
    const templateFiles = (await listFiles(templateRoot)).filter(
      (file) => file !== descriptorName && file !== '.noveltea-template.json',
    );
      ...
      if (forbidden.test(file))
        diagnostics.push(
          diagnostic(
            'sandbox-content',
            `/template/${file}`,
            `Sandbox/demo content '${file}' is forbidden.`,
          ),
        );
    for (const dependency of descriptor.runtimeDependencies)
      if (!templateFiles.includes(dependency.path))
        diagnostics.push(
          diagnostic(
            'missing-template-dependency',
            `/template/${dependency.path}`,
            `Declared template dependency '${dependency.path}' is missing.`,
          ),
        );
    const prospective = [
      ...templateFiles.map((targetPath) => ({ sourceId: `template:${targetPath}`, targetPath })),
      ...
      return {
        ...
        diagnostics: [
          diagnostic(
            ...
            '/outputDirectory',
            'Not enough disk space to build and atomically replace staging output.',
          ),
        ],
        deployment: built.model,
      };
    ...
    const dependencyKinds = new Map(
      descriptor.runtimeDependencies.map((item) => [
        item.path,
        item.kind === 'notice'
          ? ('notice' as const)
          : item.kind === 'library'
            ? ('native-dependency' as const)
    ...
    if (request.systemAssetsRoot)
      for (const file of await listFiles(request.systemAssetsRoot)) {
        checkPlatformExportCancelled(request.operationId);
        files.push(
          await copyFileTracked(
            safeRoot(request.systemAssetsRoot, file),
            temp,
    ...
    const iconResult = await generateAppIcons({
      ...
      stagingRoot: path.join(temp, '.icons'),
      platforms: [
        request.profile.target === 'linux'
          ? 'linux'
          : request.profile.target === 'web'
            ? 'web'
            : request.profile.target === 'android'
              ? 'android'
              : request.profile.target === 'windows'
                ? 'windows'
                : 'macos',
    ...
    const iconTargets = iconResult.files.map((icon) =>
      path.posix.join(
        'icons',
        path.relative(path.join(temp, '.icons'), icon.path).split(path.sep).join('/'),
      ),
    );
    const allTargets = [
      ...files.map((item) => ({ sourceId: item.originId, targetPath: item.path })),
      ...iconTargets.map((targetPath) => ({ sourceId: `icon:${targetPath}`, targetPath })),
      { sourceId: 'player-config', targetPath: 'player.json' },
    ...
    if (diagnostics.some((item) => item.severity === 'error'))
      throw new Error('NOVELTEA_EXPORT_DIAGNOSTIC');
    for (let index = 0; index < iconResult.files.length; index += 1) {
      const icon = iconResult.files[index]!;
      files.push(await copyFileTracked(icon.path, temp, iconTargets[index]!, 'icon', icon.kind));
    }
    ...
    const macos = await finalizeMacosStage(
      temp,
      { ...request, capabilities: built.model.capabilities },
      files,
      descriptor,
    );
    const windowsSigning = windows
      ? await runWindowsSigningHook(temp, request, files, windows.targetPath)
      : undefined;
          ...
          `${JSON.stringify(
            {
              ...
              signed: true,
              verified: windowsSigning.verified,
              timestamped: null,
    ...
    const packageEntry = files.find((item) => item.origin === 'runtime-package')!;
    const playerConfigPath = linux
      ? path.posix.join(path.posix.dirname(linux.executablePath), 'player.json')
      : macos
        ? 'Contents/Resources/player.json'
        : 'player.json';
    const packageFromConfig = path.posix.relative(
      path.posix.dirname(playerConfigPath),
      packageEntry.path,
    );
    const player = {
      ...
      versionName: built.model.versionName,
    ...
    files.push({
      ...
      origin: 'generated-metadata',
      originId: 'player-config',
      size: playerData.length,
      mode: 0o644,
      sha256: sha256(playerData),
    });
    ...
    if (macos && request.macosSigning) {
      ...
      for (const frameworkPath of macos.frameworkPaths) {
        await run('codesign', [
          ...
          'runtime',
          '--sign',
          request.macosSigning.identity,
          safeRoot(temp, frameworkPath),
        ]);
      }
      const signingArgs = ['--force', '--options', 'runtime'];
      let generatedEntitlementsPath: string | undefined;
      const entitlements =
        request.macosSigning.entitlementsPath ??
        (macos.entitlements ? `${temp}.entitlements.plist` : undefined);
    ...
    if (archivePath)
      artifacts.push({ kind: 'archive', path: archivePath, size: (await stat(archivePath)).size });
    if (appImagePath)
      artifacts.push({
        kind: 'appimage',
        path: appImagePath,
        size: (await stat(appImagePath)).size,
      });
    if (request.profile.target === 'macos')
      artifacts[0] = { kind: 'app-bundle', path: request.outputDirectory };
    if (dmgPath) artifacts.push({ kind: 'dmg', path: dmgPath, size: (await stat(dmgPath)).size });
    if (symbolArchivePath && existsSync(symbolArchivePath))
      artifacts.push({
        kind: 'symbols',
        path: symbolArchivePath,
        size: (await stat(symbolArchivePath)).size,
      });
    if (signingReportPath && existsSync(signingReportPath))
      artifacts.push({
        kind: 'signing-report',
        path: signingReportPath,
        size: (await stat(signingReportPath)).size,
      });
    return {
      ok: true,
      success: true,
      ...
      archivePath,
      symbolArchivePath:
        symbolArchivePath && existsSync(symbolArchivePath) ? symbolArchivePath : undefined,
      artifacts,
      webMetrics,
      diagnostics,
      deployment: built.model,
    ...
    clearPlatformExportCancellation(request.operationId);
  }
}
