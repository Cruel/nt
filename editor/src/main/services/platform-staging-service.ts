...
const measureCanvas=()=>{const rect=canvas.getBoundingClientRect();const resize=resolveWebPlayerSurfaceMetrics(rect.width,rect.height,currentDevicePixelRatio());if(canvas.width!==resize.framebufferWidth)canvas.width=resize.framebufferWidth;if(canvas.height!==resize.framebufferHeight)canvas.height=resize.framebufferHeight;pendingResize=resize;scheduleResize()};
const applyResize=()=>{if(!runtimeReady||!pendingResize||typeof Module._noveltea_player_resize!=='function')return;const r=pendingResize;const key=r.logicalWidth+'x'+r.logicalHeight+':'+r.framebufferWidth+'x'+r.framebufferHeight+':'+r.scaleX+'x'+r.scaleY;if(key===appliedResize)return;Module._noveltea_player_resize(r.logicalWidth,r.logicalHeight,r.framebufferWidth,r.framebufferHeight,r.scaleX,r.scaleY);appliedResize=key};
...
armDevicePixelRatioWatcher();new ResizeObserver(measureCanvas).observe(canvas);window.addEventListener('resize',measureCanvas);window.addEventListener('pageshow',measureCanvas);document.addEventListener('visibilitychange',()=>{if(!document.hidden)measureCanvas()});measureCanvas();
