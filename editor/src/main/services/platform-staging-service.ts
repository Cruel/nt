...
document.getElementById('start').addEventListener('click',()=>{document.getElementById('launch').style.display='none';measureCanvas();window.Module={canvas,locateFile:p=>p==='player.wasm'?${JSON.stringify(`${basePath}${wasm.path}`)}:p==='player.data'?${JSON.stringify(`${basePath}${data.path}`)}:${JSON.stringify(basePath)}+p,onRuntimeInitialized:()=>{runtimeReady=true;measureCanvas();scheduleResize()},onNovelTeaStartupError:fail};const s=document.createElement('script');s.src=${JSON.stringify(`${basePath}${script.path}`)};s.onerror=()=>fail('The player could not be downloaded.');document.body.appendChild(s)});
