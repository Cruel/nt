  ...
  if (blocking) {
    return {
      ...
      error:
        ...
        'Workflow import analysis failed.',
    };
  }
...
export async function saveImportedComfyUiWorkflow(
  request: ComfyUiSaveImportedWorkflowRequest,
): Promise<ComfyUiSaveImportedWorkflowResponse> {
  if (!request.projectFilePath) {
    return {
      ok: false,
      success: false,
      diagnostics: [
        diagnostic('/projectFilePath', 'Save the project before importing ComfyUI workflows.'),
      ],
  ...
  try {
    const workflowFileName = safeImportFileName(
      request.workflowFileName,
      '.workflow.json',
      'workflowFileName',
    );
    const manifestFileName = safeImportFileName(
    ...
    const bindingDiagnostics = validateManifestBindings(
    ...
    const message =
      error instanceof Error ? error.message : 'Failed to save imported ComfyUI workflow.';
    return {
      ok: false,
      success: false,
...
export async function repairComfyUiWorkflowManifest(
  request: ComfyUiRepairWorkflowManifestRequest,
): Promise<ComfyUiSaveImportedWorkflowResponse> {
  if (!request.projectFilePath) {
    return {
      ok: false,
      success: false,
      diagnostics: [
        diagnostic('/projectFilePath', 'Save the project before repairing ComfyUI workflows.'),
      ],
      error: 'Project file path is required.',
    };
  }
  ...
  try {
    ...
    await writeFileAtomic(
      path.join(workflowsRoot, manifestFileName),
