...
export const materialTextureFilteringValues = ['clamp-nearest', 'clamp-linear', 'repeat-nearest', 'repeat-linear'] as const;
...
export function isMaterialRecord(record: AuthoringRecordBase | undefined | null): record is AuthoringRecordBase & { data: MaterialData } {
  return !!record && parseMaterialData(record.data) !== null;
}

export function materialDataFromRecord(record: AuthoringRecordBase | undefined | null): MaterialData | null {
...
export function resolveMaterialData(project: AuthoringProject, materialId: string): { data: MaterialData | null; diagnostics: MaterialSchemaDiagnostic[] } {
...
export function validateMaterialData(project: AuthoringProject, materialId: string, record: AuthoringRecordBase): MaterialSchemaDiagnostic[] {
  const diagnostics: MaterialSchemaDiagnostic[] = [];
  ...
  if (!data.shader) {
    ...
    if (!shaderRecord) {
      diagnostics.push(diagnostic(`${base}/shader/$ref`, `Missing shader '${data.shader.$ref.id}'.`));
    } else {
  ...
  const uniformDeclarations = new Map<string, ShaderUniformData>((shader?.uniforms ?? []).map((item) => [item.name, item]));
  const uniforms = new Set<string>();
  data.uniforms.forEach((uniform, index) => {
    const path = `${base}/uniforms/${index}`;
    if (uniforms.has(uniform.name)) diagnostics.push(diagnostic(`${path}/name`, `Duplicate uniform override '${uniform.name}'.`));
    uniforms.add(uniform.name);
    const declaration = uniformDeclarations.get(uniform.name);
    if (!declaration) diagnostics.push(diagnostic(`${path}/name`, `Material assigns undeclared shader uniform '${uniform.name}'.`));
    else if (!isUniformValueCompatible(declaration.type, uniform.value)) diagnostics.push(diagnostic(`${path}/value`, `Uniform override does not match ${declaration.type}.`));
  });
  ...
  data.textures.forEach((texture, index) => {
    ...
    if (textures.has(texture.sampler)) diagnostics.push(diagnostic(`${path}/sampler`, `Duplicate texture slot '${texture.sampler}'.`));
    textures.add(texture.sampler);
    if (!samplerDeclarations.has(texture.sampler)) diagnostics.push(diagnostic(`${path}/sampler`, `Material assigns undeclared shader sampler '${texture.sampler}'.`));
    validateTextureSource(project, texture.source, `${path}/source`, diagnostics);
  });
  return diagnostics;
}

function validateTextureSource(project: AuthoringProject, source: MaterialTextureSource, path: string, diagnostics: MaterialSchemaDiagnostic[]) {
  if ('$ref' in source) {
    ...
    if (!data) diagnostics.push(diagnostic(`${path}/$ref`, `Asset '${source.$ref.id}' has invalid asset data.`));
    else if (data.kind !== 'image') diagnostics.push(diagnostic(`${path}/$ref`, `Asset '${source.$ref.id}' is ${data.kind}, not image.`, 'warning'));
  }
}
