...
export const roomEnvironmentPlaneValues = ['world-background', 'world-content', 'world-overlay'] as const;
...
export const roomNavigationTransitionSchema = strict({
  kind: z.enum(roomNavigationTransitionKindValues), durationMs: z.number().int().nonnegative(),
  color: z.string().nullable(), skippable: z.boolean(),
});
export const roomOverlayDataSchema = strict({
  id: entityIdSchema, layout: roomLayoutRefSchema, condition: conditionSchema,
  visible: z.boolean(), order: z.number().int(),
});
...
export const roomCastDataSchema = strict({
  id: entityIdSchema, character: roomCharacterRefSchema, condition: conditionSchema,
  placementId: entityIdSchema, poseId: entityIdSchema.nullable(), expressionId: entityIdSchema.nullable(), idleId: entityIdSchema.nullable().default(null),
  visible: z.boolean(), order: z.number().int(),
});
export const roomPropDataSchema = strict({
  id: entityIdSchema, condition: conditionSchema, placementId: entityIdSchema,
  asset: roomAssetRefSchema.nullable(), material: roomMaterialRefSchema.nullable(),
  visible: z.boolean(), order: z.number().int(),
});
...
export const roomExitDataSchema = strict({
  ...
  target: roomRoomRefSchema, condition: conditionSchema, transition: roomNavigationTransitionSchema.nullable().optional(),
});
...
export const roomDataSchema = strict({
  ...
  cast: z.array(roomCastDataSchema), props: z.array(roomPropDataSchema), environments: z.array(roomEnvironmentDataSchema).default([]), compose: roomCompositionHookSchema.nullable(),
...
export function defaultRoomData(label = 'Room'): RoomData {
  return {
    ...
    description: inlineTextContent(), overlays: [], placements: [], cast: [], props: [], environments: [], compose: null, exits: [],
...
export const roomCharacterRef = (id: string): RoomCharacterRef => ({ $ref: { collection: 'characters', id } });
...
export function validateRoomNavigationTransition(value: z.infer<typeof roomNavigationTransitionSchema>, path: string, diagnostics: RoomSchemaDiagnostic[]) {
  if (value.kind === 'cut' && value.durationMs !== 0) diagnostics.push(diagnostic(`${path}/durationMs`, 'Cut transitions require zero duration.'));
  if (value.kind !== 'cut' && value.durationMs === 0) diagnostics.push(diagnostic(`${path}/durationMs`, 'Animated transitions require a positive duration.'));
  if (value.kind !== 'fade' && value.color !== null) diagnostics.push(diagnostic(`${path}/color`, 'Only Fade transitions may specify a color.'));
}
export function validateRoomData(project: AuthoringProject, roomId: string, record: AuthoringRecordBase): RoomSchemaDiagnostic[] {
  ...
  uniqueIds(data.overlays, `${base}/overlays`, 'overlay', diagnostics); uniqueIds(data.exits, `${base}/exits`, 'exit', diagnostics); uniqueIds(data.placements, `${base}/placements`, 'placement', diagnostics); uniqueIds(data.cast, `${base}/cast`, 'cast', diagnostics); uniqueIds(data.props, `${base}/props`, 'prop', diagnostics); uniqueIds(data.environments, `${base}/environments`, 'environment', diagnostics);
  const placements = new Set(data.placements.map((placement) => placement.id));
  data.overlays.forEach((overlay, index) => { const layout = project.layouts[overlay.layout.$ref.id]; if (!layout) diagnostics.push(diagnostic(`${base}/overlays/${index}/layout/$ref`, `Missing layout '${overlay.layout.$ref.id}'.`)); else if (!parseLayoutData(layout.data)) diagnostics.push(diagnostic(`${base}/overlays/${index}/layout/$ref`, `Layout '${overlay.layout.$ref.id}' is invalid.`, 'warning')); validateCondition(project, overlay.condition, `${base}/overlays/${index}/condition`, diagnostics); });
  data.exits.forEach((exit, index) => { if (!project.rooms[exit.target.$ref.id]) diagnostics.push(diagnostic(`${base}/exits/${index}/target/$ref`, `Missing target room '${exit.target.$ref.id}'.`)); else if (exit.target.$ref.id === roomId) diagnostics.push(diagnostic(`${base}/exits/${index}/target/$ref`, 'Exit targets the current room.', 'warning')); validateCondition(project, exit.condition, `${base}/exits/${index}/condition`, diagnostics); if (exit.transition) validateRoomNavigationTransition(exit.transition, `${base}/exits/${index}/transition`, diagnostics); });
  data.placements.forEach((placement, index) => { if (placement.presentation.layout && !project.layouts[placement.presentation.layout.$ref.id]) diagnostics.push(diagnostic(`${base}/placements/${index}/presentation/layout/$ref`, `Missing layout '${placement.presentation.layout.$ref.id}'.`)); });
  data.cast.forEach((entry, index) => { const path = `${base}/cast/${index}`; const character = project.characters[entry.character.$ref.id]; const characterData = character && typeof character.data === 'object' && character.data !== null ? character.data as { poses?: { id: string }[]; expressions?: { id: string; poseId?: string | null }[]; idles?: { id: string }[] } : null; if (!character) diagnostics.push(diagnostic(`${path}/character/$ref`, `Missing character '${entry.character.$ref.id}'.`)); if (!placements.has(entry.placementId)) diagnostics.push(diagnostic(`${path}/placementId`, `Missing placement '${entry.placementId}'.`)); const pose = characterData?.poses?.find((item) => item.id === entry.poseId); const expression = characterData?.expressions?.find((item) => item.id === entry.expressionId); const idle = characterData?.idles?.find((item) => item.id === entry.idleId); if (entry.poseId && !pose) diagnostics.push(diagnostic(`${path}/poseId`, `Missing pose '${entry.poseId}'.`)); if (entry.expressionId && !expression) diagnostics.push(diagnostic(`${path}/expressionId`, `Missing expression '${entry.expressionId}'.`)); if (entry.idleId && !idle) diagnostics.push(diagnostic(`${path}/idleId`, `Missing idle '${entry.idleId}'.`)); if (entry.poseId && expression?.poseId && expression.poseId !== entry.poseId) diagnostics.push(diagnostic(path, 'Cast pose and expression are incompatible.')); validateCondition(project, entry.condition, `${path}/condition`, diagnostics); });
  data.props.forEach((entry, index) => { const path = `${base}/props/${index}`; if (!placements.has(entry.placementId)) diagnostics.push(diagnostic(`${path}/placementId`, `Missing placement '${entry.placementId}'.`)); if (!entry.asset && !entry.material) diagnostics.push(diagnostic(path, 'Room prop requires an asset and/or material.')); if (entry.asset && !project.assets[entry.asset.$ref.id]) diagnostics.push(diagnostic(`${path}/asset/$ref`, `Missing asset '${entry.asset.$ref.id}'.`)); if (entry.material && !project.materials[entry.material.$ref.id]) diagnostics.push(diagnostic(`${path}/material/$ref`, `Missing material '${entry.material.$ref.id}'.`)); validateCondition(project, entry.condition, `${path}/condition`, diagnostics); });
  data.environments.forEach((entry, index) => { const path = `${base}/environments/${index}`; if (entry.asset && !project.assets[entry.asset.$ref.id]) diagnostics.push(diagnostic(`${path}/asset/$ref`, `Missing asset '${entry.asset.$ref.id}'.`)); if (!project.materials[entry.material.$ref.id]) diagnostics.push(diagnostic(`${path}/material/$ref`, `Missing material '${entry.material.$ref.id}'.`)); validateCondition(project, entry.condition, `${path}/condition`, diagnostics); });
  if (data.compose && !project.scripts[data.compose.script.$ref.id]) diagnostics.push(diagnostic(`${base}/compose/script/$ref`, `Missing script '${data.compose.script.$ref.id}'.`));
