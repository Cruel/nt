...
export function parseExportCommandArguments(argv: string[]): ExportCommandArguments {
  ...
  if (!projectPath || !profileId || !outputDirectory) {
    throw new Error(
      'Usage: NovelTea Editor --export-project --project <project> --profile <profile-id> --output <directory> [--config <local-settings.json>] [--json]',
    );
  }
  return {
    projectPath,
    profileId,
    outputDirectory,
    configPath: valueAfter(argv, '--config'),
...
export function exitCodeForExportResult(result: PlatformStageResult) {
  ...
  if (
    codes.has('profile-missing') ||
    codes.has('invalid-project') ||
    codes.has('project-validation-failed') ||
    codes.has('runtime-conversion-failed')
  )
    return 2;
...
export async function runExportCommand(
  args: ExportCommandArguments,
): Promise<{ result: PlatformStageResult; output: string; exitCode: number }> {
  let localState: Parameters<typeof exportProjectToPlatform>[0]['localState'];
  if (args.configPath)
    localState = JSON.parse(await readFile(args.configPath, 'utf8')) as typeof localState;
