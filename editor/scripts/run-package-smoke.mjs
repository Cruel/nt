...
const candidates = process.platform === 'win32'
  ...
  : process.platform === 'darwin'
    ? [path.join(editorRoot, 'out', 'NovelTea Editor-darwin-arm64', 'NovelTea Editor.app', 'Contents', 'MacOS', 'NovelTea Editor')]
...
if (!executable || !existsSync(executable)) {
  console.error('Packaged NovelTea Editor executable not found. Run pnpm package first or pass its path.');
  process.exit(2);
}
...
child.on('exit', (code, signal) => {
  ...
  if (!line) {
    console.error(`Package smoke produced no result marker (exit=${code}, signal=${signal ?? 'none'}).`);
    process.exit(1);
  }
