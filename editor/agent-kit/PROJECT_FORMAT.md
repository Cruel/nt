...
Editable project source lives in `project.json`, `properties.json`, `traits.json`, `localization.json`, `editor.json`, `records/`, `scripts/`, and `assets/`. Root `AGENTS.md` contains project instructions; follow it and do not edit its marked NovelTea-managed block. Never edit or track `.noveltea/` as project source.
...
Current record collections include Assets, Variables, Shaders, Materials, Layouts, Archetypes, Characters, Rooms, Interactables, Item Definitions, Item Stacks, Verbs, Interactions, Dialogues, Scenes, Maps, Script Modules, and Tests. The generated `schemas/records/` directory contains one schema for each tracked record collection; use the focused docs first to understand semantics, then consult the schema for exact fields and variants.
