...
NovelTea projects are file-first workspaces. Edit tracked JSON, Lua, RML, and RCSS directly for ordinary authoring changes. After direct edits to managed localizable Lua/RML, run `noveltea localization sync`, then run `noveltea validate`. Use the CLI for semantic operations that need whole-project knowledge or transactions: localization sync, Asset import, create, rename, delete, usages, shader compilation, tests, and package export.
