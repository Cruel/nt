...
Layouts use RmlUi RML/RCSS plus optional dedicated Lua. For file-mode Layout channels, edit the companion `layout.rml`, `layout.rcss`, or `layout.lua` file directly. RML/RCSS may instead use Asset mode; dedicated Lua uses file or none mode and is never a Lua Asset. Do not replace a non-file channel with a companion file behind the workspace's back.
